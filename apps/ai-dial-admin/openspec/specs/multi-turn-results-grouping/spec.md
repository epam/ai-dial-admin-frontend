# multi-turn-results-grouping Specification

## Purpose
Define how the run results grid groups flat result rows into logical single- and multi-turn
conversations by `multiTurnId`, and how those conversations are rendered as collapsible, filterable
turn views while keeping detail interactions keyed to each result's `id`.
