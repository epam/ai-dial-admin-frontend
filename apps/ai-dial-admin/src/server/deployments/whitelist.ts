...
export class WhitelistApi extends BaseApi {
  getGlobalWhitelist(token: Token | undefined): Promise<ServerActionResponse> {
    return this.getAction(IMAGES_WHITELIST, token);
  }

  updateGlobalWhitelist(domains: string[], token: Token | undefined): Promise<ServerActionResponse> {
