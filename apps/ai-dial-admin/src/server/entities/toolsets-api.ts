...
export class ToolsetsApi extends BaseApi {
  getToolsetList(token: Token | undefined): Promise<Toolset[] | null> {
    return this.get(TOOLSETS_URL, token);
  }

  getToolset(name: string, token: Token | undefined, eTag: string) {
    return this.getActionWithEtag(TOOLSET_URL(name), eTag, token);
  }

  getTools(name: string, token: Token | undefined): Promise<Tool[] | null> {
  ...
  signOutToolset(toolset: Toolset, type: ToolsetAuthCredentialLevel, token: Token | undefined) {
    return this.postAction(TOOLSET_SIGN_OUT_URL, getToolsetBasicBody(toolset, type), token);
  }

  removeToolset(token: Token | undefined, name?: string): Promise<ServerActionResponse> {
    return this.deleteAction(TOOLSET_URL(name), token);
  }

  createToolset(toolset: Toolset, token: Token | undefined): Promise<ServerActionResponse> {
    return this.postAction(TOOLSETS_URL, toolset, token);
  }

  updateToolset(toolset: Toolset, token: Token | undefined, eTag: string): Promise<ServerActionResponse> {
    return this.putActionWithEtag(TOOLSET_URL(encodeURIComponent(toolset.name || '')), toolset, token, eTag);
  }

  getCoreToolset(name: string, token: Token | undefined) {
