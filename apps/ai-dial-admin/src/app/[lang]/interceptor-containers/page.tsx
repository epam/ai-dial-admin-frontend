...
export default async function Page() {
  ...
  const containers = allContainers.filter((container) => container.$type === CONTAINER_TYPE.INTERCEPTOR);
  const names = allContainers.map((container) => container.name || '');

  return (
    <SaveValidationContextProvider>
      <ContainersList route={ApplicationRoute.InterceptorContainers} containersList={containers} names={names} />
    </SaveValidationContextProvider>
  );
}
