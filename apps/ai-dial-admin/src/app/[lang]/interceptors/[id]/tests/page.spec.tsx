...
vi.mock('@/src/components/Interceptors/View/View', () => ({ __esModule: true, default: () => null }));

vi.mock('@/src/app/[lang]/models/actions', () => ({ getModelsList: vi.fn().mockResolvedValue([]) }));
vi.mock('@/src/app/[lang]/interceptor-templates/actions', () => ({ getInterceptorTemplate: vi.fn() }));
vi.mock('@/src/app/[lang]/interceptors/actions', () => ({ getConfigFileInterceptor: vi.fn() }));
vi.mock('@/src/app/api/api', () => ({
  ...
  interceptorsApi: {
    getInterceptor: vi.fn().mockResolvedValue({ etag: 'e', response: { name: 'my-interceptor' } }),
  },
  settingsApi: { getSystemProperties: vi.fn().mockResolvedValue({ response: { globalInterceptors: [] } }) },
}));
...
import { applicationsApi, applicationRunnersApi, interceptorsApi } from '@/src/app/api/api';
import { redirect } from 'next/navigation';
import Page from '../page';

const renderPage = (searchParams: Record<string, string> = {}) =>
  Page({ params: Promise.resolve({ id: 'my-interceptor' }), searchParams: Promise.resolve(searchParams) });

describe('interceptors/[id] page — configFile fallback', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  test('renders instead of redirecting when configFile=true, without the admin API', async () => {
    vi.stubEnv('DIAL_ADMIN_API_URL', undefined);
    vi.mocked(getConfigFileInterceptor).mockResolvedValue({ success: true, data: { name: 'my-interceptor' } as any });

    await renderPage({ configFile: 'true' });

    expect(redirect).not.toHaveBeenCalled();
    expect(getConfigFileInterceptor).toHaveBeenCalledWith('my-interceptor');
    expect(interceptorsApi.getInterceptor).not.toHaveBeenCalled();
  ...
  test('uses the admin-backend read path when configFile is absent and the admin API is set', async () => {
    ...
    expect(interceptorsApi.getInterceptor).toHaveBeenCalled();
    expect(getConfigFileInterceptor).not.toHaveBeenCalled();
  });
});
