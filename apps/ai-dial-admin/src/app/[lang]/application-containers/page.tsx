...
export default async function Page() {
  ...
  try {
    ...
    errorObjLog(e, 'Failed to application containers data');
  }
  ...
  const containers = allContainers.filter((container) => container.$type === CONTAINER_TYPE.APPLICATION);
  const names = allContainers.map((container) => container.name || '');

  return (
    <SaveValidationContextProvider>
      <ContainersList route={ApplicationRoute.ApplicationContainers} containersList={containers} names={names} />
    </SaveValidationContextProvider>
  );
}
