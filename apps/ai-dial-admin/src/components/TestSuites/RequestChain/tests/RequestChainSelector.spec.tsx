...
describe('RequestChainSelector', () => {
  ...
  test('calls onRemove with the selected request index when removed', async () => {
    const user = userEvent.setup();
    ...
    await user.click(screen.getByRole('button', { name: TestSuitesI18nKey.RemoveRequest }));

    expect(onRemove).toHaveBeenCalledWith(2);
  });

  test('does not render a remove affordance while the first request is selected', () => {
    render(
      <RequestChainSelector
        testSuite={baseSuite}
        selectedIndex={0}
  ...
  test('marks the selected request tab as active', () => {
    render(
      <RequestChainSelector
        testSuite={baseSuite}
        selectedIndex={1}
    ...
    expect(screen.getByRole('tab', { selected: true })).toHaveTextContent('2. Second request');
  });
