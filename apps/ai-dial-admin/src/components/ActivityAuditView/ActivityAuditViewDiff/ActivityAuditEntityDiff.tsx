...
const ActivityAuditEntityDiff: FC<Props> = ({ currentEntity, compareEntity, type, diffView }) => {
      ...
      <div id="activity-audit-diff" className="flex-1 flex flex-row gap-4 w-full min-h-0 mb-4 overflow-auto">
        <div className="flex-1 flex flex-col gap-6 min-h-0 ">
          {Object.entries(sections).map(([key, value]) => (
            <ActivityAuditEntityDiffSection type={type} sections={value} name={key} key={key} diffView={diffView} />
          ))}
        </div>
      </div>
