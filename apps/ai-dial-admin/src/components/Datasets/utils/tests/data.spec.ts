...
describe('rowToDatasetTestCase', () => {
  ...
  test('should include both multi-turn fields when both present (turnIndex 0 counts)', () => {
    const result = rowToDatasetTestCase({ id: 'tc', createdAt: 0, data: {}, multiTurnId: 'conv-1', turnIndex: 0 });

    expect(result.multiTurnId).toBe('conv-1');
    expect(result.turnIndex).toBe(0);
  });

  test('should omit both when only one of the multi-turn fields is present', () => {
    const onlyId = rowToDatasetTestCase({ id: 'tc', createdAt: 0, data: {}, multiTurnId: 'conv-1' });
    const onlyTurn = rowToDatasetTestCase({ id: 'tc', createdAt: 0, data: {}, turnIndex: 1 });

    expect(onlyId).not.toHaveProperty('multiTurnId');
    expect(onlyId).not.toHaveProperty('turnIndex');
    expect(onlyTurn).not.toHaveProperty('multiTurnId');
    expect(onlyTurn).not.toHaveProperty('turnIndex');
  });
});

describe('getDatasetTestCaseGridData multi-turn passthrough', () => {
  test('top-level multiTurnId/turnIndex land on the grid row', () => {
    const result = getDatasetTestCaseGridData([
      { testCaseName: 'c1', data: { q: 'a' }, multiTurnId: 'conv-1', turnIndex: 2 },
    ]);
