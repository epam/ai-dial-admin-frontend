...
describe('ExportRunModal', () => {
  ...
  it('renders modal title with run id', async () => {
  ...
  it('fetches preview on mount with the correct runId', async () => {
  ...
  it('fetches preview exactly once on mount', async () => {
    render(<ExportRunModal runId="run-1" onClose={vi.fn()} />);
    await waitFor(() => expect(mockExportRunPreview).toHaveBeenCalledTimes(1));
  });

  it('Export CSV button is enabled after preview loads', async () => {
    render(<ExportRunModal runId="run-1" onClose={vi.fn()} />);
    await waitFor(() => {
  ...
  it('Export CSV button is disabled while exporting', async () => {
  ...
  it('shows error state when preview fetch fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(ExportRunI18nKey.PreviewLoadError)).toBeInTheDocument();
    });
  });
