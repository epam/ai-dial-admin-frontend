...
const PreviewAccordion: FC<Props> = ({ previewData, checkedColumns, isLoading, error }) => {
  const t = useI18n();
  ...
  const content = useMemo(() => {
    ...
    return (
      <div className="flex flex-col gap-2">
        <p className="text-secondary text-sm">{t(ExportRunI18nKey.PreviewDescription)}</p>
        <div className="h-64">
          <AgGridWrapper columnDefs={columnDefs} rowData={rowData} isLiveData={true} />
        </div>
      </div>
    );
  }, [columnDefs, error, isLoading, rowData, t]);

  return (
    <Accordion title={t(ExportRunI18nKey.PreviewAccordionLabel)} collapsed={true}>
      {content}
    </Accordion>
  );
};
