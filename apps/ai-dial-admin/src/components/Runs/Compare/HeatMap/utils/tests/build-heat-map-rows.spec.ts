...
describe('buildHeatMapRows', () => {
  ...
  test('maps cell values per test case column for each run', () => {
    ...
    expect(primaryRow?.values[formatHeatMapTestCaseColId('tc-1')]).toBe(0.5);
    expect(primaryRow?.values[formatHeatMapTestCaseColId('tc-2')]).toBe(0.2);
    expect(comparedRow?.values[formatHeatMapTestCaseColId('tc-1')]).toBe(0.8);
    expect(comparedRow?.values[formatHeatMapTestCaseColId('tc-2')]).toBeNull();
  });
