...
describe('buildComparisonSections', () => {
  ...
  it('builds two-column sections for pinned + active', () => {
    ...
    expect(tc!.rows[0].values[0].raw).toBe('a');
    expect(tc!.rows[0].values[1].raw).toBe('b');
  });
  ...
  it('handles null/missing values with union of keys', () => {
    ...
    expect(extraRow!.values[0].raw).toBe('y');
    expect(extraRow!.values[1].raw).toBeNull();
  });
  ...
  it('unions metric groups across results', () => {
    ...
    expect(groupA.rows[0].values[1].raw).toBeNull();
  });
