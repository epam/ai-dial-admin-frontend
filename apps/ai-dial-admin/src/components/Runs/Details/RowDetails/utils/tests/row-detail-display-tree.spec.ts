...
const field = (fieldKey: string, label: string): RowDetailSection['rows'][number] => ({
  fieldKey,
  label,
  primaryRaw: 'a',
  secondaryRaw: 'b',
...
describe('buildRowDetailDisplayTree', () => {
  ...
  test('hides fields listed in defaultHiddenFields on fresh tree', () => {
    const hidden = new Set(['status']);
    const tree = buildRowDetailDisplayTree(sections, undefined, hidden);

    expect(getChildren(tree[0]).find((c) => keyOf(c) === 'status')?.hide).toBe(true);
    expect(getChildren(tree[0]).find((c) => keyOf(c) === 'duration')?.hide).toBe(false);
  });

  test('shows all fields when defaultHiddenFields is empty', () => {
    const withDefaults = buildRowDetailDisplayTree(
      [
        {
          key: 'execution',
          label: 'Execution',
          rows: [field('runNumber', '# Run number'), field('httpStatusCode', 'HTTP')],
        },
      ],
      undefined,
      new Set(),
    ...
    expect(getChildren(withDefaults[0]).every((c) => c.hide !== true)).toBe(true);
  });
