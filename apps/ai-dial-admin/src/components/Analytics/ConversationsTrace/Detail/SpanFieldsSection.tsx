...
import SpanGroupPanel from '@/src/components/Analytics/ConversationsTrace/Detail/SpanGroupPanel';
import { useSpanFieldGroups } from '@/src/components/Analytics/ConversationsTrace/Detail/use-span-field-groups';
import { ConversationsTraceI18nKey } from '@/src/constants/i18n';
import { useI18n } from '@/src/locales/client';
import { SpanFieldGroup, SpanFieldRow } from '@/src/models/analytics/conversations-trace';
...
const SpanFieldsSection: FC<Props> = ({ groups, row }) => {
  ...
  return (
    <section className="flex min-h-0 flex-col">
      {groups.length === 0 ? (
        <p className="text-secondary dial-tiny-text">{t(ConversationsTraceI18nKey.SpanFieldsUnavailable)}</p>
      ) : (
