...
import { ButtonsI18nKey, SessionsTraceI18nKey } from '@/src/constants/i18n';
...
import {
  ...
  SessionGridContext,
  SessionRow,
  SessionValueFilterModel,
  SessionValuesState,
} from '@/src/models/analytics/sessions-trace';
import { formatCompactNumber } from '@/src/utils/analytics/session-formatting';
import { getErrorNotification } from '@/src/utils/notification';

const STATE_MESSAGE_KEY: Partial<Record<SessionValuesState, string>> = {
  [SessionValuesState.Loading]: SessionsTraceI18nKey.ValueFilterLoading,
  [SessionValuesState.Empty]: SessionsTraceI18nKey.ValueFilterEmpty,
  [SessionValuesState.LoadFailed]: SessionsTraceI18nKey.ValueFilterLoadFailed,
};
...
type Props = CustomFilterProps<SessionRow, SessionGridContext, SessionValueFilterModel>;
 ...
 */
const SessionValueFilter: FC<Props> = ({ model, onModelChange, colDef, context }) => {
  const t = useI18n();
  const { showNotification } = useNotification();

  const [state, setState] = useState(SessionValuesState.Loading);
  const [failureMessage, setFailureMessage] = useState<string | null>(null);
  const [values, setValues] = useState<SessionFieldValue[]>([]);
  ...
  useEffect(() => {
    ...
    let isCurrent = true;
    setState(SessionValuesState.Loading);
    ...
    const read = async () => {
      ...
      if (!resolved.success) {
        ...
        setState(SessionValuesState.LoadFailed);
        showNotification(
          getErrorNotification(
            resolved.errorHeader ?? t(SessionsTraceI18nKey.ValuesLoadFailed),
      ...
      setFailureMessage(null);
      setState(resolved.response?.length ? SessionValuesState.Available : SessionValuesState.Empty);
    };
  ...
  const isAvailable = state === SessionValuesState.Available;
  const isLoading = state === SessionValuesState.Loading;
    ...
    <div className="flex w-[205px] flex-col gap-2 bg-layer-4 p-3">
      ...
      <span
        ...
        className={classNames(
          ...
          state === SessionValuesState.LoadFailed ? 'text-error' : 'text-secondary',
        )}
      >
        {stateMessage}
      </span>
      ...
      {isAvailable && values.length > SEARCH_THRESHOLD && (
        <DialInput
          ...
          placeholder={t(SessionsTraceI18nKey.ValueFilterSearch)}
          value={search}
      ...
      {isAvailable && (
        <>
          <DialCheckbox
            ...
            label={t(SessionsTraceI18nKey.ValueFilterSelectAll)}
            onChange={onToggleAllVisible}
          />
          <div
            role="group"
            aria-label={t(SessionsTraceI18nKey.ValueFilterGroup, { column: colDef.headerName ?? fieldName })}
...
export default SessionValueFilter;
