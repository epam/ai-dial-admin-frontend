...
import { SESSIONS_HEADER_STACK_HEIGHT, SESSIONS_TIME_PERIOD } from '@/src/constants/analytics/sessions-trace';
...
const searchBox = () => screen.getByPlaceholderText(SessionsTraceI18nKey.SearchPlaceholder);

const lastRequest = (): SessionPageRequest => getSessions.mock.calls.at(-1)?.[0];
...
const okPage = (rows: SessionRow[], extras: PageExtras = { period: PERIOD }) => ({
  success: true,
  response: {
    rows,
    total: extras.period?.totals ? Number(extras.period.totals.sessions) : null,
    ...extras,
  },
});

const laterPage = (rows: SessionRow[]) => ({ success: true, response: { rows, total: null } });
...
const SCHEMA_FIELDS = [
  { name: SessionsField.ChatId, type: AnalyticsFieldType.String, source: SessionsField.ChatId },
  { name: SessionsField.TotalTokens, type: AnalyticsFieldType.Integer, source: SessionsField.TotalTokens },
  { name: SessionsField.InsightTopics, type: AnalyticsFieldType.String, source: 'topics' },
] as AnalyticsEntityField[];

const renderView = (schemaFields?: AnalyticsEntityField[]) => {
  const result = render(<SessionsTraceView schemaFields={schemaFields} />);
...
beforeEach(() => {
  ...
  getSessions.mockResolvedValue(okPage(FIRST_PAGE));
});

describe('SessionsTraceView :: header', () => {
  test('renders the page heading and the provenance line', () => {
    renderView();

    expect(screen.getByRole('heading', { name: SessionsTraceI18nKey.Title })).toBeInTheDocument();
...
describe('SessionsTraceView :: paging', () => {
  ...
  test('requests a later block with a larger offset, changing nothing else', async () => {
    ...
    getSessions.mockResolvedValue(laterPage(SECOND_PAGE));
  ...
  test('falls back to a short block when the summary is unavailable', async () => {
    getSessions.mockResolvedValue({ success: true, response: { rows: FIRST_PAGE, total: null } });
    renderView();
  ...
  test('leaves the end of the result unknown when a full block arrives without a total', async () => {
    ...
    getSessions.mockResolvedValue({ success: true, response: { rows: fullBlock, total: null } });
    renderView();
...
describe('SessionsTraceView :: feedback candidates', () => {
  const CANDIDATES: SessionCandidateIds = { ids: ['a', 'b'], isCapped: false };

  const selectNegative = async () => {
    const user = userEvent.setup();
    await user.click(screen.getByText(SessionsTraceI18nKey.FeedbackNegative));
  };

  test('sends no ids on the first block, letting the request resolve them', async () => {
    getSessions.mockResolvedValue(okPage(FIRST_PAGE, { period: PERIOD, candidates: CANDIDATES }));
    renderView();
  ...
  test('does not carry ids resolved under a previous filter state', async () => {
    ...
    await user.click(screen.getByText(SessionsTraceI18nKey.FeedbackPositive));
    await fetchBlock(PAGE_SIZE, PAGE_SIZE * 2);
...
describe('SessionsTraceView :: filters', () => {
  test('a search term is debounced into one request carrying the term', async () => {
    ...
    const terms = getSessions.mock.calls.map((call) => (call[0] as SessionPageRequest).search);
...
describe('SessionsTraceView :: sort and column filters', () => {
  const SORT_MODEL = [{ colId: SessionsField.TotalPrice, sort: 'desc' }];
  const FILTER_MODEL = { [SessionsField.ProjectId]: { type: 'contains', filter: 'acme' } };
  ...
  test('a sort model reaches the server action as sort keys', async () => {
    ...
    expect(lastRequest().sort).toEqual([{ field: SessionsField.TotalPrice, direction: QuerySortDirection.Desc }]);
  });
  ...
  test('changing the filter model re-requests the first block under the new filter', async () => {
    ...
    getSessions.mockResolvedValue(okPage(SECOND_PAGE));
    await fetchBlock(0, PAGE_SIZE, { filterModel: FILTER_MODEL });
...
describe('SessionsTraceView :: capped feedback result', () => {
  const cappedIds: SessionCandidateIds = {
  ...
  test('discloses that a capped feedback result may be incomplete', async () => {
    getSessions.mockResolvedValue(okPage(FIRST_PAGE, { period: PERIOD, candidates: cappedIds }));
    renderView();

    await selectNegative();
    await fetchBlock();

    await waitFor(() => expect(screen.getByText(SessionsTraceI18nKey.FeedbackCappedNotice)).toBeInTheDocument());
  });

  test('shows no disclosure when the candidate set is below the limit', async () => {
    getSessions.mockResolvedValue(okPage(FIRST_PAGE, { period: PERIOD, candidates: { ids: ['a'], isCapped: false } }));
    renderView();

    await selectNegative();
    await fetchBlock();

    expect(screen.queryByText(SessionsTraceI18nKey.FeedbackCappedNotice)).not.toBeInTheDocument();
  });

  test('drops the disclosure when the request fails', async () => {
    getSessions.mockResolvedValue({ success: false, status: 500 });
    renderView();
  ...
  test('clears the disclosure when the feedback filter returns to all', async () => {
    ...
    await user.click(screen.getByText(SessionsTraceI18nKey.FeedbackAll));
    getSessions.mockResolvedValue(okPage(FIRST_PAGE));
    await fetchBlock();

    await waitFor(() => expect(screen.queryByText(SessionsTraceI18nKey.FeedbackCappedNotice)).not.toBeInTheDocument());
  });
});

describe('SessionsTraceView :: schema availability', () => {
  test('reports a failed schema read by notification rather than as a toolbar notice', async () => {
    ...
    expect(screen.queryByText(SessionsTraceI18nKey.SchemaLoadFailed)).not.toBeInTheDocument();
  });

  test('falls back to its own title when the service supplied no header', async () => {
    render(<SessionsTraceView schemaFields={null} schemaFailure={{}} />);

    await waitFor(() =>
      expect(showNotificationSpy).toHaveBeenCalledWith(
        expect.objectContaining({ title: SessionsTraceI18nKey.SchemaLoadFailed }),
      ),
    );
  });
...
describe('SessionsTraceView :: projection', () => {
  ...
  test('sends every source-backed field even with its column hidden', async () => {
    columnState = [{ colId: SessionsField.TotalTokens, hide: true }];
    renderView(SCHEMA_FIELDS);
    await waitFor(() => expect(datasource).toBeDefined());

    await fetchBlock();

    expect(lastRequest().sourceFields).toContain(SessionsField.TotalTokens);
  });

  test('sends an enrichment-backed field only while its column is visible', async () => {
    columnState = [{ colId: SessionsField.InsightTopics, hide: false }];
    renderView(SCHEMA_FIELDS);
    await waitFor(() => expect(datasource).toBeDefined());

    await fetchBlock();

    expect(lastRequest().visibleEnrichmentFields).toEqual([SessionsField.InsightTopics]);
  });

  test('omits a hidden enrichment-backed column', async () => {
    columnState = [{ colId: SessionsField.InsightTopics, hide: true }];
    renderView(SCHEMA_FIELDS);
    await waitFor(() => expect(datasource).toBeDefined());
  ...
  test('projects the identity column title with no column of its own', async () => {
    columnState = [{ colId: SessionsField.ChatId, hide: false }];
    renderView([
      ...SCHEMA_FIELDS,
      { name: SessionsField.InsightTitle, type: AnalyticsFieldType.String, source: 'title' },
    ] as AnalyticsEntityField[]);
    await waitFor(() => expect(datasource).toBeDefined());

    await fetchBlock();

    expect(lastRequest().visibleEnrichmentFields).toContain(SessionsField.InsightTitle);
  });

  test('never projects a grid-only column, even though it is visible', async () => {
    columnState = [
      { colId: SessionColumn.Rating, hide: false },
      { colId: SessionsField.ChatId, hide: false },
    ];
    renderView(SCHEMA_FIELDS);
    await waitFor(() => expect(datasource).toBeDefined());
  ...
  test('sends no schema-driven fields at all when the schema is unavailable', async () => {
    columnState = [{ colId: SessionsField.TotalTokens, hide: false }];
    renderView();
...
describe('SessionsTraceView :: projecting a heavy field', () => {
...
describe('SessionsTraceView :: revealing a column', () => {
  ...
  test('does not restart paging for a source-backed column', async () => {
    ...
    revealColumn(SessionsField.TotalTokens);
  ...
  test('restarts paging for an enrichment-backed column', async () => {
    ...
    revealColumn(SessionsField.InsightTopics);
  ...
  test('does not restart paging for a curated column', async () => {
    ...
    revealColumn(SessionsField.ChatId);

    expect(purgeInfiniteCache).not.toHaveBeenCalled();
  });
});

describe('SessionsTraceView :: hiding a filtered column', () => {
  // A filter outliving its column keeps narrowing every later page with nothing on screen to explain it, and
  // on an enrichment field the narrowing is severe: only sessions the evaluation has reached can match.
  test('clears the filter the hidden column carried', async () => {
    filterModel = { [SessionsField.InsightTopics]: { type: 'contains', filter: 'security' } };
    renderView(SCHEMA_FIELDS);
    await awaitGridReady();

    hideColumn(SessionsField.InsightTopics);
  ...
  test('leaves every other column filter standing', async () => {
    filterModel = {
      [SessionsField.InsightTopics]: { type: 'contains', filter: 'security' },
      [SessionsField.ProjectId]: { type: 'contains', filter: 'data-team' },
    };
    renderView(SCHEMA_FIELDS);
    await awaitGridReady();
  ...
  test('does not touch the filter model when the hidden column carried no filter', async () => {
    filterModel = { [SessionsField.ProjectId]: { type: 'contains', filter: 'data-team' } };
    renderView(SCHEMA_FIELDS);
    await awaitGridReady();
...
describe('SessionsTraceView :: period figures', () => {
  ...
  test('names the selected period on every pill, spelled as the control spells it', async () => {
    ...
    const preset = timePeriodOptionsConfig.find((option) => option.value === SESSIONS_TIME_PERIOD);

    await waitFor(() => expect(screen.getAllByText(preset?.label as string)).toHaveLength(4));
    expect(screen.queryByText(SESSIONS_TIME_PERIOD)).not.toBeInTheDocument();
  });
});

describe('SessionsTraceView :: empty and failed states', () => {
  test('renders the no-data state when the result holds no sessions', async () => {
    getSessions.mockResolvedValue(
      okPage([], { period: { totals: { sessions: 0, cost: null }, ratings: { rated: 0, negative: 0 } } }),
    );
    renderView();

    await fetchBlock();

    await waitFor(() => expect(screen.getByText(SessionsTraceI18nKey.NoSessions)).toBeInTheDocument());
  });
  ...
  test('the empty state leaves the grid header and its filter row uncovered', async () => {
    ...
    const overlay = (await screen.findByText(SessionsTraceI18nKey.NoSessions)).closest('[style*="top"]') as HTMLElement;

    expect(overlay).toBeTruthy();
    expect(overlay.style.top).toBe(`${SESSIONS_HEADER_STACK_HEIGHT}px`);
  ...
  test('the failed state leaves the header uncovered too, so a filter can still be cleared', async () => {
    ...
    const overlay = (await screen.findByText(SessionsTraceI18nKey.SessionsLoadFailed)).closest(
      '[style*="top"]',
    ) as HTMLElement;
  ...
  // An emptied grid alone cannot be told apart from a period that genuinely held no sessions.
  test('a failed block raises a notification and says so in the empty state', async () => {
    ...
    await waitFor(() => expect(screen.getByText(SessionsTraceI18nKey.SessionsLoadFailed)).toBeInTheDocument());
  });
  ...
  test('a failed later block keeps the rows already loaded', async () => {
    ...
    expect(screen.queryByText(SessionsTraceI18nKey.NoSessions)).not.toBeInTheDocument();
  });
  ...
  test('a successful block clears an earlier failure', async () => {
    ...
    await waitFor(() => expect(screen.queryByText(SessionsTraceI18nKey.SessionsLoadFailed)).not.toBeInTheDocument());
  });
