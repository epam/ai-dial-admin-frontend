...
const row = (overrides: Partial<SessionRow> = {}): SessionRow => ({
  client_session_id: 'sessions/chat-1',
...
const renderCell = (data?: SessionRow | null) =>
  render(<ProjectCellRenderer {...({ data } as ICellRendererParams<SessionRow>)} />);
...
describe('ProjectCellRenderer', () => {
  ...
  // The rollup does not carry `deployment`, so the page has no model to attribute to a session.
  ...
  test('marks an unattributed project rather than rendering an empty cell', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.NoProject)).toBeInTheDocument();
  });

  test('does not show the placeholder when a project is present', () => {
    renderCell(row());

    expect(screen.queryByText(SessionsTraceI18nKey.NoProject)).not.toBeInTheDocument();
  });
