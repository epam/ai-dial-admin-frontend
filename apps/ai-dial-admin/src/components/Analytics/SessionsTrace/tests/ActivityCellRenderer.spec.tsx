...
const renderCell = (data?: SessionRow | null) =>
  render(<ActivityCellRenderer {...({ data, nowMs: NOW } as unknown as ICellRendererParams<SessionRow>)} />);

describe('ActivityCellRenderer', () => {
  test('shows how long ago the session was last active', () => {
    renderCell(row());

    expect(screen.getByText('12m ago')).toBeInTheDocument();
  });

  test('shows how long the session ran beneath it', () => {
    renderCell(row());
