...
const PERIOD: SessionPeriodSummary = {
  totals: { sessions: 212, cost: '654.070540769000' },
...
const renderSummary = ({ period = PERIOD, periodLabel = '7d' }: Options = {}) =>
  render(<SessionsSummary period={period} periodLabel={periodLabel} />);
...
describe('SessionsSummary', () => {
  test('shows the period session count, with no approximation marker', () => {
    renderSummary();
  ...
  test('reads a numeric cost as well as a string one', () => {
    renderSummary({ period: { totals: { sessions: 1, cost: 0.5335 }, ratings: { rated: 0, negative: 0 } } });

    expect(screen.getByText('$0.534')).toBeInTheDocument();
  });

  test('shows how many sessions in the period carry a rating', () => {
    renderSummary();

    expect(pillFor(SessionsTraceI18nKey.SummaryRated)).toHaveTextContent('19');
  });

  // Ratings are bounded by when they were submitted and sessions by when they were last active, so a
  // ratio over the session count would not be a proportion and could read above one.
  ...
  test('shows how many sessions in the period carry a negative rating', () => {
    renderSummary();
  ...
  test('states on every pill that it covers the period', () => {
    ...
    expect(screen.getAllByTitle(SessionsTraceI18nKey.SummaryPeriodHint)).toHaveLength(4);
  });
  ...
  test('carries the period hint as screen-reader-only text alongside the visible caption', () => {
    ...
    expect(srOnlyTexts(container).filter((text) => text === SessionsTraceI18nKey.SummaryPeriodHint)).toHaveLength(4);
  });

  test('reports unavailability to assistive technology, not only in the tooltip', () => {
    const { container } = renderSummary({ period: null });

    expect(srOnlyTexts(container).filter((text) => text === SessionsTraceI18nKey.SummaryUnavailableHint)).toHaveLength(
      4,
    );
  });

  // The rated count no longer depends on the session count, so a failed totals aggregate leaves it
  // standing rather than blanking it.
  test('the rated count stands when only the session count is unresolved', () => {
    renderSummary({ period: { ratings: { rated: 19, negative: 13 } } });

    expect(pillFor(SessionsTraceI18nKey.SummaryRated)).toHaveTextContent('19');
    expect(pillFor(SessionsTraceI18nKey.SummarySessions)).toHaveTextContent('—');
  });

  test('no pill claims to cover only the sessions loaded so far', () => {
    const { container } = renderSummary();
  ...
  test('reports the count and cost as unavailable when their aggregate failed', () => {
    ...
    expect(pillFor(SessionsTraceI18nKey.SummarySessions)).toHaveTextContent('—');
    expect(screen.getAllByTitle(SessionsTraceI18nKey.SummaryUnavailableHint).length).toBeGreaterThan(0);
  });

  test('keeps the count and cost standing when only the rating aggregate failed', () => {
    renderSummary({ period: { totals: { sessions: 212, cost: '10' } } });
  ...
  test('reports every figure as unavailable when the whole summary failed', () => {
    ...
    expect(screen.getAllByTitle(SessionsTraceI18nKey.SummaryUnavailableHint)).toHaveLength(4);
  });
  ...
  test('renders an empty period as zero cost, not as unavailable', () => {
    renderSummary({ period: { totals: { sessions: 0, cost: null }, ratings: { rated: 0, negative: 0 } } });

    expect(screen.getByText('$0')).toBeInTheDocument();
    expect(screen.queryByText('—')).not.toBeInTheDocument();
    expect(screen.queryByTitle(SessionsTraceI18nKey.SummaryUnavailableHint)).not.toBeInTheDocument();
  });
});
