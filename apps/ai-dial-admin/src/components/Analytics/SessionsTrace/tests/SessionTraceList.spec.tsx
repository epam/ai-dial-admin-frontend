...
import { SessionTraceCard, SessionTraceGroup, RatingCounts } from '@/src/models/analytics/sessions-trace';
import { formatCompactNumber, formatSignificantCost } from '@/src/utils/analytics/session-formatting';

const card = (over: Partial<SessionTraceCard> = {}): SessionTraceCard => ({
  traceId: 'trace-1',
  coreSpanId: 'root-1',
...
const group = (over: Partial<SessionTraceGroup> = {}): SessionTraceGroup => ({
  traceId: 'trace-1',
  startedAt: Date.UTC(2026, 7, 27, 10, 9, 18),
  spans: 4,
  tokens: 11615,
...
const renderList = (props: Partial<ComponentProps<typeof SessionTraceList>> = {}) =>
  render(
    <SessionTraceList
      groups={[group()]}
...
describe('SessionTraceList', () => {
  ...
  test('states a card own spend against its chain spend', () => {
    ...
    const own = screen.getByText(SessionsTraceI18nKey.TraceCardOwnPrice).parentElement;
    const chain = screen.getByText(SessionsTraceI18nKey.TraceCardChainPrice).parentElement;
  ...
  test('states a card own duration and message count', () => {
    ...
    expect(screen.getByText(new RegExp(SessionsTraceI18nKey.TraceCardMessages))).toBeInTheDocument();
  });
  ...
  test('keeps a card own status when another hop in its trace failed', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.TraceCardSucceeded)).toBeInTheDocument();
  });
  ...
  test('marks a Core-internal card as such', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.TraceCardCoreInternal)).toBeInTheDocument();
  });

  test('does not mark a card recorded under the session own project', () => {
    renderList();

    expect(screen.queryByText(SessionsTraceI18nKey.TraceCardCoreInternal)).toBeNull();
  });
  ...
  test('reports a trace whose root was not recorded, from its own figures', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.TraceRootNotRecorded)).toBeInTheDocument();
    // Its figures are real even though its root is not.
    expect(screen.getByText(formatCompactNumber(11615))).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: new RegExp(SessionsTraceI18nKey.TraceRootNotRecorded) }),
    ).toBeInTheDocument();
  });
  ...
  test('discloses how many further calls a capped trace records', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.TraceFurtherCalls, { exact: false })).toBeInTheDocument();
  });

  test('discloses nothing when every card of a trace is shown', () => {
    renderList();

    expect(screen.queryByText(SessionsTraceI18nKey.TraceFurtherCalls, { exact: false })).toBeNull();
  });
  ...
  test('states a card ratings where they resolve', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.RatingUpTotal).parentElement).toHaveTextContent('3');
    expect(screen.getByText(SessionsTraceI18nKey.RatingDownTotal).parentElement).toHaveTextContent('1');
  });

  test('states that no traces were recorded, distinctly from a failed read', () => {
    renderList({ groups: [] });

    expect(screen.getByText(SessionsTraceI18nKey.TraceListEmpty)).toBeInTheDocument();
  ...
  test('reports a failed read as a failure rather than as an empty listing', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.TraceListLoadFailed)).toBeInTheDocument();
  ...
  test('reports a failed later page while keeping the traces already loaded', () => {
    ...
    expect(screen.getByText('applications/public/pg-chat-hub__1.0.0')).toBeInTheDocument();
    expect(screen.getByText(SessionsTraceI18nKey.TraceListLoadFailedHint)).toBeInTheDocument();
  });
});
...
describe('SessionTraceList — grouping', () => {
  const twoCardGroup = () =>
    group({
      cards: [
        card({ coreSpanId: 'client', deployment: 'pg-chat-hub' }),
  ...
  test('names each group by its trace, for assistive technology', () => {
    ...
    expect(screen.getByRole('group', { name: SessionsTraceI18nKey.TraceGroupLabel })).toBeInTheDocument();
  });
  ...
  test('states the chips a trace recorded, with their counts', () => {
    ...
    expect(screen.getByText(new RegExp(SessionsTraceI18nKey.TraceChipUnclassified))).toBeInTheDocument();
  });

  test('states that a trace contains failures without changing a card own status', () => {
    renderList({ groups: [group({ failedSpans: 2 })] });

    expect(screen.getByText(SessionsTraceI18nKey.TraceFailuresInside, { exact: false })).toBeInTheDocument();
    expect(screen.getByText(SessionsTraceI18nKey.TraceCardSucceeded)).toBeInTheDocument();
  });
  ...
  test('groups a capped trace so its disclosure sits with the cards it belongs to', () => {
    ...
    expect(traceGroup).toHaveTextContent(SessionsTraceI18nKey.TraceFurtherCalls);
  });
});
...
// it claims to be.
describe('SessionTraceList — tiers', () => {
  test('states the card own facts in the middle tier, each labelled', () => {
    renderList();

    for (const label of [
      SessionsTraceI18nKey.TraceCardStarted,
      SessionsTraceI18nKey.TraceCardMessages,
      SessionsTraceI18nKey.TraceCardOwnTokens,
      SessionsTraceI18nKey.TraceCardOwnPrice,
      SessionsTraceI18nKey.TraceCardChainPrice,
    ]) {
  ...
  test('keeps the card own tokens out of the trace tier', () => {
    ...
    const ownTokens = screen.getByText(SessionsTraceI18nKey.TraceCardOwnTokens).parentElement;
  ...
  test('states the trace span count in the bottom tier', () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.TraceCardSpans, { exact: false })).toBeInTheDocument();
  });

  test('names the rule the listing follows, without calling a card a turn', () => {
    renderList();

    expect(screen.getByRole('heading', { name: SessionsTraceI18nKey.TraceSectionTitle })).toBeInTheDocument();
    expect(screen.getByText(SessionsTraceI18nKey.TraceSectionRule)).toBeInTheDocument();
    expect(screen.queryByText(/Turn \d/)).toBeNull();
  });
  ...
  test('carries the trace tier once, whether collapsed or grouped', () => {
    ...
    expect(screen.getAllByText(SessionsTraceI18nKey.TraceCardSpans, { exact: false })).toHaveLength(1);
    unmount();
    ...
    expect(screen.getAllByText(SessionsTraceI18nKey.TraceCardSpans, { exact: false })).toHaveLength(1);
    expect(screen.getAllByText(SessionsTraceI18nKey.TraceCardStarted)).toHaveLength(2);
  });

  // Ratings are a trace fact, so a Core service call inside the same trace must not display the reader's
  // rating of the session's own answer.
  test('attaches the trace ratings only to the card carrying the session label', () => {
    renderList({
      groups: [
        group({
          cards: [
            card({ coreSpanId: 'client', hasSessionLabel: true }),
            card({ coreSpanId: 'service', hasSessionLabel: false, isCoreInternal: true }),
          ],
        }),
      ],
    ...
    expect(screen.getAllByText(SessionsTraceI18nKey.RatingUpTotal)).toHaveLength(1);
  });
  ...
  test('opens a trace whose root was not recorded, rather than rendering it inert', async () => {
    ...
    await user.click(screen.getByRole('button', { name: new RegExp(SessionsTraceI18nKey.TraceRootNotRecorded) }));
  ...
  test('offers a manual way to load the next page when more remain', async () => {
    ...
    await user.click(screen.getByRole('button', { name: SessionsTraceI18nKey.TraceListLoadMore }));
...
describe('SessionTraceList — accessible naming', () => {
  test('names a card by everything it states, not by its title alone', () => {
    ...
    expect(name).toContain(SessionsTraceI18nKey.TraceCardSucceeded);
    expect(name).toContain(SessionsTraceI18nKey.TraceCardCoreInternal);
    expect(name).toContain(SessionsTraceI18nKey.TraceCardOwnPrice);
  });
});
