...
import { SpanFieldGroup, SpanFieldRow, SpanFieldTag } from '@/src/models/analytics/sessions-trace';
...
describe('SpanFieldsSection', () => {
  test('names each group by the label this release has for its tag', () => {
    ...
    expect(headerFor(SessionsTraceI18nKey.SpanTagIdentifier)).toBeInTheDocument();
    expect(headerFor(SessionsTraceI18nKey.SpanTagClient)).toBeInTheDocument();
  });
  ...
  test('reveals every field the group declares on opening it, absent ones included', async () => {
    ...
    await user.click(headerFor(SessionsTraceI18nKey.SpanTagIdentifier));
  ...
  test('previews the first recorded value of a collapsed group', () => {
    ...
    expect(headerFor(SessionsTraceI18nKey.SpanTagClient)).toHaveAccessibleName(/chat/);
  });
  ...
  test('drops the preview once the group is open, where the value is the first row', async () => {
    ...
    await user.click(headerFor(SessionsTraceI18nKey.SpanTagClient));

    expect(headerFor(SessionsTraceI18nKey.SpanTagClient)).toHaveAccessibleName(SessionsTraceI18nKey.SpanTagClient);
  ...
  test('states no field count in the header', () => {
    ...
    expect(headerFor(SessionsTraceI18nKey.SpanTagIdentifier)).not.toHaveAccessibleName(/\d+ *\/ *\d+/);
  });
  ...
  test('previews a figure group by the column named for it, not by its first field', () => {
    ...
    expect(headerFor(SessionsTraceI18nKey.SpanTagTokenUsage)).toHaveAccessibleName(/7,512/);
  });
  ...
  test('keeps one group open at a time', async () => {
    const user = userEvent.setup();
    renderSection();
    const identity = headerFor(SessionsTraceI18nKey.SpanTagIdentifier);
    const client = headerFor(SessionsTraceI18nKey.SpanTagClient);
