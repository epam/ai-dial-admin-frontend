...
const VALUES: SessionFieldValue[] = [
...
const MANY_VALUES: SessionFieldValue[] = Array.from({ length: 12 }, (_, i) => ({
...
const renderFilter = (model: SessionValueFilterModel | null = null) =>
  render(
    withGrid(
      <SessionValueFilter
        {...({
...
const optionsGroup = () => screen.getByRole('group', { name: SessionsTraceI18nKey.ValueFilterGroup });
...
const selectAllCheckbox = () => screen.getByRole('checkbox', { name: SessionsTraceI18nKey.ValueFilterSelectAll });
...
describe('SessionValueFilter', () => {
  ...
  test('the loading state is announced and offers nothing to select', async () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent(SessionsTraceI18nKey.ValueFilterLoading);
  ...
  test('an empty result says so and offers nothing to select', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('status')).toHaveTextContent(SessionsTraceI18nKey.ValueFilterEmpty));
    expect(screen.queryAllByRole('checkbox')).toHaveLength(0);
    expect(onModelChange).not.toHaveBeenCalled();
  });
  ...
  test('a failed read says so, in the error treatment, and offers no text entry', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole('status')).toHaveTextContent(SessionsTraceI18nKey.ValueFilterLoadFailed),
    );
  ...
  test('a failed read states the service message in place rather than the fixed string', async () => {
    ...
    expect(screen.getByRole('status')).not.toHaveTextContent(SessionsTraceI18nKey.ValueFilterEmpty);
  });
