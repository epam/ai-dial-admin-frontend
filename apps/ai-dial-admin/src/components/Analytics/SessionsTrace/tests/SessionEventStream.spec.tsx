...
import SessionEventStream from '@/src/components/Analytics/SessionsTrace/Detail/SessionEventStream';
import { SessionsTraceI18nKey } from '@/src/constants/i18n';
import { SessionSpanRow, HopTreeNode } from '@/src/models/analytics/sessions-trace';
import { buildHopTree } from '@/src/utils/analytics/session-span-tree';
...
const renderStream = (tree: HopTreeNode[] = TREE, onSelectSpan = vi.fn()) =>
  render(<SessionEventStream tree={tree} selectedSpanId={null} onSelectSpan={onSelectSpan} />);

const filters = () => within(screen.getByRole('group', { name: SessionsTraceI18nKey.StreamFilterLabel }));
const filterFor = (key: string) => filters().getByRole('button', { name: new RegExp(key) });
const rows = () => within(screen.getByRole('group', { name: SessionsTraceI18nKey.StreamLabel }));
const rowFor = (text: RegExp) => rows().getByRole('button', { name: text });
const expanders = () => rows().queryAllByRole('button', { name: SessionsTraceI18nKey.TreeCollapse });
...
describe('SessionEventStream', () => {
  ...
  test('states each row kind in words', () => {
    ...
    expect(rowFor(/a-model/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanLlm));
    expect(rowFor(/a-toolset/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanMcp));
    expect(rowFor(/an-embedding-model/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanEmbeddings));
  });
  ...
  test('collapsing a row hides its descendants and states the collapsed state', async () => {
    ...
    expect(rows().getByRole('button', { name: SessionsTraceI18nKey.TreeExpand }).getAttribute('aria-expanded')).toBe(
      'false',
    );
  });
  ...
  test('marks the selected row as current', () => {
    render(<SessionEventStream tree={TREE} selectedSpanId="embed" onSelectSpan={vi.fn()} />);
...
describe('SessionEventStream — figures', () => {
  test('states tokens and request messages for a hop that metered its own', () => {
    renderStream();

    expect(rowFor(/a-model/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanTokens));
    expect(rowFor(/a-model/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanRequestMessages));
  });
  ...
  test('states the chain cost for a hop that metered nothing of its own, and no token count', () => {
    ...
    expect(rowFor(/an-application/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanChainCost));
    expect(rowFor(/an-application/)).not.toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanTokens));
  });
  ...
  // Recorded hop durations start at single-digit milliseconds, which session-scale formatting rendered
  ...
  test('states no figures for a hop that metered nothing and spent nothing', () => {
    ...
    expect(rowFor(/a-model/)).not.toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanTokens));
    expect(rowFor(/a-model/)).not.toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanChainCost));
  });
...
describe('SessionEventStream — the kind axis', () => {
  test('offers a control for each kind the turn recorded and no other', () => {
    renderStream();

    expect(filterFor(SessionsTraceI18nKey.SpanLlm)).toBeInTheDocument();
    expect(filterFor(SessionsTraceI18nKey.SpanMcp)).toBeInTheDocument();
    expect(filterFor(SessionsTraceI18nKey.SpanEmbeddings)).toBeInTheDocument();
    expect(filters().queryByRole('button', { name: new RegExp(SessionsTraceI18nKey.SpanRoute) })).toBeNull();
    expect(filters().queryByRole('button', { name: new RegExp(SessionsTraceI18nKey.SpanRating) })).toBeNull();
  });

  test('starts with no kind emphasised and no match count', () => {
    renderStream();

    expect(filterFor(SessionsTraceI18nKey.StreamTabAll)).toHaveAttribute('aria-pressed', 'true');
  ...
  test('emphasising a kind marks its rows, keeps every row, and announces the count', async () => {
    ...
    await user.click(filterFor(SessionsTraceI18nKey.SpanMcp));

    expect(filterFor(SessionsTraceI18nKey.SpanMcp)).toHaveAttribute('aria-pressed', 'true');
    expect(rows().getAllByText(SessionsTraceI18nKey.StreamMatch)).toHaveLength(1);
    expect(rows().getByRole('button', { name: /a-model/ })).toBeInTheDocument();
    expect(screen.getByRole('status').textContent).toBe(SessionsTraceI18nKey.StreamMatchCount);
  });
  ...
  test('the separate control returns to no emphasis', async () => {
    ...
    await user.click(filterFor(SessionsTraceI18nKey.SpanMcp));
    await user.click(filterFor(SessionsTraceI18nKey.StreamTabAll));

    expect(filterFor(SessionsTraceI18nKey.StreamTabAll)).toHaveAttribute('aria-pressed', 'true');
    expect(rows().queryAllByText(SessionsTraceI18nKey.StreamMatch)).toHaveLength(0);
    expect(screen.getByRole('status').textContent).toBe('');
  });
  ...
  test('offers a Route control for a turn that recorded a route hop', () => {
    ...
    expect(filterFor(SessionsTraceI18nKey.SpanRoute)).toBeInTheDocument();
  });
  ...
  test('offers a Rating control for a rating trace', () => {
    ...
    expect(filterFor(SessionsTraceI18nKey.SpanRating)).toBeInTheDocument();
  });
});

describe('SessionEventStream — the outcome axis', () => {
  ...
  test('offers a Failed control when the turn recorded a failure', () => {
    ...
    expect(filterFor(SessionsTraceI18nKey.EventFailed)).toBeInTheDocument();
  });
  ...
  test('offers none when nothing failed', () => {
    ...
    expect(filters().queryByRole('button', { name: new RegExp(SessionsTraceI18nKey.EventFailed) })).toBeNull();
  });
  ...
  test('a failed row keeps its kind and carries the failure marker', () => {
    ...
    expect(rowFor(/a-model/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanLlm));
    expect(rowFor(/a-model/)).toHaveAccessibleName(new RegExp(SessionsTraceI18nKey.SpanFailedMarker));
  });

  test('the failure marker does not depend on emphasis', () => {
    renderStream(FAILED_TURN);

    expect(rows().getByText(SessionsTraceI18nKey.SpanFailedMarker)).toBeInTheDocument();
  });
  ...
  test('emphasising Failed marks the failed row whatever its kind', async () => {
    ...
    await user.click(filterFor(SessionsTraceI18nKey.EventFailed));

    expect(rows().getAllByText(SessionsTraceI18nKey.StreamMatch)).toHaveLength(1);
  });
});

describe('SessionEventStream — an unrecorded root', () => {
  ...
  test('renders the placeholder and states that it was not recorded', () => {
    ...
    expect(rows().getByText(SessionsTraceI18nKey.TraceRootNotRecorded)).toBeInTheDocument();
  });
