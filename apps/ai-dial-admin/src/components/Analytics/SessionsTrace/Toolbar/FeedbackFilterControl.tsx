...
const FeedbackFilterControl: FC<Props> = ({ feedback, onFeedbackChange }) => {
  ...
  const options = useMemo<SegmentedControlOption<FeedbackFilter>[]>(
    () => [
      { value: FeedbackFilter.All, label: t(SessionsTraceI18nKey.FeedbackAll) },
      {
        value: FeedbackFilter.Positive,
        icon: <IconThumbUp {...BASE_BUTTON_ICON_PROPS} />,
        label: <span className="sr-only">{t(SessionsTraceI18nKey.FeedbackPositive)}</span>,
      },
      {
        value: FeedbackFilter.Negative,
        icon: <IconThumbDown {...BASE_BUTTON_ICON_PROPS} />,
        label: <span className="sr-only">{t(SessionsTraceI18nKey.FeedbackNegative)}</span>,
      },
      { value: FeedbackFilter.Rated, label: t(SessionsTraceI18nKey.FeedbackRated) },
    ],
    [t],
  );
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="dial-small-text text-secondary whitespace-nowrap">{t(SessionsTraceI18nKey.Feedback)}</span>
      <DialSegmentedControl
        options={options}
        value={feedback}
        onChange={onFeedbackChange}
        ariaLabel={t(SessionsTraceI18nKey.Feedback)}
      />
    </div>
  );
};
