...
import { getSessionFieldValues, getSessions } from '@/src/app/[lang]/sessions/actions';
import { SESSIONS_SEARCH_DEBOUNCE_MS, SESSIONS_TIME_PERIOD } from '@/src/constants/analytics/sessions-trace';
...
const filterKey = ({ search, startMs, endMs, feedback }: SessionFilters): string =>
...
interface ResolvedArrayFilters {
  key: string;
  filters: SessionArrayFilter[];
}

const arrayFilterKey = (filters: SessionFilters, columnFilters: SessionColumnFilter[]): string =>
  JSON.stringify([filters.startMs, filters.endMs, columnFilters]);

export const useSessions = (schemaFields?: AnalyticsEntityField[] | null) => {
  const t = useI18n();
  const { showNotification } = useNotification();
  const getReqRef = useRef(useProtectedRequest());

  const [gridApi, setGridApi] = useState<GridApi<SessionRow> | null>(null);

  const [hasLoadError, setHasLoadError] = useState(false);
  const [period, setPeriod] = useState<SessionPeriodSummary | null>(null);
  ...
  const { timePeriod, timeRange, isCustom, onTimePeriodChange, onTimeRangeChange } = useTimeFilter({
    defaultTimeFilter: SESSIONS_TIME_PERIOD,
  });
  ...
  const modelScope: SessionModelScope = useMemo(() => {
  ...
  const reportFailure = useCallback(() => {
    showNotification(getErrorNotification(t(SessionsTraceI18nKey.SessionsLoadFailed)));
  ...
  const datasource: IDatasource = useMemo(
    () => ({
      getRows: async (params: IGetRowsParams) => {
        ...
        const sort = translateSessionSortModel(params.sortModel, modelScope);
        const columnFilters = translateSessionFilterModel(params.filterModel as SessionGridFilterModel, modelScope);
        ...
        try {
          const result = await getReqRef.current(getSessions, {
            ...filters,
            columnFilters,
            sort,
            sourceFields,
          ...
          const page = result.response as SessionsPage | undefined;
  ...
  const requestFieldValues = useCallback(
    async (fieldName: string): Promise<ServerActionResponse<SessionFieldValue[]>> => {
      const columnFilters = translateSessionFilterModel(
        gridApi?.getFilterModel() as SessionGridFilterModel,
        modelScope,
      );
      const chatIds = candidateRef.current?.key === key ? candidateRef.current.ids : undefined;
      const arrayKey = arrayFilterKey(filters, columnFilters);
      ...
      const result = await getSessionFieldValues({
        ...filters,
        field: fieldName,
        columnFilters,
        ...(chatIds ? { chatIds } : {}),
  ...
  const gridContext: SessionGridContext = useMemo(() => ({ requestFieldValues }), [requestFieldValues]);
  ...
  const applySearch = useMemo(() => debounce(setAppliedSearch, SESSIONS_SEARCH_DEBOUNCE_MS), []);
  ...
  useEffect(() => {
    ...
    const onColumnVisible = (event: ColumnVisibleEvent) => {
      ...
      if (!event.visible) {
        ...
        // it — and on an enrichment field that narrowing is severe, since only the sessions the
