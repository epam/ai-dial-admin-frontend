...
import HopMessageRow from '@/src/components/Analytics/SessionsTrace/Detail/Inspector/HopMessageRow';
...
import { HopInspectorSide, HopRequestEnvelope, MessageRole, SessionScope } from '@/src/models/analytics/sessions-trace';
...
const HopRequestPanel: FC<Props> = ({ envelope, isRaw, scope, traceId, coreSpanId, requestTime }) => {
  ...
  if (!envelope.messages.length) {
    return <HopStateNote messageKey={SessionsTraceI18nKey.InspectorNoMessages} />;
  }
    ...
    <div className="flex min-w-0 flex-col">
      ...
      <div className="sticky top-0 z-10 flex flex-wrap items-center gap-x-3 gap-y-1 bg-layer-1 pb-3">
        ...
        {!isRaw && (
          <div
            role="group"
            aria-label={t(SessionsTraceI18nKey.InspectorRolesLabel)}
            className="flex min-w-0 flex-wrap items-center gap-1"
          >
            <GhostButton
              size={ElementSize.Small}
              aria-pressed={role === null}
              label={`${t(SessionsTraceI18nKey.InspectorRoleAll)} ${envelope.messages.length}`}
            ...
            <span role="status" aria-live="polite" className="sr-only">
              {role === null
                ? ''
                : t(SessionsTraceI18nKey.InspectorRoleMatches, {
                    count: shown.length,
