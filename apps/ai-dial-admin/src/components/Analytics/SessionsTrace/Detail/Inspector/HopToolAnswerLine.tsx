...
import { HOP_FAILED_CHIP_CLASS } from '@/src/constants/analytics/sessions-trace';
import { SessionsTraceI18nKey } from '@/src/constants/i18n';
import { useI18n } from '@/src/locales/client';
import { HopToolAnswer } from '@/src/models/analytics/sessions-trace';
import { shortCallId } from '@/src/utils/analytics/session-formatting';
...
const HopToolAnswerLine: FC<Props> = ({ answers, isError }) => {
  ...
  return (
    <span className="flex min-w-0 flex-wrap items-center gap-1.5 font-mono text-secondary dial-caption-text">
      {answers.length > 0 && (
        <>
          <span>{t(SessionsTraceI18nKey.InspectorToolAnswers)}</span>
      ...
      {isError && (
        <span className={classNames('rounded border px-1.5 py-0.5', HOP_FAILED_CHIP_CLASS)}>
          {t(SessionsTraceI18nKey.InspectorToolFailed)}
        </span>
      )}
    </span>
  );
