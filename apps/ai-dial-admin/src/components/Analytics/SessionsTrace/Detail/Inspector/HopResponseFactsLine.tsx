...
import { HopResponseFacts } from '@/src/models/analytics/sessions-trace';
...
const HopResponseFactsLine: FC<Props> = ({ facts, finishReason, requestedModel }) => {
  ...
  const stated: { label: string; value: string; isAccented?: boolean }[] = [
    ...(facts.model === null || facts.model === requestedModel
      ? []
      : [{ label: t(SessionsTraceI18nKey.InspectorUpstreamModel), value: facts.model }]),
    ...(finishReason === null ? [] : [{ label: t(SessionsTraceI18nKey.InspectorFinishReason), value: finishReason }]),
    ...(facts.promptTokens === null
      ? []
      : [
          {
            label: t(SessionsTraceI18nKey.InspectorPromptTokens),
    ...
    ...(facts.completionTokens === null
      ...
      : [
          {
            label: t(SessionsTraceI18nKey.InspectorCompletionTokens),
    ...
    ...(facts.cachedTokens === null
      ...
      : [
          {
            label: t(SessionsTraceI18nKey.InspectorCachedTokens),
  ...
  return (
    <dl
      role="group"
      aria-label={t(SessionsTraceI18nKey.InspectorResponseFactsLabel)}
