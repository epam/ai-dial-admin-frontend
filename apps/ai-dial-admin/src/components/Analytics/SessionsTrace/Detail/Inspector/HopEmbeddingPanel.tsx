...
import { HopEmbeddingFacts } from '@/src/models/analytics/sessions-trace';
...
const HopEmbeddingPanel: FC<Props> = ({ facts, isLoading, tokens }) => {
  ...
  const cells = [
    { label: t(SessionsTraceI18nKey.InspectorEmbeddingModel), value: facts.model ?? UNAVAILABLE_VALUE },
    {
      label: t(SessionsTraceI18nKey.InspectorEmbeddingInputs),
      value: facts.inputCount === null ? UNAVAILABLE_VALUE : String(facts.inputCount),
    },
    { label: t(SessionsTraceI18nKey.TraceTokens), value: formatCompactNumber(tokens) || UNAVAILABLE_VALUE },
  ];
    ...
    <div className="flex min-w-0 flex-col gap-3">
      ...
      {facts.inputText === null ? (
        ...
        <div className="flex min-w-0 flex-col gap-1">
          <span className="text-secondary dial-tiny-text">{t(SessionsTraceI18nKey.InspectorEmbeddingText)}</span>
