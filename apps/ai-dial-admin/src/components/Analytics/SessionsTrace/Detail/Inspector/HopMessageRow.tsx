...
import HopMessageCard from '@/src/components/Analytics/SessionsTrace/Detail/Inspector/HopMessageCard';
import HopToolAnswerLine from '@/src/components/Analytics/SessionsTrace/Detail/Inspector/HopToolAnswerLine';
import HopToolCalls from '@/src/components/Analytics/SessionsTrace/Detail/Inspector/HopToolCalls';
import { SessionsTraceI18nKey } from '@/src/constants/i18n';
import { useI18n } from '@/src/locales/client';
import { HopMessageEntry, HopMessageValue } from '@/src/models/analytics/sessions-trace';
  ...
  return (
      ...
      footer={
        (message.isTextClamped || isOpen) && (
          <LinkButton
            ...
            disabled={isLoading}
            label={t(
              isOpen ? SessionsTraceI18nKey.InspectorHideFullMessage : SessionsTraceI18nKey.InspectorShowFullMessage,
            )}
    ...
    >
      ...
      {!hasText && !toolCalls.length && (
        <p className="text-secondary dial-tiny-text">{t(SessionsTraceI18nKey.InspectorNoText)}</p>
      )}
    </HopMessageCard>
  );
};
