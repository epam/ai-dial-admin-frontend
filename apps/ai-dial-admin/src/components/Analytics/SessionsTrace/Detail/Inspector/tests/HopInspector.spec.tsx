...
import HopInspector from '@/src/components/Analytics/SessionsTrace/Detail/Inspector/HopInspector';
...
const getSessionHopProtocol = vi.fn();
...
vi.mock('@/src/app/[lang]/sessions/actions', () => ({
  ...
  getSessionHopProtocol: (...args: unknown[]) => getSessionHopProtocol(...args),
}));
...
const span = (overrides: Partial<SessionSpanRow> = {}): SessionSpanRow => ({
  core_span_id: 's1',
...
beforeEach(() => {
  vi.clearAllMocks();
  getSessionHopRequest.mockResolvedValue({ success: true, response: envelope() });
  getSessionHopResponse.mockResolvedValue({ success: true, response: response() });
});
...
describe('HopInspector — the request side', () => {
  ...
  test('states the recorded message count in the parameter line, not on the tab', async () => {
    ...
    const params = await screen.findByLabelText(SessionsTraceI18nKey.InspectorParamsLabel);
  ...
  test('states the system message under its own role', async () => {
    ...
    expect(screen.getByRole('group', { name: `${SessionsTraceI18nKey.InspectorRoleSystem} #1` })).toBeInTheDocument();
  });
  ...
  test('offers a role control per role present, and none for a role absent', async () => {
    ...
    const roles = await screen.findByRole('group', { name: SessionsTraceI18nKey.InspectorRolesLabel });
  ...
  test('narrowing to one role states how many of how many messages match', async () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent(SessionsTraceI18nKey.InspectorRoleMatches);
  });
  ...
  test('states a zero temperature and an absent tool catalogue', async () => {
    ...
    expect(params).toHaveTextContent(SessionsTraceI18nKey.InspectorParamTools);
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorParamsRest)).toBeInTheDocument();
  });
  ...
  test('opens one message in full on demand rather than shipping it with the list', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: SessionsTraceI18nKey.InspectorShowFullMessage }));
  ...
  test('renders an assistant tool call as the message content', async () => {
    ...
    expect(screen.queryByText(SessionsTraceI18nKey.InspectorNoText)).toBeNull();
  });
  ...
  test('states a message that recorded neither text nor a call', async () => {
    ...
    expect(await screen.findByText(SessionsTraceI18nKey.InspectorNoText)).toBeInTheDocument();
  });
  ...
  test('states that the envelope clamped rather than dropping messages silently', async () => {
    getSessionHopRequest.mockResolvedValue({ success: true, response: envelope({ isClamped: true }) });
    renderInspector();

    await waitFor(() => expect(screen.getByText(SessionsTraceI18nKey.InspectorEnvelopeClamped)).toBeInTheDocument());
  });
  ...
  test('falls back to the raw view for a dialect no parser claims', async () => {
    ...
    getSessionHopRawBody.mockResolvedValue({
      success: true,
      response: {
        state: HopReadState.Available,
        text: '{"raw":true}',
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorUnstructured)).toBeInTheDocument();
  });
});
...
describe('HopInspector — the recorded bytes', () => {
  ...
  test('defers the response read until a tab that shows it is open', async () => {
    ...
    await screen.findByText('who are you?');
    expect(getSessionHopResponse).not.toHaveBeenCalled();

    await user.click(screen.getByRole('tab', { name: /InspectorChat/ }));

    await waitFor(() => expect(getSessionHopResponse).toHaveBeenCalledOnce());
  });

  test('offers the raw switch on the request side too', async () => {
    renderInspector();

    expect(await screen.findByRole('button', { name: SessionsTraceI18nKey.InspectorRaw })).toBeInTheDocument();
  });
  ...
  test('reads the request body raw only once the switch is on', async () => {
    ...
    await screen.findByText('who are you?');
    expect(getSessionHopRawBody).not.toHaveBeenCalled();

    await user.click(screen.getByRole('button', { name: SessionsTraceI18nKey.InspectorRaw }));
  ...
  test('the raw switch closes the facts line, at its end', async () => {
    ...
    const params = await screen.findByLabelText(SessionsTraceI18nKey.InspectorParamsLabel);
    const raw = screen.getByRole('button', { name: SessionsTraceI18nKey.InspectorRaw });
  ...
  test('drops the role filter while the recorded bytes are shown', async () => {
    ...
    expect(screen.queryByRole('group', { name: SessionsTraceI18nKey.InspectorRolesLabel })).toBeNull();
  });
});
...
describe('HopInspector — the response side', () => {
  ...
  test('states why a requested tool has no recorded result', async () => {
    const user = userEvent.setup();
    getSessionHopResponse.mockResolvedValue({
      success: true,
      response: response({
    ...
    expect(await screen.findByText(SessionsTraceI18nKey.InspectorToolNotRecorded)).toBeInTheDocument();
  ...
  test('makes no such claim when the turn did record a call of that tool', async () => {
    ...
    expect(screen.queryByText(SessionsTraceI18nKey.InspectorToolNotRecorded)).toBeNull();
  });
  ...
  test('reads the raw body only when the raw mode is selected', async () => {
    ...
    await waitFor(() => expect(getSessionHopRawBody).toHaveBeenCalled());
  });
  ...
  test('states what a clamped raw body withheld', async () => {
    ...
    await waitFor(() => expect(screen.getByText(SessionsTraceI18nKey.InspectorRawClamped)).toBeInTheDocument());
  });
});
...
describe('HopInspector — the transport line', () => {
  ...
  test('a hop whose body columns are withheld still states how the call went', async () => {
    ...
    const transport = await screen.findByRole('group', { name: SessionsTraceI18nKey.InspectorTransportLabel });
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorWithheldStats)).toBeInTheDocument();
  });
  ...
  test('a failed hop states the status it failed with', async () => {
    ...
    expect(screen.getByRole('group', { name: SessionsTraceI18nKey.InspectorTransportLabel })).toHaveTextContent(
      '502 Bad Gateway',
    );
  });
  ...
  test('the response side states the outcome, not the verb', async () => {
    ...
    await user.click(await screen.findByRole('tab', { name: /InspectorResponse/ }));
    const transport = screen.getByRole('group', { name: SessionsTraceI18nKey.InspectorTransportLabel });

    expect(transport).toHaveTextContent('200 OK');
    expect(transport).not.toHaveTextContent('POST');
  });

  // Chat states a session, not a call, so neither half of the transport belongs to it.
  test('the chat tab states no transport facts', async () => {
    ...
    expect(screen.queryByRole('group', { name: SessionsTraceI18nKey.InspectorTransportLabel })).toBeNull();
  });
});
...
describe('HopInspector — every clamp states itself', () => {
  test('the assembled response states its clamp', async () => {
    ...
    expect(await screen.findByText(SessionsTraceI18nKey.InspectorRawClamped)).toBeInTheDocument();
  });
  ...
  test('an MCP result states its clamp', async () => {
    const user = userEvent.setup();
    getSessionHopMcp.mockResolvedValue({
      success: true,
      response: {
        state: HopReadState.Available,
        method: 'tools/call',
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorRawClamped)).toBeInTheDocument();
  });
  ...
  test('an unclamped response states nothing', async () => {
    ...
    expect(screen.queryByText(SessionsTraceI18nKey.InspectorRawClamped)).toBeNull();
  });
});
...
describe('HopInspector — the recorded bytes keep the facts about the hop', () => {
  ...
  test('the response facts stay stated with the raw switch on', async () => {
    ...
    expect(screen.getByRole('group', { name: SessionsTraceI18nKey.InspectorResponseFactsLabel })).toBeInTheDocument();
  });
});
...
describe('HopInspector — absence and entitlement', () => {
  ...
  test('and states the absence on the response side alone', async () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorNoResponse)).toBeInTheDocument();
    expect(getSessionHopResponse).not.toHaveBeenCalled();
  });

  test('a notification states that the protocol defines no response body', async () => {
    const user = userEvent.setup();
    getSessionHopProtocol.mockResolvedValue(protocolFacts({ requestText: null, names: [], stated: [] }));
    renderInspector({
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorProtocolNoBody)).toBeInTheDocument();
    expect(screen.queryByText(SessionsTraceI18nKey.InspectorNoResponse)).toBeNull();
  });
  ...
  test('a caller entitled only to the response side gets the response read without choosing it', async () => {
    ...
    expect(getSessionHopResponse).toHaveBeenCalledOnce();
  });

  test('a failed read is stated as a failure, not as an empty hop', async () => {
    getSessionHopRequest.mockResolvedValue({ success: false, response: undefined });
    renderInspector();

    await waitFor(() => expect(screen.getByText(SessionsTraceI18nKey.InspectorLoadFailed)).toBeInTheDocument());
  });
  ...
  test('a failed read is reported by notification as well, quoting the service', async () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorLoadFailed)).toBeInTheDocument();
  });

  test('a failed read falls back to its own title when the service supplied no header', async () => {
    getSessionHopRequest.mockResolvedValue({ success: false, status: 500, response: undefined });
    renderInspector();

    await waitFor(() =>
      expect(showNotification).toHaveBeenCalledWith(
        expect.objectContaining({ title: SessionsTraceI18nKey.InspectorLoadFailed }),
      ),
    );
  });
  ...
  test('a read that succeeds raises no notification', async () => {
    ...
    await waitFor(() => expect(getSessionHopRequest).toHaveBeenCalled());
  ...
  test('a failed response read is stated as a failure on the response tab', async () => {
    getSessionHopResponse.mockResolvedValue({ success: false, response: undefined });
    const user = userEvent.setup();
    renderInspector();

    await user.click(await screen.findByRole('tab', { name: /InspectorResponse/ }));

    expect(await screen.findByText(SessionsTraceI18nKey.InspectorLoadFailed)).toBeInTheDocument();
  });
  ...
  test('a granted response the log no longer holds states its absence, and states no facts about it', async () => {
    ...
    expect(await screen.findByText(SessionsTraceI18nKey.InspectorNoBody)).toBeInTheDocument();
    expect(screen.queryByRole('group', { name: SessionsTraceI18nKey.InspectorResponseFactsLabel })).toBeNull();
  });
});
...
describe('HopInspector — MCP and embedding hops', () => {
  ...
  test('an MCP hop states its arguments on the request tab and its result on the response tab', async () => {
    const user = userEvent.setup();
    getSessionHopMcp.mockResolvedValue(mcpFacts());
  ...
  test('the tab strip heads the section, with the hop-row facts below it', async () => {
    ...
    const facts = await screen.findByRole('group', { name: SessionsTraceI18nKey.InspectorMcpFactsLabel });
  ...
  test('an MCP hop keeps its method, tool and toolset visible on both tabs', async () => {
    ...
    expect(screen.getByRole('group', { name: SessionsTraceI18nKey.InspectorMcpFactsLabel })).toHaveTextContent(
      'docs-mcp',
    );
  });
  ...
  test('an MCP hop offers no chat tab, because a protocol message is not a session', async () => {
    getSessionHopMcp.mockResolvedValue(mcpFacts());
    renderInspector({ span: mcpSpan, kind: SpanKind.Mcp });
  ...
  test('an MCP result the caller may not read is stated as withheld, not as empty', async () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorWithheldStats)).toBeInTheDocument();
    expect(screen.queryByText(SessionsTraceI18nKey.InspectorNoBody)).toBeNull();
  });
  ...
  test('a protocol hop states the result the server answered with', async () => {
    const user = userEvent.setup();
    getSessionHopProtocol.mockResolvedValue(protocolFacts());
  ...
  test('an initialize hop states what the server answered with, not a summary of it', async () => {
    ...
    getSessionHopProtocol.mockResolvedValue(protocolFacts({ method: 'initialize', resultText: negotiated }));
  ...
  test('a protocol request carrying no parameters says so', async () => {
    ...
    expect(await screen.findByText(SessionsTraceI18nKey.InspectorProtocolNoParams)).toBeInTheDocument();
    expect(screen.queryByText(SessionsTraceI18nKey.InspectorNoBody)).toBeNull();
  });

  test('an embedding hop states its probe text on the request tab', async () => {
    getSessionHopEmbedding.mockResolvedValue(embeddingFacts());
  ...
  test('an embedding hop states its dimension count on the response tab, and never the vector', async () => {
    ...
    expect(screen.getByText(SessionsTraceI18nKey.InspectorVector)).toBeInTheDocument();
  });

  test('an embedding dimension count the caller may not read is stated as withheld', async () => {
    const user = userEvent.setup();
    getSessionHopEmbedding.mockResolvedValue(embeddingFacts({ dimensions: null, isDimensionsWithheld: true }));
    renderInspector({ span: embeddingSpan, kind: SpanKind.Embeddings });

    await user.click(await screen.findByRole('tab', { name: /InspectorResponse/ }));

    expect(screen.getByText(SessionsTraceI18nKey.InspectorWithheldValue)).toBeInTheDocument();
  });
...
describe('HopInspector — the tab set', () => {
  ...
  test('keeps the remaining tabs in order when the response column is withheld', async () => {
    ...
    expect(tabNames()).toEqual([SessionsTraceI18nKey.InspectorRequest, SessionsTraceI18nKey.InspectorChat]);
  });
