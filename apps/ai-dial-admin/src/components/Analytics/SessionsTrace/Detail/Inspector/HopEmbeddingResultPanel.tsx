...
import { HopEmbeddingFacts, HopReadState, HopSideSuppression } from '@/src/models/analytics/sessions-trace';
...
const dimensionsValue = (facts: HopEmbeddingFacts, t: (key: string) => string): string => {
  ...
  if (facts.isDimensionsWithheld) {
    return t(SessionsTraceI18nKey.InspectorWithheldValue);
  }
    ...
    <>
      <dl className="grid grid-cols-2 gap-2 rounded border border-primary bg-layer-3 p-3">
        <div className="flex min-w-0 flex-col gap-0.5">
          <dt className="text-secondary dial-tiny-text">{t(SessionsTraceI18nKey.InspectorEmbeddingDimensions)}</dt>
