...
import { HopMcpFacts } from '@/src/models/analytics/sessions-trace';
 ...
 *
 * The toolset is the hop's deployment: one measured session recorded all of its MCP hops under a single
...
const HopMcpFactsLine: FC<Props> = ({ facts }) => {
  ...
  const rows = [
    { label: t(SessionsTraceI18nKey.InspectorMcpMethod), value: facts?.method ?? null },
    { label: t(SessionsTraceI18nKey.InspectorMcpTool), value: facts?.toolName ?? null },
    { label: t(SessionsTraceI18nKey.InspectorMcpToolset), value: facts?.toolset ?? null },
  ...
  return (
    <dl
      role="group"
      aria-label={t(SessionsTraceI18nKey.InspectorMcpFactsLabel)}
