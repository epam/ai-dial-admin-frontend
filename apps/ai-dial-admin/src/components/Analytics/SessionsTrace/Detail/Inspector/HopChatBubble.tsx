...
import { MESSAGE_ROLE_LABEL_KEY } from '@/src/constants/analytics/sessions-trace';
...
interface Props {
  ...
  // session announces identically — the same reason the request history's rows carry theirs. The span's
  // own answer has no position in the history it answered, so it states its role alone.
  position?: number;
  bodyId?: string;
  children: ReactNode;
  // Rendered inside the bubble, under what the turn said: a control for this turn, not for the session.
  footer?: ReactNode;
}

// One bubble for every turn of a span's session, whether it came from the request's history or from the
