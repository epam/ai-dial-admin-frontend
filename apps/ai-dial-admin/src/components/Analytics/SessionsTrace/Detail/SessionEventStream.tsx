...
import {
  ...
  kindsOf,
  markMatchingNodes,
} from '@/src/utils/analytics/session-span-tree';
...
const RowFacts: FC<FactsProps> = ({ facts }) => {
  ...
  if (facts.shape === HopFactsShape.Metered) {
    ...
    return (
      <>
        {facts.tokens !== null && (
          <span>{t(SessionsTraceI18nKey.SpanTokens, { count: formatCompactNumber(facts.tokens) })}</span>
        )}
        {facts.requestMessages !== null && (
          <span>{t(SessionsTraceI18nKey.SpanRequestMessages, { count: facts.requestMessages })}</span>
        )}
  ...
  return <span className={COST_TEXT_CLASS}>{t(SessionsTraceI18nKey.SpanChainCost, { cost: chainCost })}</span>;
};
...
const HopTreeRowView: FC<RowProps> = ({
  ...
  const content = (
    <>
      ...
      >
        {type === null ? t(SessionsTraceI18nKey.TraceRootNotRecorded) : t(SPAN_KIND_LABEL_KEY[type])}
      </span>
      ...
      <div className={cardClassName}>
        ...
        {children.length > 0 ? (
          <DialGhostIconButton
            ...
            aria-label={t(expanded ? SessionsTraceI18nKey.TreeCollapse : SessionsTraceI18nKey.TreeExpand)}
...
const SessionEventStream: FC<Props> = ({ tree, selectedSpanId, onSelectSpan }) => {
  const t = useI18n();
  const treeId = useId();
    ...
    return (
      <div className="flex flex-1 flex-col justify-center bg-layer-1">
        <DialNoDataContent
          ...
          title={t(SessionsTraceI18nKey.TraceNoSpans)}
      ...
      >
        ...
        {hasFailures && (
          <GhostButton
            ...
            label={t(SessionsTraceI18nKey.EventFailed)}
            iconBefore={
        ...
        <span role="status" aria-live="polite" className="ml-auto font-mono text-secondary dial-caption-text">
          {emphasis === null ? '' : t(SessionsTraceI18nKey.StreamMatchCount, { count: matchCount, total: totalCount })}
        </span>
      </div>
      <div
        role="group"
        aria-label={t(SessionsTraceI18nKey.StreamLabel)}
...
export default SessionEventStream;
