...
import SessionTraceChips from '@/src/components/Analytics/SessionsTrace/Detail/SessionTraceChips';
import RatingCounts from '@/src/components/Analytics/SessionsTrace/RatingCounts';
import { COST_TEXT_CLASS, EMPTY_ICON_SIZE, UNAVAILABLE_VALUE } from '@/src/constants/analytics/sessions-trace';
...
import { sessionCardId, traceCardTitle } from '@/src/utils/analytics/session-trace-groups';
...
const TraceTier: FC<{ group: SessionTraceGroup; hasChevron?: boolean; hasTotals?: boolean }> = ({
  group,
  hasChevron = true,
  hasTotals = false,
}) => {
  ...
  return (
    <span className="flex min-w-0 items-center gap-3 border-t border-primary pl-6 pt-2">
      <SessionTraceChips chips={group.chips} />
      ...
      {group.failedSpans > 0 && (
        <span className="shrink-0 text-error dial-tiny-text">
          {t(SessionsTraceI18nKey.TraceFailuresInside, { count: formatCompactNumber(group.failedSpans) })}
        </span>
      )}
      <span className="ml-auto flex shrink-0 items-center gap-1 font-mono text-accent-primary dial-tiny-text">
        {t(SessionsTraceI18nKey.TraceCardSpans, { count: formatCompactNumber(group.spans) })}
...
interface CardProps {
  ...
  onOpenTrace: (group: SessionTraceGroup, card: SessionTraceCard) => void;
}
  ...
  return (
    ...
    >
      ...
      <span className="flex min-w-0 items-center gap-2">
        ...
        {card.isCoreInternal && (
          <span className="shrink-0 rounded bg-warning px-1.5 text-warning dial-tiny-text">
            {t(SessionsTraceI18nKey.TraceCardCoreInternal)}
          </span>
        )}
        ...
        <span className={classNames('flex shrink-0 items-center gap-1', isFailed && 'text-error')}>
          ...
          <span className="dial-tiny-text">
            {isFailed
              ? `${t(SessionsTraceI18nKey.TraceCardFailed)} ${card.responseStatus ?? ''}`.trim()
              : t(SessionsTraceI18nKey.TraceCardSucceeded)}
          </span>
        </span>
      ...
      <span className="flex flex-wrap items-center gap-x-4 gap-y-1 pl-6">
        <Fact label={t(SessionsTraceI18nKey.TraceCardStarted)}>
          {startedAtMs === null ? UNAVAILABLE_VALUE : formatTimeToLocalString(startedAtMs)}
        </Fact>
        <Fact label={t(SessionsTraceI18nKey.TraceCardMessages)}>
          {formatCompactNumber(card.requestMessages) || UNAVAILABLE_VALUE}
        </Fact>
        <Fact label={t(SessionsTraceI18nKey.TraceCardOwnTokens)}>
        ...
        <Fact label={t(SessionsTraceI18nKey.TraceCardOwnPrice)}>
          <span className={COST_TEXT_CLASS}>{formatSignificantCost(card.ownPrice) || UNAVAILABLE_VALUE}</span>
        </Fact>
        <Fact label={t(SessionsTraceI18nKey.TraceCardChainPrice)}>
...
interface UnrecordedProps {
  group: SessionTraceGroup;
  onOpenTrace: (group: SessionTraceGroup) => void;
}
...
// capability every row of the previous listing had, on sessions whose every trace has this shape.
  ...
  return (
    ...
    >
      <span className="flex min-w-0 items-center gap-2">
        ...
        <span className="min-w-0 text-primary dial-small-semi-text">
          {t(SessionsTraceI18nKey.TraceRootNotRecorded)}
        </span>
        <span className="max-w-[16ch] shrink-0 rounded bg-layer-4 px-1.5 font-mono text-secondary dial-tiny-text">
      ...
      <span className="pl-6 text-secondary dial-tiny-text">{t(SessionsTraceI18nKey.TraceRootNotRecordedHint)}</span>
...
const TraceGroup: FC<GroupProps> = ({ group, ratings, onOpenTrace }) => {
  ...
  const ratedCardId = sessionCardId(group);

  return (
    <div
      role="group"
      aria-label={t(SessionsTraceI18nKey.TraceGroupLabel, { traceId: group.traceId })}
    ...
    >
      {group.cards.map((card) => (
        <TraceCard
          ...
          group={group}
          card={card}
          // Ratings are a trace fact, so they attach to the one card that is the session's own call —
      ...
      {group.elidedCardCount > 0 && (
        <DialNotification
          ...
          message={t(SessionsTraceI18nKey.TraceFurtherCalls, {
...
const SessionTraceList: FC<Props> = ({
  groups,
  traceRatings,
  hasMore,
  isLoading,
    ...
    return (
      <div className="flex flex-1 flex-col justify-center bg-layer-1">
        <DialNoDataContent
          ...
          title={t(hasLoadError ? SessionsTraceI18nKey.TraceListLoadFailed : SessionsTraceI18nKey.TraceListEmpty)}
          description={t(
            hasLoadError ? SessionsTraceI18nKey.TraceListLoadFailedHint : SessionsTraceI18nKey.TraceListEmptyHintLive,
          )}
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto bg-layer-1">
      <div className="flex flex-col gap-1.5 p-4">
        ...
        <div className="flex flex-wrap items-baseline justify-between gap-2 pb-1">
          <h3 className="text-primary dial-small-semi-text">{t(SessionsTraceI18nKey.TraceSectionTitle)}</h3>
          <p className="font-mono text-secondary dial-tiny-text">{t(SessionsTraceI18nKey.TraceSectionRule)}</p>
        </div>
        {hasLoadError && (
          <DialNotification
            variant={NotificationVariant.Error}
            iconSize={NOTIFICATION_ICON_SIZE}
            message={t(SessionsTraceI18nKey.TraceListLoadFailedHint)}
          />
        )}
        {groups.map((group) => (
          <TraceGroup
        ...
        {hasMore && !isLoading && (
          <DialLinkButton label={t(SessionsTraceI18nKey.TraceListLoadMore)} onClick={onLoadMore} />
        )}
...
export default SessionTraceList;
