...
import { SessionTraceChip } from '@/src/models/analytics/sessions-trace';
import { formatCompactNumber, readableWords } from '@/src/utils/analytics/session-formatting';
...
const SessionTraceChips: FC<Props> = ({ chips }) => {
  const t = useI18n();
  ...
  return (
    <span className="flex flex-wrap items-center gap-1.5">
      {chips.map(({ eventKind, spans }) => (
        ...
        >
          {eventKind ? readableWords(eventKind) : t(SessionsTraceI18nKey.TraceChipUnclassified)}
...
export default SessionTraceChips;
