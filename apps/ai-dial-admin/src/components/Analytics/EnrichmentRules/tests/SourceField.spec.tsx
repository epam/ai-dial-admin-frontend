...
describe('SourceField', () => {
  ...
  test('says the source is unresolved before the target lands', () => {
    ...
    expect(screen.getByText(AnalyticsEnrichmentRulesI18nKey.SourceFollowUnresolved)).toBeTruthy();
  });

  test('clears the source when following is chosen', async () => {
    const onChange = vi.fn();
    const user = userEvent.setup();
    renderField({ source: 'legacy_log', onChange });
  ...
  test('switches to pin and stays there', async () => {
    ...
    expect(screen.getByRole('radio', { name: /SourcePin/ })).toBeChecked();
    expect(screen.getByText(AnalyticsEnrichmentRulesI18nKey.SourceTable)).toBeTruthy();
  });
  ...
  test('hides the table select while following', () => {
    ...
    expect(screen.queryByText(AnalyticsEnrichmentRulesI18nKey.SourceTable)).toBeNull();
  });
});
...
describe('SqlPredicateField', () => {
  ...
  test('says so when the read source has not resolved', () => {
    ...
    expect(screen.getByText(AnalyticsEnrichmentRulesI18nKey.PredicateSourceUnresolved)).toBeTruthy();
  });
