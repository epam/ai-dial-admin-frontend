...
import { MemberSelectOrderBy, SortDirection } from '@/src/models/analytics/rule';
...
const OrderByEditor: FC<Props> = ({ orderBy, columns, onChange }) => {
  ...
  const directionOptions = [
    { value: SortDirection.Asc, label: t(AnalyticsEnrichmentRulesI18nKey.SortAscending) },
    { value: SortDirection.Desc, label: t(AnalyticsEnrichmentRulesI18nKey.SortDescending) },
  ];
    ...
    <div className="flex flex-col gap-3">
      {rows.map((row, index) => (
        <div
          ...
          role="group"
          aria-label={`${t(AnalyticsEnrichmentRulesI18nKey.OrderBy)} ${index + 1}`}
          className="flex items-end gap-2"
        >
          <DialSelectField
            id={`order-by-column-${index}`}
            label={index === 0 ? t(AnalyticsEnrichmentRulesI18nKey.OrderByColumn) : undefined}
          ...
          <DialSelectField
            ...
            label={index === 0 ? t(AnalyticsEnrichmentRulesI18nKey.OrderByDirection) : undefined}
          ...
          <DialGhostIconButton
            ...
            aria-label={`${t(ButtonsI18nKey.Delete)} ${t(AnalyticsEnrichmentRulesI18nKey.OrderBy)} ${index + 1}`}
      ...
      <DialGhostButton
        ...
        label={t(AnalyticsEnrichmentRulesI18nKey.AddOrderBy)}
