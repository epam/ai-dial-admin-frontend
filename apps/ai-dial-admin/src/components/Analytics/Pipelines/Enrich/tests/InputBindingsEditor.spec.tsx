...
import { toInputBindings, toInputBindingRows } from '@/src/components/Analytics/Pipelines/Enrich/input-bindings';
...
import { InputBindingKind } from '@/src/models/analytics/pipeline-ui';
import { SortDirection } from '@/src/models/analytics/pipeline';
...
describe('InputBindingsEditor', () => {
  ...
  test('prompts for the evaluator and the source before either resolves', () => {
    ...
    expect(screen.getByText(AnalyticsPipelinesI18nKey.InputBindingsEmpty)).toBeTruthy();
  });
  ...
  test('renders an expression input when the row binds JSONata', () => {
    ...
    expect(screen.getByText(AnalyticsPipelinesI18nKey.BindingJsonata)).toBeTruthy();
  });
  ...
  test('clears the value when the row switches between column and expression', async () => {
    ...
    await user.click(screen.getByText(AnalyticsPipelinesI18nKey.BindingKindColumn));
    await user.click(screen.getByText(AnalyticsPipelinesI18nKey.BindingKindJsonata));
  ...
  test('does not report a JSONata expression as an unresolvable column', () => {
    ...
    expect(screen.queryByText(AnalyticsPipelinesI18nKey.BindingUnavailable)).toBeNull();
  });
...
describe('OrderByEditor', () => {
  ...
  test('adds a complete entry rather than a blank one', async () => {
    ...
    await user.click(screen.getByText(AnalyticsPipelinesI18nKey.AddOrderBy));
  ...
  test('cannot add an entry before the source columns resolve', () => {
    ...
    expect(screen.getByRole('button', { name: AnalyticsPipelinesI18nKey.AddOrderBy })).toBeDisabled();
  });
...
describe('MemberSelectEditor', () => {
  ...
  test('describes the preference as a preference rather than a filter', () => {
    ...
    expect(screen.getByText(new RegExp(AnalyticsPipelinesI18nKey.PreferSqlCaption))).toBeTruthy();
  });
  ...
  test('reports the missing limit once anything is declared', () => {
    ...
    expect(screen.getByText(AnalyticsPipelinesI18nKey.MemberLimitRequired)).toBeTruthy();
  });
  ...
  test('drops member selection entirely when its last value is cleared', async () => {
    ...
    await user.clear(screen.getByLabelText(AnalyticsPipelinesI18nKey.MemberLimit, { exact: false }));
