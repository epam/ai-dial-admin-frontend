...
import { removeRole, updateRole } from '@/src/app/[lang]/assets-roles/actions';
...
const RoleAssetView: FC<Props> = ({ etag, originalRole }) => {
  const t = useI18n();
  const tabs = getTabsForAsset(t, ApplicationRoute.AssetsRoles);
      ...
      if (res.success) {
        showNotification(
          getSuccessNotification(
            getUpdateNotificationTitle(ApplicationRoute.AssetsRoles, t),
            getUpdateNotificationDescription(ApplicationRoute.AssetsRoles, selectedRole.name, t),
          ),
        );
