...
describe('KeysList', () => {
  test('Renders BaseAssetList with the PlatformKeys route', () => {
    const { getByText } = render(<KeysList />);

    expect(getByText('BaseAssetList')).toBeInTheDocument();
  });

  test('Passes the /platform-keys view to BaseAssetList', () => {
    const { container } = render(<KeysList />);

    expect(container.querySelector('[data-view="/platform-keys"]')).toBeTruthy();
  });
});
