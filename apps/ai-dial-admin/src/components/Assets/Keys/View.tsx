...
import { removeKey, rotateKey, updateKey } from '@/src/app/[lang]/assets-keys/actions';
...
const KeyAssetView: FC<Props> = ({ etag, originalKey, roles }) => {
  const t = useI18n();
  const tabs = getTabsForAsset(t, ApplicationRoute.AssetsKeys);
      ...
      if (res.success) {
        showNotification(
          getSuccessNotification(
            getUpdateNotificationTitle(ApplicationRoute.AssetsKeys, t),
            getUpdateNotificationDescription(ApplicationRoute.AssetsKeys, selectedKey.name, t),
          ),
        );
