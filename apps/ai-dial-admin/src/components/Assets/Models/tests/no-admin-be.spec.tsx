...
describe('Model asset :: no admin-backend dependency', () => {
  ...
  test('Should not request the topic catalogue on the model asset surface', async () => {
    render(
      <TopicsControl entity={{ descriptionKeywords: [] }} view={ApplicationRoute.AssetsModels} onChange={vi.fn()} />,
    );
    await openTopics();
  ...
  test('Should still let a topic be added on the model asset surface without the catalogue', async () => {
    ...
    render(
      <TopicsControl entity={{ descriptionKeywords: [] }} view={ApplicationRoute.AssetsModels} onChange={onChange} />,
    );
    await openTopics();
