...
export const isEmbedding = ({ event_kind }: SessionSpanRow): boolean => event_kind?.trim() === EMBEDDING_EVENT_KIND;

export const isMcpCall = ({ event_kind }: SessionSpanRow): boolean => event_kind?.trim() === MCP_EVENT_KIND;
...
export const isRating = ({ request_uri }: SessionSpanRow): boolean => pathOf(request_uri).endsWith(RATE_URI_SUFFIX);

export const isModelCall = ({ event_kind, request_uri }: SessionSpanRow): boolean => {
...
export const spanKindOf = (span: SessionSpanRow): SpanKind => {
  if (isRating(span)) {
...
export const isFailedHop = ({ success, response_status }: SessionSpanRow): boolean =>
...
export const spanLabelOf = ({ deployment, request_uri, core_span_id }: SessionSpanRow): string =>
...
export const spanPhaseOf = ({ mcp_tool_call_name, mcp_method }: SessionSpanRow): string | null =>
  mcp_tool_call_name?.trim() || mcp_method?.trim() || null;

export const areSpansPartial = (spans: SessionSpanRow[], hopCount: number | null): boolean =>
...
// empty.
export const isProtocolEnvelope = ({ mcp_tool_call_name }: SessionSpanRow): boolean => !mcp_tool_call_name?.trim();

// The one protocol message answered with no body. By prefix: the set a server sends is not ours to fix.
export const isProtocolNotification = ({ mcp_method, mcp_tool_call_name }: SessionSpanRow): boolean =>
...
export const hopTransportOf = (span: SessionSpanRow): HopTransport => {
...
export const hopFactsOf = (span: SessionSpanRow): HopFacts | null => {
...
export const mcpToolCallTallyOf = (spans: SessionSpanRow[], isComplete: boolean): McpToolCallTally => {
...
export const hopSideSuppressionsOf = (span: SessionSpanRow): HopSideSuppressions => {
...
export const spanBodyTabsOf = (span: SessionSpanRow, grants: HopBodyGrants): SpanBodyTab[] => {
