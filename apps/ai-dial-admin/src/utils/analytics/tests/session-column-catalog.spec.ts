...
import { SESSIONS_TRACE_COLUMNS } from '@/src/constants/grid-columns/grid-columns';
import {
  ColumnProvenance,
  SessionColumn,
  SessionsField,
...
import {
  ...
  hopBodyFields,
} from '@/src/utils/analytics/session-column-catalog';
import {
...
const SCHEMA: AnalyticsEntityField[] = [
  ...
  sourceField('chain_price_total', {
    ...
    description: 'Session cost summed top-down. That is a coverage gap, not an accounting difference.',
  }),
...
const columns = (schemaFields: AnalyticsEntityField[] = SCHEMA): ColDef[] => SESSIONS_TRACE_COLUMNS(t, schemaFields);
...
describe('SESSIONS_TRACE_COLUMNS', () => {
  ...
  test('withholds a non-scalar field, whose shape the grid does not know', () => {
    expect(fieldsOf(columns())).not.toContain(SessionsField.Traces);
  });
  ...
  test('offers a field a curated column already reads only once', () => {
    ...
    expect(offered.filter((name) => name === SessionsField.Deployments)).toHaveLength(1);
    expect(offered.filter((name) => name === SessionsField.InsightTopics)).toHaveLength(1);
    expect(offered).not.toContain(SessionsField.FirstRequestTime);
    expect(offered).not.toContain(SessionsField.InsightTitle);
  });
...
describe('columnProvenance', () => {
  test('attributes an unqualified name to the rollup', () => {
    expect(columnProvenance(SessionsField.TotalPrice)).toBe(ColumnProvenance.Sessions);
  });

  test('attributes a qualified name to the enrichment that supplies it', () => {
    expect(columnProvenance(SessionsField.InsightTopics)).toBe(ColumnProvenance.Insights);
  });
  ...
  test('attributes the composed rating column to the feedback source', () => {
    expect(columnProvenance(SessionColumn.Rating)).toBe(ColumnProvenance.Feedback);
  });
});

describe('sessionColumnGroups', () => {
  const groups = (schemaFields: AnalyticsEntityField[] = SCHEMA) =>
    sessionColumnGroups(columns(schemaFields), schemaFields);

  test('groups on the pair of origin and tag', () => {
    const identity = groups().find(
      (group) => group.provenance === ColumnProvenance.Sessions && group.tag === 'identity',
    );

    expect(identity?.fields).toContain(SessionsField.ChatId);
    expect(identity?.fields).toContain(SessionsField.LastRequestTime);
  });
  ...
  test('keeps a rollup field and an enrichment field sharing a tag in separate groups', () => {
    ...
    expect(costGroups.map((group) => group.provenance)).toEqual([ColumnProvenance.Sessions, ColumnProvenance.Other]);
  });
  ...
  test('collects the evaluator bookkeeping into one group of its own', () => {
    ...
    expect(provenance?.fields).not.toContain(SessionsField.InsightTopics);
  });

  test('collapses to one group per origin when the schema reports no tags', () => {
    expect(groups([]).map((group) => group.provenance)).toEqual([ColumnProvenance.Sessions, ColumnProvenance.Feedback]);
  });
});
...
describe('projectableSchemaFields', () => {
  test('projects a cheap source field whether or not its column is visible', () => {
    ...
    expect(cheapSource).toContain(SessionsField.ChatId);
    expect(cheapSource).toContain('duration_ms');
    expect(cheapSource).toContain(SessionsField.FirstRequestTime);
  });
  ...
  test('projects no field for a column the grid does not render', () => {
    ...
    expect(cheapSource).not.toContain(SessionsField.Traces);
  });
  ...
  test('separates an enrichment field, whose join is paid per page', () => {
    ...
    expect(enrichment).toContain(SessionsField.InsightTopics);
    expect(enrichment).toContain('session_insights.sentiment');
    expect(cheapSource).not.toContain(SessionsField.InsightTopics);
  });
  ...
  test('projects the identity column title unconditionally, with no column of its own', () => {
    ...
    expect(requiredEnrichment).toEqual([SessionsField.InsightTitle]);
    expect(enrichment).not.toContain(SessionsField.InsightTitle);
  });
  ...
  test('excludes the composed rating column, which reads no field of this entity', () => {
    ...
    expect([...cheapSource, ...heavySource, ...enrichment, ...requiredEnrichment]).not.toContain(SessionColumn.Rating);
  });
...
describe('catalogValueTypes', () => {
  ...
  test('keeps the curated value types where the schema adds nothing', () => {
    expect(catalogValueTypes([])[SessionsField.TotalPrice]).toBe(QueryValueType.Decimal);
  });
});
...
describe('sortableColumnFields / filterableColumnFields', () => {
  test('offer a derived scalar column and withhold the arrays', () => {
    ...
    expect(sortableColumnFields(columns())).not.toContain(SessionsField.Deployments);
    expect(sortableColumnFields(columns())).not.toContain(SessionsField.InsightTopics);
  });

  test('withhold the period axis the toolbar already owns', () => {
    expect(filterableColumnFields(columns())).not.toContain(SessionsField.LastRequestTime);
  });

  test('drop an enrichment predicate the instance cannot answer', () => {
    const withoutInsights = SCHEMA.filter((field) => !field.name.startsWith('session_insights.'));

    expect(filterableColumnFields(columns(withoutInsights))).not.toContain(SessionsField.InsightTopics);
  });
});
...
describe('a derived column binds a filter its type can answer', () => {
  ...
  test('an enum field binds the value filter and its own floating filter', () => {
    ...
    expect(column.filter).toBe(SESSION_VALUE_FILTER);
    expect(column.floatingFilterComponent).toBe(SESSION_VALUE_FLOATING_FILTER);
...
describe('composedSourceEntities', () => {
  ...
  test('names an enrichment once however many of its fields the schema reports', () => {
    const entities = composedSourceEntities('sessions', [
  ...
  test('attributes an enrichment it cannot name to the unattributed provenance', () => {
    const [, unknown] = composedSourceEntities('sessions', [field('some_future_enrichment.value')]);
  ...
  test('names the base entity alone when the schema reports no enrichment', () => {
    expect(composedSourceEntities('sessions', [field('chat_id')])).toEqual([
      { name: 'sessions', provenance: ColumnProvenance.Sessions },
    ]);
  });

  test('names nothing beyond the base entity when given no schema at all', () => {
    expect(composedSourceEntities('sessions')).toEqual([{ name: 'sessions', provenance: ColumnProvenance.Sessions }]);
  });
  ...
  test('names an entity once, keeping the declared attribution over the derived fallback', () => {
    const entities = composedSourceEntities('sessions', [field('response_ratings.rate_pos_count')], [RATINGS]);

    expect(entities).toEqual([{ name: 'sessions', provenance: ColumnProvenance.Sessions }, RATINGS]);
  });
