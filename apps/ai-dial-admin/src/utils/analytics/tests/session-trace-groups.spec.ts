...
} from '@/src/utils/analytics/session-trace-groups';
...
const pageRow = (traceId: string, firstMs: number, lastMs = firstMs): SessionTracePageRow => ({
  trace_id: traceId,
  first_request_time: firstMs,
  last_request_time: lastMs,
});

const root = (over: Partial<SessionTraceRootRow> = {}): SessionTraceRootRow => ({
  trace_id: TRACE,
  ...
  'usage_client_identity.client_session_id': 'the-session',
...
const figure = (over: Partial<SessionTraceFigureRow> = {}): SessionTraceFigureRow => ({
  trace_id: TRACE,
  event_kind: 'llm_call',
  spans: 4,
  tokens: 11615,
...
const groupsOf = (
  ...
  rootCap?: number,
) => traceGroupsOf(pageRows, rootRows, figureRows, SESSION_PROJECT, rootCap);

describe('isCoreInternalRoot', () => {
  test('marks a root recorded under a project other than the session own', () => {
    expect(isCoreInternalRoot(root({ project_id: CORE_PROJECT }), 'demo-project')).toBe(true);
  });

  test('does not mark a root sharing the session project', () => {
    expect(isCoreInternalRoot(root({ project_id: 'claude-eval-key1' }), 'claude-eval-key1')).toBe(false);
    expect(isCoreInternalRoot(root({ project_id: '' }), SESSION_PROJECT)).toBe(false);
  });

  // The measured chat shape, and the one an earlier version of this predicate left unmarked: the session
  // carries no project at all while Core records its own service call under `dial`. Requiring the
  // session's project to be non-empty hid the marker on exactly the traces it exists for — caught in the
  // browser against trace 64a9c812, where the title-generation card rendered unmarked beside its client call.
  test('marks a Core service root even where the session carries no project', () => {
    expect(isCoreInternalRoot(root({ project_id: CORE_PROJECT }), SESSION_PROJECT)).toBe(true);
  });

  // The blank handling is asymmetric on purpose. A blank on the root side is an absence — the row records no
  // project, which says nothing about who called it. A blank on the session side is still a difference
...
describe('traceGroupsOf', () => {
  ...
  // root the session own filter would have missed, and no count is incremented.
  ...
  test('records whether a card own row carried the session header', () => {
    const [group] = groupsOf(
      [pageRow(TRACE, 1000)],
      [
        root({
    ...
    expect(group.cards.map(({ hasSessionLabel }) => hasSessionLabel)).toEqual([true, false]);
  });
...
describe('attributeRatingsToTraces', () => {
  const feedback = (over: Partial<SessionFeedbackRow> = {}): SessionFeedbackRow => ({
    response_id: 'r1',
  ...
  // still counted by the feedback panel, whose figures come from a session-scoped aggregate.
...
describe('traceInvariantViolations', () => {
  const violationsOf = (roots: SessionTraceRootRow[], projectId = 'demo-project') =>
  ...
  test('reports a trace whose roots carry two sessions', () => {
    expect(
      violationsOf([
    ...
    ).toContain(TraceInvariant.OneSessionPerTrace);
  });
  ...
  test('reports several roots where none carries the session header', () => {
    expect(
      violationsOf([
  ...
  // session's own call. No such trace is recorded, so this guards rather than being relied upon.
