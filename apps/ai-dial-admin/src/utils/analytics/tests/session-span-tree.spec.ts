...
const hop = (core_span_id: string, overrides: Partial<SessionSpanRow> = {}): SessionSpanRow =>
  ({
    core_span_id,
...
describe('buildHopTree', () => {
  ...
  describe('route hops', () => {
    ...
    test('renders every hop of a turn whose rows all record an empty session id', () => {
      const tree = buildHopTree([
        hop('a', { chat_id: '' } as Partial<SessionSpanRow>),
