...
const ALL_FIELDS: string[] = Object.values(SessionsField);
...
const detailQuery = (availableFields: string[] | undefined = ALL_FIELDS) =>
  buildSessionDetailQuery(CHAT_ID, availableFields && asSchemaFields(availableFields));
...
describe('buildSessionDetailQuery', () => {
  test('reads the sessions entity in row mode', () => {
    const query = detailQuery();

    expect(query.entity).toBe(SESSIONS_ENTITY);
    expect(query.mode).toBe(QueryMode.Row);
    expect(query.group_by).toBeUndefined();
  });

  test('filters to one session by equality and requests a single row', () => {
    const query = detailQuery();
    const predicate = asPredicate(query.filter);

    expect(predicate.op).toBe(QueryOperator.Eq);
    expect(predicate.args[0]).toEqual({ type: QueryExprType.Field, name: SessionsField.ChatId });
  ...
  // to render it silently widens every single-session query.
  test('selects every stored column of the rollup', () => {
    const names = selectedNames(detailQuery().select);

    expect(names).toEqual(Object.values(SessionsField));
  ...
  test('names the heavy trace column explicitly', () => {
    ...
    expect(names).toContain(SessionsField.Traces);
  ...
  test('names the insight columns by their qualified flat names', () => {
    ...
    expect(names).toContain(SessionsField.InsightTitle);
  });
  ...
  test('omits one descriptive insight column the schema does not report', () => {
    const withoutResolution = ALL_FIELDS.filter((name) => name !== SessionsField.InsightResolutionStatus);
    const names = selectedNames(detailQuery(withoutResolution).select);

    expect(names).not.toContain(SessionsField.InsightResolutionStatus);
    expect(names).toContain(SessionsField.InsightSentiment);
    expect(names).toContain(SessionsField.InsightSummary);
  });
  ...
  test('does not name an insight column the detail view could not render as a value', () => {
    ...
    const names = selectedNames(buildSessionDetailQuery(CHAT_ID, fields).select);
  ...
  test('omits the columns an instance does not carry', () => {
    ...
    expect(names).not.toContain(SessionsField.InsightTitle);
    expect(names).toContain(SessionsField.Traces);
    expect(names).toContain(SessionsField.ChatId);
  });

  test('names the required core alone when no schema is available', () => {
    const names = selectedNames(buildSessionDetailQuery(CHAT_ID).select);

    expect([...names].sort()).toEqual([...REQUIRED_DETAIL_SELECT_FIELDS].sort());
    expect(names).not.toContain(SessionsField.Traces);
    expect(names).not.toContain(SessionsField.InsightTitle);
  });
  ...
  test('classifies every field of the entity as required or optional', () => {
    expect([...REQUIRED_DETAIL_SELECT_FIELDS, ...OPTIONAL_DETAIL_SELECT_FIELDS].sort()).toEqual(
      [...Object.values(SessionsField)].sort(),
    );
  });

  // The list query bounds last_request_time to the selected period; a detail view addressed by id must
  // resolve whatever period the log was showing, so a bookmark cannot break as a session ages out.
  test('carries no time bound', () => {
    const query = detailQuery();
    const serialized = JSON.stringify(query.filter);

    expect(serialized).not.toContain(SessionsField.LastRequestTime);
    expect(serialized).not.toContain(SessionsField.FirstRequestTime);
  ...
  test('encodes an id containing path separators verbatim into the value', () => {
    const pathLike = 'sessions/eRxsos/chathub-claude4__E2ESession';
    const predicate = asPredicate(buildSessionDetailQuery(pathLike).filter);

    expect((predicate.args[1] as QueryValueExpr).value).toBe(pathLike);
  });
});

describe('buildSessionFeedbackQuery', () => {
  const ALL_FEEDBACK_FIELDS: string[] = Object.values(ResponseRatingsField);
  const feedbackQuery = (limit = SESSION_FEEDBACK_LIMIT, schemaFields: string[] | undefined = ALL_FEEDBACK_FIELDS) =>
    buildSessionFeedbackQuery(CHAT_ID, limit, schemaFields);

  test('reads the rating rollup in row mode, filtered to one session', () => {
  ...
  test('names the comment text only when the schema reports it', () => {
    ...
    const names = selectedNames(feedbackQuery(SESSION_FEEDBACK_LIMIT, withoutText).select);
  ...
  test('always names the comment count, which the service does not gate', () => {
    const names = selectedNames(buildSessionFeedbackQuery(CHAT_ID, SESSION_FEEDBACK_LIMIT).select);
...
describe('buildSessionSpansQuery', () => {
  ...
  const query = (names: string[] = SPAN_BASE_FIELDS) => buildSessionSpansQuery(TRACE_ID, SESSION_SPAN_LIMIT, names);
  ...
  // carrying no session header, and the Core-internal calls recorded under the trace — so the drawer
  // reported fewer hops than the card that opened it, and the root the card describes was missing from its
  // own span tree.
  test('narrows to one trace and does not require the session header', () => {
  ...
  test('orders hops by when they were made and asks how many there are', () => {
    ...
    expect(page.limit).toBe(SESSION_SPAN_LIMIT);
    expect(page.include_total).toBe(true);
  });
});
...
const window = () => paddedUtcDayRange([NOON_UTC]) as SessionTraceWindow;
...
describe('paddedUtcDayRange', () => {
  ...
  test('covers a root recorded just before midnight and its child just after', () => {
    ...
    const { fromMs, toMs } = paddedUtcDayRange([childMs]) as SessionTraceWindow;
  ...
  test('the padding, not just the day boundary, is what covers that root', () => {
    ...
    expect(rootMs).toBeGreaterThanOrEqual((paddedUtcDayRange([childMs]) as SessionTraceWindow).fromMs);
  });
...
describe('buildSessionTracePageQuery', () => {
  const query = () => buildSessionTracePageQuery(CHAT_SCOPE, PROJECT_ID, window(), 0, SESSION_TRACE_PAGE_SIZE);
  ...
  test('filters by the session, its project and the padded window', () => {
  ...
  test('yields the trace id and its own two bounds under their aliases', () => {
    ...
    expect(aliases).toEqual([undefined, SessionTracePageField.FirstRequestTime, SessionTracePageField.LastRequestTime]);
  ...
  test('orders ascending by the trace start, tie-broken by trace id', () => {
    expect(query().sort).toEqual([
      { field: SessionTracePageField.FirstRequestTime, dir: QuerySortDirection.Asc },
      { field: UsageLogField.TraceId, dir: QuerySortDirection.Asc },
    ]);
  });

  test('pages by offset at the listing page size', () => {
    const page = buildSessionTracePageQuery(CHAT_SCOPE, PROJECT_ID, window(), 100, SESSION_TRACE_PAGE_SIZE)
      .page as QueryOffsetPage;

    expect(page.offset).toBe(100);
    expect(page.limit).toBe(SESSION_TRACE_PAGE_SIZE);
  });
});

describe('buildSessionTraceRootsQuery', () => {
  const query = () => buildSessionTraceRootsQuery(PAGE_TRACE_IDS, window(), SESSION_TRACE_PAGE_SIZE);
  ...
  // project against the session's, so the value must be read; filtering on it would drop the
...
describe('buildSessionTraceFiguresQuery', () => {
  const query = () => buildSessionTraceFiguresQuery(PAGE_TRACE_IDS, window(), SESSION_TRACE_PAGE_SIZE);
  ...
  test('counts failures with a conditional sum rather than a filter', () => {
    ...
    expect(serialized).toContain(SessionTraceFigureField.FailedSpans);
  });
...
describe('the roots and figures passes are scoped identically', () => {
  ...
  test('their filters are equal modulo the root-span predicate', () => {
    const roots = buildSessionTraceRootsQuery(PAGE_TRACE_IDS, window(), SESSION_TRACE_PAGE_SIZE);
    const figures = buildSessionTraceFiguresQuery(PAGE_TRACE_IDS, window(), SESSION_TRACE_PAGE_SIZE);
