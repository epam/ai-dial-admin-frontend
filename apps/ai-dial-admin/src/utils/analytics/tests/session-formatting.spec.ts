...
describe('formatSessionSpan', () => {
  test.each([
    ...
    expect(formatSessionSpan(NOW - span, NOW)).toBe(expected);
  });

  test('renders a single-turn session as at least one second, not zero', () => {
    expect(formatSessionSpan(NOW, NOW)).toBe('1 sec');
  });

  test('renders nothing when the bounds are inverted rather than a negative span', () => {
    expect(formatSessionSpan(NOW, NOW - HOUR)).toBe('');
  });
  ...
  test.each([
    ...
    expect(formatSessionSpan(from, to)).toBe('');
  });
});

describe('formatHopDuration', () => {
  // Hop scale, not session scale: recorded hop durations start at 6 ms, and rendering those as `0s` is
...
describe('formatSessionDuration', () => {
  test.each([
    ...
    expect(formatSessionDuration(millis)).toBe(expected);
  });

  test('reads a numeric string as well as a number', () => {
    expect(formatSessionDuration('6709')).toBe('6.7s');
  });

  // A session that ran took time, so a zero records that the backend never measured it.
  test.each([
    ['zero', 0],
    ['a negative value', -1],
    ['null', null],
    ...
    expect(formatSessionDuration(value)).toBe(UNAVAILABLE_VALUE);
  });
});

describe('sessionTopics', () => {
  test('splits a comma-and-space list into its terms', () => {
    expect(sessionTopics('security, code review, validation')).toEqual(['security', 'code review', 'validation']);
  });
  ...
  test('splits on the comma alone', () => {
    expect(sessionTopics('capabilities,error')).toEqual(['capabilities', 'error']);
  });

  test('trims whatever spacing came back', () => {
    expect(sessionTopics('  security ,   validation  ')).toEqual(['security', 'validation']);
  });
  ...
  test('returns a single term for a value with no separator', () => {
    expect(sessionTopics('security')).toEqual(['security']);
  });

  // Absent is the common case: under a quarter of sessions carry an insight row.
  test.each([[undefined], [null], [''], ['   '], [', ,']])('returns no terms for %s', (raw) => {
    expect(sessionTopics(raw as string | undefined)).toEqual([]);
  });
  ...
  test.each([[42], [true]])('returns no terms for the non-string value %s', (raw) => {
    expect(sessionTopics(raw as unknown as string)).toEqual([]);
  });
});
