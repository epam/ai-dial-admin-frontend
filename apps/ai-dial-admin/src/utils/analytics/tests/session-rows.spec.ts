...
import { SessionRatingRow, SessionRow } from '@/src/models/analytics/sessions-trace';
import {
  attachRatings,
  sessionRatingCounts,
  ...
  negativeRatingGap,
  unresolvedRatings,
} from '@/src/utils/analytics/session-rows';

const ratingRow = (chat_id: string, overrides: Partial<SessionRatingRow> = {}): SessionRatingRow => ({
  chat_id,
  rating_up: 0,
  rate_zero: 0,
  rate_negative: 0,
...
const rows = (count: number, overrides: Partial<SessionRow> = {}) =>
  Array.from({ length: count }, (_, index) => row({ client_session_id: `chat-${index}`, ...overrides }));

describe('sessionRatingCounts', () => {
  test('composes the negative figure from the zero and negative counts', () => {
    expect(sessionRatingCounts(ratingRow('chat-1', { rating_up: 2, rate_zero: 3, rate_negative: 1 }))).toMatchObject({
      rating_up: 2,
      rating_down: 4,
    });
  });
  ...
  test('reports one like and one dislike as one each', () => {
    expect(sessionRatingCounts(ratingRow('chat-1', { rating_up: 1, rate_negative: 1 }))).toMatchObject({
      rating_up: 1,
      rating_down: 1,
    });
  });
  ...
  test('counts a provable negative from the boolean-false and negative columns', () => {
    expect(
      sessionRatingCounts(ratingRow('chat-1', { rate_zero: 5, rate_negative: 2, rate_bool_false: 3 })),
    ).toMatchObject({ rating_down: 7, provable_down: 5 });
  });

  test('reads a count returned as a string, as a ClickHouse aggregate may', () => {
    expect(sessionRatingCounts(ratingRow('chat-1', { rating_up: '4', rate_zero: '2' }))).toMatchObject({
      rating_up: 4,
      rating_down: 2,
    });
  });
  ...
  test.each([
    ...
    expect(sessionRatingCounts(ratingRow('chat-1', { rating_up: count }))).toMatchObject({ rating_up: 0 });
  });

  test('reports a resolved zero where the rollup returned no row', () => {
    expect(sessionRatingCounts(undefined)).toEqual({
      rating_up: 0,
      rating_down: 0,
      provable_down: 0,
      captured_form: 0,
...
describe('hasNegativeRatingCaveat', () => {
  ...
  test('an unrated session carries none', () => {
    expect(
...
describe('attachRatings', () => {
  ...
  test('ignores rating rows for sessions not on the page, preserving order and other fields', () => {
