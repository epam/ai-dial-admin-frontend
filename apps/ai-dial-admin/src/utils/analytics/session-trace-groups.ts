import { SESSION_TRACE_ROOT_CAP } from '@/src/constants/analytics/sessions-trace';
import {
  SessionFeedbackRow,
  SessionTraceCard,
  SessionTraceChip,
...
import { feedbackRowCounts } from '@/src/utils/analytics/session-detail-fields';
import { toMillis } from '@/src/utils/analytics/session-formatting';
import { toNumber } from '@/src/utils/analytics/scalar';

// A root recorded under a project other than the session's is one of Core's own service calls — title
// generation and similar. Core makes them under its own project while the client's rows carry the
// session's, so the projects differing is a categorical signal, not an inference from the request's size
// or shape.
//
...
// about who called it — so it is not marked. A blank on the **session** side is still a real difference
// when the root names a project: chat sessions routinely carry no project at all while Core records its
// own service calls under `dial`, which is exactly the measured two-card shape. Requiring the session's
// project to be non-empty left that shape unmarked.
export const isCoreInternalRoot = (root: SessionTraceRootRow, sessionProjectId: string): boolean => {
  const rootProject = root.project_id ?? '';

  return rootProject !== '' && rootProject !== sessionProjectId;
};
...
const hasSessionLabel = (root: SessionTraceRootRow): boolean => (root[UsageLogField.ClientSessionId] ?? '') !== '';
...
export const traceCardTitle = (card: SessionTraceCard, traceId: string): string =>
  card.deployment || card.requestUri || traceId;

const cardOf = (root: SessionTraceRootRow, sessionProjectId: string): SessionTraceCard => ({
  ...
  hasSessionLabel: hasSessionLabel(root),
  isCoreInternal: isCoreInternalRoot(root, sessionProjectId),
});
...
const byRecordedTime = (a: SessionTraceRootRow, b: SessionTraceRootRow): number => {
...
const chipsOf = (figures: SessionTraceFigureRow[]): SessionTraceChip[] =>
  figures
...
const sum = (figures: SessionTraceFigureRow[], read: (row: SessionTraceFigureRow) => number): number =>
...
// root's value back into a total or increments a count for a row the session's own filter would have
// missed — there is no such row to compensate for.
export const traceGroupsOf = (
  pageRows: SessionTracePageRow[],
  rootRows: SessionTraceRootRow[],
  figureRows: SessionTraceFigureRow[],
  sessionProjectId: string,
  rootCap: number = SESSION_TRACE_ROOT_CAP,
): SessionTraceGroup[] => {
  ...
  return pageRows.map((pageRow) => {
    ...
    return {
      ...
      cards: roots.slice(0, rootCap).map((root) => cardOf(root, sessionProjectId)),
...
// Which card is the session's own call: the root carrying the chat id where one does, and otherwise the
...
// view guessing at it.
export const sessionCardId = (group: SessionTraceGroup): string | null => {
  const labelled = group.cards.filter(({ hasSessionLabel }) => hasSessionLabel);
...
// unplaced here. It is not lost: the feedback panel's own figures come from a session-scoped aggregate
...
export enum TraceInvariant {
  OneSessionPerTrace = 'one-session-per-trace',
...
// session-per-trace check covers the roots alone — which is where a second session would have to
...
export const traceInvariantViolations = (
  ...
  for (const [traceId, roots] of groupBy(rootRows, ({ trace_id }) => trace_id)) {
    const labelled = roots.filter(hasSessionLabel);
    const sessionIds = distinct(roots.map((root) => root[UsageLogField.ClientSessionId]));
    const coreInternal = roots.filter((root) => isCoreInternalRoot(root, sessionProjectId));

    if (sessionIds.length > 1) {
      violations.push({
        invariant: TraceInvariant.OneSessionPerTrace,
        traceId,
    ...
    const candidates = roots.filter((root) => !isCoreInternalRoot(root, sessionProjectId));
    if (!labelled.length && candidates.length > 1) {
      violations.push({
        invariant: TraceInvariant.OneRootWhenNoneIsLabelled,
        traceId,
        detail: `no root carries the session header and ${candidates.length} roots could be the client call`,
      });
    }

    // The shape that would split the labelling rule from the marker: a labelled root alongside another root
    // that the marker leaves unmarked, so "the session's own call" has two candidates and neither rule
    ...
    const unmarkedUnlabelled = roots.filter(
      (root) => !hasSessionLabel(root) && !isCoreInternalRoot(root, sessionProjectId),
    );
    if (labelled.length && unmarkedUnlabelled.length) {
      violations.push({
        invariant: TraceInvariant.LabellingAgreesWithMarker,
        traceId,
        detail: `a labelled root sits beside ${unmarkedUnlabelled.length} unlabelled root(s) under the session's own project`,
      });
    }
  }
