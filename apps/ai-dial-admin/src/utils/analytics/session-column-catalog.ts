...
import { columnHeaderName, enrichmentOf, unqualified } from '@/src/utils/analytics/session-enrichment';
...
export const composedSourceEntities = (
  ...
  const entities: ProvenanceEntity[] = [
    { name: baseEntity, provenance: ColumnProvenance.Sessions },
...
export const columnProvenance = (fieldName: string): ColumnProvenance => {
  ...
  if (!enrichment) {
    return ColumnProvenance.Sessions;
  }
...
// `translateSessionFilterModel` reads nothing and drops the entry: the header would show an active
...
const typeColumn = (type: AnalyticsFieldType): Partial<ColDef> => {
  ...
  if (type === AnalyticsFieldType.Enum) {
    return { filter: SESSION_VALUE_FILTER, floatingFilterComponent: SESSION_VALUE_FLOATING_FILTER };
  }
...
// `dial_usage_log`, while this grid derives from `sessions`, whose schema reports no body field. There
...
export const buildSessionColumnCatalog = (curated: ColDef[], fields: AnalyticsEntityField[] = []): ColDef[] => {
...
export const sessionColumnGroups = (catalog: ColDef[], fields: AnalyticsEntityField[] = []): SessionColumnGroup[] => {
  const tagByField = new Map(fields.map((field) => [field.name, field.tag ?? '']));
  const groups: SessionColumnGroup[] = [];
...
export const catalogValueTypes = (fields: AnalyticsEntityField[] = []): Record<string, QueryValueType> => {
  const types = { ...SESSION_FIELD_VALUE_TYPE } as Record<string, QueryValueType>;
