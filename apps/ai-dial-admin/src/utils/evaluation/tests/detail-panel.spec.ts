...
describe('parseValue', () => {
  test('returns displayText, rawText, and isLong false for a short plain string', () => {
    const result = parseValue('hello');
    expect(result).toEqual({ displayText: 'hello', rawText: 'hello', isLong: false });
  });

  test('marks a plain string longer than 100 chars as isLong true', () => {
  ...
  test('parses a JSON array and returns correct shape', () => {
    const arr = [{ id: 1 }, { id: 2 }];
  ...
  test('parses an empty JSON array correctly', () => {
  ...
  test('parses a JSON object and returns correct shape', () => {
  ...
  test('treats non-JSON string as plain string', () => {
...
describe('formatContent', () => {
  test('formats valid JSON content with json contentType', () => {
    const input = '{"a":1}';
    const result = formatContent(input, ViewerContentType.Json);
    expect(result).toBe(JSON.stringify({ a: 1 }, null, 2));
  });

  test('returns content as-is when json parsing fails for json contentType', () => {
    const input = 'not json';
    const result = formatContent(input, ViewerContentType.Json);
    expect(result).toBe('not json');
  });

  test('converts escaped newlines for text contentType', () => {
    const result = formatContent('line1\\nline2', ViewerContentType.Text);
    expect(result).toBe('line1\nline2');
  });

  test('converts escaped tabs for text contentType', () => {
    const result = formatContent('col1\\tcol2', ViewerContentType.Text);
    expect(result).toBe('col1\tcol2');
  });

  test('returns plain text as-is for text contentType with no escape sequences', () => {
...
describe('beautifyValue', () => {
  ...
  test('returns pretty-printed JSON for objects', () => {
    const obj = { a: 1, b: 'x' };
    expect(beautifyValue(obj)).toBe(JSON.stringify(obj, null, 2));
  });

  test('returns pretty-printed JSON for arrays', () => {
    const arr = [1, 2, 3];
    expect(beautifyValue(arr)).toBe(JSON.stringify(arr, null, 2));
  });

  test('returns pretty-printed JSON for a string containing JSON object', () => {
    const inner = { key: 'val' };
    const result = beautifyValue(JSON.stringify(inner));
    expect(result).toBe(JSON.stringify(inner, null, 2));
  });

  test('returns the string as-is when it is not JSON', () => {
