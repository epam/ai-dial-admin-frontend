...
import { SESSIONS_TRACE_COLUMNS, SESSIONS_TRACE_COLUMN_GROUPS } from '@/src/constants/grid-columns/grid-columns';
import { SessionsTraceI18nKey } from '@/src/constants/i18n';
import { ColumnProvenance, SessionColumn, SessionsField } from '@/src/models/analytics/sessions-trace';
...
const columns = (schemaFields: AnalyticsEntityField[] = ALL_FIELDS): ColDef[] =>
  SESSIONS_TRACE_COLUMNS(t, schemaFields);
...
const groups = (schemaFields: AnalyticsEntityField[] = ALL_FIELDS): ColGroupDef[] =>
  SESSIONS_TRACE_COLUMN_GROUPS(t, schemaFields);
...
describe('sessions columns :: composition', () => {
  ...
  test('curated headers come from i18n keys, not from the schema', () => {
    expect(column(SessionsField.ChatId).headerName).toBe(SessionsTraceI18nKey.Session);
    expect(column(SessionsField.TotalPrice).headerName).toBe(SessionsTraceI18nKey.Cost);
    expect(column(SessionsField.InsightTopics).headerName).toBe(SessionsTraceI18nKey.Topics);
  });
  ...
  test('offers no column for an array field the curated set does not read', () => {
    expect(columns().map((col) => col.field)).not.toContain(SessionsField.Traces);
  });
  ...
  test('gives the insight title no column of its own', () => {
    expect(columns().map((col) => col.field)).not.toContain(SessionsField.InsightTitle);
  });

  test('gives a field the activity cell composes no column of its own', () => {
    expect(columns().map((col) => col.field)).not.toContain(SessionsField.FirstRequestTime);
  });
  ...
  test('the identity column cannot be hidden', () => {
    const identity = column(SessionsField.ChatId);
  ...
  test('every other column can be hidden', () => {
    columns()
      .filter((col) => col.field !== SessionsField.ChatId)
      .forEach((col) => expect(col.lockVisible).toBeFalsy());
  });

  test('the identity column discloses where its title comes from', () => {
    expect(column(SessionsField.ChatId).headerTooltip).toBe(SessionsTraceI18nKey.SessionHint);
  });

  test('the topics column discloses that its values come from an evaluation', () => {
    expect(column(SessionsField.InsightTopics).headerTooltip).toBe(SessionsTraceI18nKey.TopicsHint);
  });

  test('the session column renders through a cell renderer', () => {
    expect(column(SessionsField.ChatId).cellRenderer).toBeTypeOf('function');
  });
});

describe('sessions columns :: origins and tags', () => {
  ...
  test('groups on the pair of origin and tag rather than on origin alone', () => {
    expect(childFields(groupOf(ColumnProvenance.Sessions, 'identity'))).toEqual([
      SessionsField.ChatId,
      SessionsField.LastRequestTime,
    ]);
    expect(childFields(groupOf(ColumnProvenance.Sessions, 'performance'))).toEqual(['duration_ms', 'avg_duration_ms']);
  });

  test('a rollup group is named by its tag in readable words', () => {
    expect(groupOf(ColumnProvenance.Sessions, 'token-usage').headerName).toBe(SessionsTraceI18nKey.TagTokenUsage);
  });
  ...
  test('an enrichment group names the enrichment in its label, while a rollup group does not', () => {
    ...
    expect(groupOf(ColumnProvenance.Sessions, 'cost').headerName).toBe(SessionsTraceI18nKey.TagCost);
  });
  ...
  test('an insight column is attributed to the enrichment, not the rollup', () => {
    expect(childFields(groupOf(ColumnProvenance.Insights, 'insight'))).toContain(SessionsField.InsightTopics);
    expect(childFields(groupOf(ColumnProvenance.Sessions, 'identity'))).not.toContain(SessionsField.InsightTopics);
  });
  ...
  test('a tag it holds no label for still yields a group and keeps its columns', () => {
    ...
    const saturation = groups(withNewTag).find(
      (group) => group.groupId === `${ColumnProvenance.Sessions}:saturation`,
    ) as ColGroupDef;
  ...
  test('two enrichments it cannot name get distinct groups and distinct ids', () => {
    ...
    expect(unnamed.map((group) => group.headerName)).toEqual([
      `conversation_buckets · ${SessionsTraceI18nKey.TagCost}`,
      `conversation_topics · ${SessionsTraceI18nKey.TagCost}`,
    ]);
  });
  ...
  test('each group states its origin on hover, so colour is not the only carrier', () => {
    expect(groupOf(ColumnProvenance.Insights, 'insight').headerTooltip).toContain(
      SessionsTraceI18nKey.ProvenanceInsights,
    );
    expect(groupOf(ColumnProvenance.Sessions, 'cost').headerTooltip).toContain(SessionsTraceI18nKey.ProvenanceSessions);
  });
  ...
  test('groups collapse to one per origin when no schema was read', () => {
    expect(groups([]).map((group) => group.headerName)).toEqual([
      SessionsTraceI18nKey.ProvenanceSessions,
      SessionsTraceI18nKey.ProvenanceFeedback,
    ]);
  });
});

describe('sessions columns :: a lagging deployment', () => {
  ...
  test('omits the topics column when the schema reports no insight field', () => {
    expect(columns(WITHOUT_INSIGHTS).map((col) => col.field)).not.toContain(SessionsField.InsightTopics);
  });
  ...
  test('keeps the columns the instance does carry', () => {
    ...
    expect(fields).toContain(SessionsField.Deployments);
    expect(fields).toContain('duration_ms');
  });

  test('keeps the rating column, which reads no field of this entity', () => {
    expect(columns(WITHOUT_INSIGHTS).map((col) => col.field)).toContain(SessionColumn.Rating);
    expect(columns([]).map((col) => col.field)).toContain(SessionColumn.Rating);
  });

  test('falls back to the curated columns alone when no schema is given', () => {
    expect(columns([]).map((col) => col.field)).toEqual(
      CURATED.filter((fieldName) => fieldName !== SessionsField.InsightTopics),
    );
  });
});

describe('sessions columns :: sort and filter contract', () => {
  const SORTABLE = [
    SessionsField.ChatId,
    SessionsField.ProjectId,
    SessionsField.UserHash,
  ...
  test('rating is not sortable and offers no filter', () => {
    expect(column(SessionColumn.Rating).sortable).toBe(false);
    expect(column(SessionColumn.Rating).filter).toBe(false);
  });
  ...
  test('deployments offers a text filter but no sort', () => {
    expect(column(SessionsField.Deployments).sortable).toBe(false);
    expect(column(SessionsField.Deployments).filter).not.toBe(false);
    expect(column(SessionsField.Deployments).filterParams?.filterOptions).toEqual(
  ...
  test('topics offers a text filter but no sort', () => {
    expect(column(SessionsField.InsightTopics).sortable).toBe(false);
    expect(column(SessionsField.InsightTopics).filter).not.toBe(false);
    expect(column(SessionsField.InsightTopics).filterParams?.filterOptions).toEqual(
      baseStringFilter.filterParams?.filterOptions,
    );
  });

  test.each([[SessionsField.ChatId], [SessionsField.ProjectId], [SessionsField.UserHash]])(
  ...
  test.each([[SessionsField.TurnCount], [SessionsField.TotalTokens], [SessionsField.TotalPrice]])(
  ...
  test('activity sorts but offers no filter, because the period control owns that axis', () => {
    expect(column(SessionsField.LastRequestTime).sortable).not.toBe(false);
    expect(column(SessionsField.LastRequestTime).filter).toBe(false);
    expect(column(SessionsField.LastRequestTime).floatingFilter).toBe(false);
  });

  test('text filters offer no prefix or suffix matching', () => {
    const options = column(SessionsField.ChatId).filterParams?.filterOptions as string[];
  ...
  test('the default ordering is stated as a sort model on activity', () => {
    expect(column(SessionsField.LastRequestTime).sort).toBe('desc');
...
describe('sessions columns :: proportions', () => {
  test('the session column is the widest', () => {
    const flexes = columns().map((col) => col.flex ?? 0);

    expect(column(SessionsField.ChatId).flex).toBe(Math.max(...flexes));
  });

  test('the session column reserves room for a production-length id', () => {
    expect(column(SessionsField.ChatId).minWidth).toBeGreaterThanOrEqual(280);
  });

  test('the user column is sized for a hash rather than a display name', () => {
    expect(column(SessionsField.UserHash).minWidth).toBeGreaterThanOrEqual(140);
    expect(column(SessionsField.UserHash).flex).toBeLessThan(column(SessionsField.ProjectId).flex as number);
  });

  test('numeric columns are narrower than the session column', () => {
    [SessionsField.TurnCount, SessionsField.TotalTokens, SessionsField.TotalPrice].forEach((fieldName) => {
      expect(column(fieldName).flex).toBeLessThan(column(SessionsField.ChatId).flex as number);
    });
  });
});

describe('sessions columns :: value formatting', () => {
  test('token counts are compacted rather than delimited', () => {
    expect(format(SessionsField.TotalTokens, 1284507)).toBe('1.3 M');
    expect(format(SessionsField.TotalTokens, 7200)).toBe('7.2 K');
  });

  test('cost renders as currency', () => {
    expect(format(SessionsField.TotalPrice, '0.09')).toBe('$0.09');
  });
  ...
  test('a full-scale decimal cost is rounded to significant digits', () => {
    expect(format(SessionsField.TotalPrice, '0.090000000001')).toBe('$0.09');
    expect(format(SessionsField.TotalPrice, '0.003612544180')).toBe('$0.0036');
  });

  test.each([[SessionsField.TotalTokens], [SessionsField.TotalPrice], [SessionsField.TurnCount]])(
  ...
  test('activity renders through a cell renderer rather than a value formatter', () => {
    expect(column(SessionsField.LastRequestTime).valueFormatter).toBeUndefined();
    expect(typeof column(SessionsField.LastRequestTime).cellRenderer).toBe('function');
  });

  test('project renders through a cell renderer so an unattributed project is marked', () => {
    expect(typeof column(SessionsField.ProjectId).cellRenderer).toBe('function');
  });

  test('user renders through a cell renderer so a missing hash is marked', () => {
    expect(typeof column(SessionsField.UserHash).cellRenderer).toBe('function');
    expect(column(SessionsField.UserHash).valueFormatter).toBeUndefined();
  });
  ...
  test('deployments renders the recorded array through a cell renderer, unnarrowed', () => {
    const deploymentsColumn = column(SessionsField.Deployments);
  ...
  test('topics renders through a cell renderer and keeps its whole list in the tooltip', () => {
    const topicsColumn = column(SessionsField.InsightTopics);
  ...
  test('a session the evaluation has not reached carries no topics tooltip', () => {
    expect(column(SessionsField.InsightTopics).tooltipValueGetter?.({ data: {} } as never)).toBeNull();
  });
});
...
describe('sessions columns :: a stored choice from the smaller set', () => {
  const STORED_STATE: ColumnState[] = CURATED.map((colId) => ({
    colId,
    hide: !(
      colId === SessionsField.ChatId ||
      colId === SessionsField.ProjectId ||
      colId === SessionsField.UserHash ||
      colId === SessionsField.LastRequestTime ||
      colId === SessionsField.TotalPrice ||
      colId === SessionColumn.Rating
    ),
  }));
