...
- [ ] 1.1 Add a leaf-descriptor model (`field`, `headerName`, `hide`, `groupId`) and a pure derivation from
...
- [ ] 1.2 Add the pure inverse: write a visibility change or a reorder back into the definitions tree at the
      leaf's location, returning new definitions and leaving other groups untouched.
- [ ] 1.3 Change `src/components/Grid/ColumnsPanel/ColumnsPanel.tsx` to render leaf descriptors, keeping the
...
- [ ] 1.4 In `src/components/Grid/GridView/GridView.tsx`, derive the descriptor list from `columnDefs` and
...
- [ ] 2.1 Clamp `onMoveColumn` to the dragged leaf's own group: resolve the target index's group and return
      unchanged when it differs, so no partial move is applied.
- [ ] 2.2 On a toggle to hidden, clear that column's sort and its filter-model entry through the grid API
...
- [ ] 3.1 In `src/components/Grid/utils.ts`, add grouped counterparts to `applyColumnStateOrderToColDefs`
      ...
      exists.
- [ ] 3.2 Add a grouped variant of `checkColDefsChanges` in
...
- [ ] 3.3 Teach `getDefaultSorts` in `src/components/Grid/AgGridWrapper.tsx` to read `sort` from leaf columns,
      so a grouped grid's default sort is applied.
- [ ] 3.4 Have `GridView` select the flat or grouped helper based on the shape of `columnDefs`, in one place.

## 4. Shared-grid tests

- [ ] 4.1 Add specs for the descriptor derivation and its inverse in `src/components/Grid/tests/`: flat input
      round-trips unchanged, grouped input yields leaves with their `groupId`, and a write-back leaves other
      groups untouched.
- [ ] 4.2 Add specs for the grouped state helpers and the grouped comparator, including a stored `colId` that
      no longer exists.
- [ ] 4.3 Add `GridView` component specs over both shapes: toggling visibility, reordering within a group, a
...
- [ ] 4.4 Run the existing specs of the grids already using the panel (Usage Log, Evaluation list, Containers,
...
- [ ] 5.1 Fetch the `conversations` entity schema for the view: call the existing `getEntitySchema` action from
...
- [ ] 5.2 Add a pure catalog builder in `src/utils/analytics/` taking the schema fields and the curated column
      ...
      `last_request_time`).
- [ ] 5.3 Map `AnalyticsFieldType` to rendering and querying: the existing `ConversationFieldFormat`
...
- [ ] 5.4 Attribute every catalog column to the `conversations` provenance group, and keep the Rating column
      outside the catalog entirely.

## 6. Projection follows visibility

- [ ] 6.1 Pass the visible schema-driven fields into `buildConversationListQuery`
      (`src/utils/analytics/conversations-queries.ts`) and union them with the curated fields in the select.
- [ ] 6.2 In `src/components/Analytics/ConversationsTrace/use-conversations.ts`, restart paging when a column
      becomes visible and do not re-query when one is hidden.
- [ ] 6.3 Confirm the totals query is untouched by visibility — it aggregates over the filtered result, not
      over the projection.
- [ ] 6.4 Widen the list row model in `src/models/analytics/conversations-trace.ts` so a row can carry
      schema-driven fields alongside the curated ones.

## 7. Conversations opt-in

- [ ] 7.1 In `src/components/Analytics/ConversationsTrace/List/ConversationsList.tsx`, add the Columns button
...
- [ ] 7.2 Give every column an explicit width now that `autoSizeStrategy: fitGridWidth` is inactive, and check
      the seven default columns fill the container at the app's supported widths.
- [ ] 7.3 Gate the datasource assignment on the persisted state having been applied, so the first page request
      carries a restored sort and restored predicates.

## 8. Analytics tests

- [ ] 8.1 Add a spec for the catalog builder in `src/utils/analytics/tests/`: each exclusion rule, the
      type-to-format mapping, the curated columns kept as-is, and a schema reporting an unknown type.
- [ ] 8.2 Extend `src/utils/analytics/tests/conversations-queries.spec.ts` for the projection following the
      visible fields, and that a hidden field is absent from the select.
- [ ] 8.3 Extend `src/components/Analytics/ConversationsTrace/tests/ConversationsList.spec.tsx` and
      ...
      state.
- [ ] 8.4 Extend `src/app/[lang]/conversations-trace/tests/actions.spec.ts` and the page spec for the schema
