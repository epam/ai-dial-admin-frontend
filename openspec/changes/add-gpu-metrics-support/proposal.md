...
- Convert the GPU Memory card from a plain summed-bytes card to a gauge showing used/total (e.g. "14.3 / 40 GB") with the same warn/crit threshold convention as KV-cache usage.
- Add a new GPU Utilization card (gauge, 0–1 ratio averaged across pods) — `gpuUtilization` is not rendered anywhere today.
...
- Extend the shared Gauge card to optionally show a formatted detail label (used/total) instead of a bare percentage, so the mechanism generalizes beyond KV-cache's percentage-only case.
...
- `src/components/Common/MetricCard/GaugeCard.tsx` — optional detail-label override.
