...
The Total tokens single-value chart on the Dashboard's `ChartsDashboard` section SHALL compute its displayed value by fetching `ENTITY_CONSUMPTION_TREE_QUERY`, transforming the response rows via `buildEntitiesConsumptionTree`, and summing `prompts + completions` of every top-level `TreeRow` returned. The chart SHALL NOT sum every row's tokens. The sum SHALL use `Big` arithmetic and return a `number` for display, matching the precision convention of `getSingleValueChartData`.

#### Scenario: Single root row contributes its own tokens

- **GIVEN** the response contains one root row with `prompts: '100'` and `completions: '50'`
- **WHEN** the chart renders
- **THEN** it SHALL display `150`

#### Scenario: Orchestrator + child chain counts root only

- **GIVEN** the response contains:
  `{ name: 'A', parent_deployment: '', execution_path: 'A', prompts: '100', completions: '50' }`,
  `{ name: 'B', parent_deployment: 'A', execution_path: 'A/B', prompts: '80', completions: '40' }`
- **WHEN** the chart renders
...
- **GIVEN** the response contains
  ...
  with no row for `'A'`
- **WHEN** the chart renders
- **THEN** `buildEntitiesConsumptionTree` SHALL inject a synthetic root `A` whose `prompts` and `completions` equal `B`'s (direct-children rollup as defined by [[entities-consumption-tree]])
- **AND** the chart SHALL display `120`

#### Scenario: Multiple roots are summed

- **GIVEN** the response contains two real roots
  `{ name: 'A', parent_deployment: '', execution_path: 'A', prompts: '100', completions: '50' }`,
  `{ name: 'X', parent_deployment: '', execution_path: 'X', prompts: '30', completions: '20' }`,
  plus a child of A `{ name: 'B', parent_deployment: 'A', execution_path: 'A/B', prompts: '999', completions: '999' }`
- **WHEN** the chart renders
...
- **WHEN** the chart renders
- **THEN** it SHALL display `0`, consistent with no-data behavior on the other cards

#### Scenario: Large token totals retain precision

- **GIVEN** the sum of root tokens approaches or exceeds `Number.MAX_SAFE_INTEGER`
- **WHEN** the chart renders
- **THEN** the reducer SHALL accumulate via `Big` and return a `number` representing the precise sum (within JS `Number` range), without intermediate float drift
