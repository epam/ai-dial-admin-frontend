...
- [x] 1.1 Add `DIAL_CORE_API_URL` to `.env.template` and document in `README.md`. **Correction on archive:** the originally-planned `PUBLICATIONS_USE_CORE` flag was not shipped — the merged code is a hard, unconditional cutover to `CorePublicationsApi` with no flag and no surviving BE-backed `PublicationsApi` class. Confirmed via direct repo inspection when archiving this change: `PUBLICATIONS_USE_CORE` has zero occurrences in the codebase, and `src/app/api/api.ts` instantiates `CorePublicationsApi` unconditionally. Tasks below that reference the flag (4.8, 6.1) describe the original plan, not what actually shipped.
...
- [x] 4.8 ~~Gate on `PUBLICATIONS_USE_CORE` in `api.ts`~~ — **not shipped as planned**: `api.ts` instantiates `CorePublicationsApi` unconditionally; there is no flag and no surviving BE-backed `PublicationsApi` to gate against (see the correction on task 1.1)
...
- [ ] 5.1 (runtime gate — needs a live Core) With the flag on, exercise list/get/approve/reject/delete/update for all 5 types against a real Core (non-prod) and diff the resulting `Publication` against the BE path — left unchecked; no live Core instance available in this environment to run it retroactively at archive time, and the code has been merged/running since PR #3776
- [ ] 5.2 (runtime gate — R3) Trigger a Core 400/404/403/409 and confirm `ServerActionResponse.errorMessage/errorHeader` match the BE path — same caveat as 5.1
- [ ] 5.3 (runtime gate — R4) Confirm `assetsApi` (with `DEFAULT_ETAG`) accepts the FE-style paths from `path.ts` and returns the expected merged shape for every type — same caveat as 5.1
