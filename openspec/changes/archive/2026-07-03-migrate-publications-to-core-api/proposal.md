...
Introduce the **first direct FE→DIAL-Core client** and re-implement the publication server logic in TypeScript. **As shipped: a hard, unconditional cutover** (see design D2) — no feature flag, no BE fallback; the BE-backed `PublicationsApi` is removed outright. (An earlier draft of this proposal considered a flag-gated rollout; D2 superseded that during design and is what actually merged — corrected here at archive time.)
