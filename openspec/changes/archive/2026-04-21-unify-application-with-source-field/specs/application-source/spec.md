...
The Applications source dropdown MUST offer exactly two options in this scope:

- `SOURCE_TYPE.ENDPOINTS` — renders `ApplicationEndpoint` (chat + MCP inputs) in the panel below.
- `SOURCE_TYPE.SCHEMA` — renders `AppRunners` (runner picker) in the panel below.

The Applications source dropdown MUST NOT offer `CONTAINER`, `RUNNER`, `ADAPTER`, or `MCP_REGISTRY` in this scope (those are reserved for other entities or deferred to a follow-up change).

#### Scenario: Dropdown selection in Applications view

- **WHEN** a user opens a `DialApplication` and views the source section
- **THEN** a dropdown labeled with options mapping to `SOURCE_TYPE.ENDPOINTS` and `SOURCE_TYPE.SCHEMA` is displayed
- **AND** selecting `ENDPOINTS` renders `ApplicationEndpoint`
