...
When `isNameChanged` is `true`, the primary fork button (rendered inside `ChangedEntityButtons` children) SHALL display the label `t(ButtonsI18nKey.SaveAsNewImage)`. When `isNameChanged` is `false`, it SHALL display `t(ButtonsI18nKey.SaveAsNewVersion)` (today's behavior). The `onClick` handler is unchanged in both cases — it opens the same modal type (`ModalType.saveNewVersion`).
...
- **THEN** the primary fork button reads "Save as new version" (current behavior)

### Requirement: Conditional modal title

When the user opens the version modal from the rename context (`isNameChanged === true`), `AddVersionModal` SHALL receive `header={t(ImagesI18nKey.SaveNewImageModalTitle)}`. Otherwise it SHALL receive `header={t(ImagesI18nKey.SaveNewVersionModalTitle)}` (today's behavior). The `submitLabel` (`t(ButtonsI18nKey.Save)`) remains unchanged.
...
- **THEN** the modal title reads "Save new version" (current behavior)
...
- **THEN** the modal opens with the version field pre-filled to `1.2.1` (current behavior)
...
- **THEN** `verifyVersion` settles by computing `getSemanticVersionError`, sets `versionError` for inline UI, and dispatches `field: 'version', isValid: false` so the popup's `Create` button is disabled (current behavior preserved)
...
- **THEN** the modal's own `validateVersion` dispatches `field: 'version', isValid: false`, the aggregate `isValid` is false, and the `Create` button is disabled (existing behavior, unchanged)
...
| File | Change |
|---|---|
| `apps/ai-dial-admin/src/locales/en.ts` | Add `Buttons.SaveAsNewImage` and `Images.SaveNewImageModalTitle` entries |
| `apps/ai-dial-admin/src/constants/i18n.ts` | Add matching enum entries on `ButtonsI18nKey` and `ImagesI18nKey` |
| `apps/ai-dial-admin/src/components/Assets/Modals/AddVersionModal.tsx` | Add optional `defaultVersion?: string` prop; use it as the `useState` initializer when provided |
| `apps/ai-dial-admin/src/components/Assets/Modals/tests/AddVersionModal.spec.tsx` | Add test cases: `defaultVersion` provided overrides patch-bump; `defaultVersion` omitted preserves patch-bump |
| `apps/ai-dial-admin/src/components/EntityHeaderControls/Wrappers/ImagesButtonsWrapper.tsx` | Lift `isNameChanged` to a `useMemo`; conditionally select button label and modal `header`; pass `defaultVersion='1.0.0'` only when `isNameChanged` and the typed name has no known versions |
| `apps/ai-dial-admin/src/components/EntityHeaderControls/Wrappers/tests/ImagesButtonsWrapper.spec.tsx` | Extend the existing component tests with rename scenarios: button reads "Save as new image" when name changed; modal title and default version follow the same conditional |
| `apps/ai-dial-admin/src/components/Images/Fields/ImageFields.tsx` | Branch `verifyVersion` settle on `isModal`: when `isModal=true`, compute `getSemanticVersionError`, set `versionError`, dispatch `field: 'version', isValid: !error`; when `isModal=false`, dispatch `field: 'version', isValid: true` (clear pessimistic race guard) and skip inline error |
