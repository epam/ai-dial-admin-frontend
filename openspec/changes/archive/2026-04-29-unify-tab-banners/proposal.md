...
`ContainerStatusBanner` currently renders outside the tab-content scroll area on Models / Applications / Interceptors / Toolsets detail views. It eats permanent vertical space on every tab and never scrolls away even when the user is deep in another tab's content. Meanwhile, the image-not-installed banner on Container detail already has the desired behavior — it lives inside the Properties tab, above `EntityInfoHeader`, and scrolls with the tab content — but it is implemented as an inline `DialAlert` with custom CSS overrides, duplicating the visual pattern of `ContainerStatusBanner`. This change unifies the two banners onto one configurable component and relocates `ContainerStatusBanner` to match the image-banner placement so both behave the same way: visible only on the Properties tab, above `EntityInfoHeader`, scrolling with the tab.

## What Changes

- **New component**: `EntityBanner` at `components/Deployments/Common/EntityBanner/EntityBanner.tsx`. A thin configurable wrapper around `DialAlert` accepting `variant`, optional `title`, `message`, `className`, and `children` (CTA slot). It is the only place that constructs the `DialAlert` markup used by deployment-related warning banners.
...
- **Refactor `ContainerStatusBanner`**: keep current data-fetch + trigger logic, replace the local `DialAlert` markup with `<EntityBanner>`.
...
- **Replace inline image-not-installed banner**: in `Containers/View/TabsContent.tsx` Properties branch, drop the inline `DialAlert` (and its custom `[&>div]` CSS overrides) in favor of `<ImageStatusBanner image={image} />` in the same position.
...
- **Components modified**:
  ...
  - `Containers/View/TabsContent.tsx` — replace inline `DialAlert` with `<ImageStatusBanner>`.
