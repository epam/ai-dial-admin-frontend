...
- **AND** SHALL NOT render `DialAlert` directly
...
- `variant?: AlertVariant` — defaults to `AlertVariant.Warning` when omitted.
...
- `className?: string` — forwarded to the underlying `DialAlert` for caller-controlled spacing.
- `children?: ReactNode` — optional CTA slot (typically a `DialNeutralButton`) passed through as `DialAlert` children.

The component SHALL render `DialAlert` with `variant={variant ?? AlertVariant.Warning}`, `className={className}`, a `message` prop containing a `<span className="small">` wrapping the optional bold title (`<span className="small-text-semi">{title}</span>` when title is truthy) followed by `{message}`, and `children` passed through unchanged.

#### Scenario: Default variant is Warning

- **WHEN** `<EntityBanner message="hello" />` is rendered without `variant`
- **THEN** the underlying `DialAlert` SHALL receive `variant={AlertVariant.Warning}`
...
#### Scenario: CTA children are rendered inside DialAlert

- **WHEN** `<EntityBanner message="..."><DialNeutralButton label="Go" /></EntityBanner>` is rendered
- **THEN** the `DialNeutralButton` SHALL appear as a child of the underlying `DialAlert`

#### Scenario: className is forwarded to DialAlert

- **WHEN** `<EntityBanner message="..." className="mb-4" />` is rendered
- **THEN** the underlying `DialAlert` SHALL receive `className="mb-4"`

#### Scenario: Custom variant is forwarded

- **WHEN** `<EntityBanner variant={AlertVariant.Error} message="..." />` is rendered
- **THEN** the underlying `DialAlert` SHALL receive `variant={AlertVariant.Error}`
