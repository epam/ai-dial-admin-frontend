...
### Requirement: Folder listing merges all five resource types and validates consistency
The system SHALL build a folder's listing by querying all five resource types (application-resource, toolset-resource, conversation, prompt, file), merging their results into one tree, and SHALL reject the merge with an error if the types disagree on a folder's name, parent path, bucket, or path.
...
The system SHALL move a folder by first confirming it exists in every targeted resource type (or all five, if none are specified), then copying its rules to the destination path, then moving each type's resources one at a time — stopping at the first per-type failure without rolling back types already moved.
