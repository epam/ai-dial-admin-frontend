...
### Requirement: Deployments view row action menu shows Open-in-new-tab; clicks are no-ops

The per-row action menu in the Deployments view SHALL render exactly one item: `Open in new tab`. The item is the visible affordance that a detail experience is forthcoming, but in this change clicking it SHALL NOT navigate — the handler SHALL short-circuit. Row-body clicks (anywhere outside the action menu) SHALL also be no-ops. The audit detail route at `/activity-audit/{activityId}` SHALL remain admin-backend only in this change. A dedicated detail experience for deployment activities is deferred to a follow-up change, which will wire up both the menu action and the row click.
...
#### Scenario: Open-in-new-tab click is a no-op
- **WHEN** the user clicks `Open in new tab` on a Deployments view row
- **THEN** no new browser tab opens
- **AND** the page does not navigate
- **AND** no audit detail page request is issued

#### Scenario: Row-body click is a no-op
- **WHEN** the user clicks anywhere on a Deployments row outside the action menu
- **THEN** the page does not navigate
- **AND** no audit detail page request is issued

#### Scenario: Audit detail route stays admin-only
- **WHEN** the user opens `/activity-audit/{activityId}` for an activity owned by the admin backend
- **THEN** the existing detail page renders the full diff view as it does today
- **AND** the detail page does not fall back to the deployment-manager backend
