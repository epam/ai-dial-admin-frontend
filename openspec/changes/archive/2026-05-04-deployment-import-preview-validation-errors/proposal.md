...
- **Per-tab banner**: new `ValidationBanner` renders a red `DialAlert` *inside* the active tab's content area (between `DialTabs` and the grid) when `validationSummary.errorsByTab[selectedTab] > 0`. Message is composed of two i18n keys — `ValidationBannerHeading` (semi-bold, with `{count}`) followed by `ValidationBannerHelp` (regular).
