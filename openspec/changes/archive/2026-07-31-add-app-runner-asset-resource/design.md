...
`AppRunners` and `SelectAppRunnersModal` are shared by both application surfaces (`SourceField.tsx` for entities, `ResourceSourceField.tsx` for assets), so the reduced columns and `$id` label initially reached `Entities > Applications` too — a cosmetic change to a surface this change's non-goals declared untouched. It is gated instead: both components take the `view` already in scope at each call site and switch presentation only for `AssetsApplications`. Gating on `view` rather than on "does this list contain an asset runner" keeps the surface's appearance stable on an install that happens to have no asset runners yet, and `view` is what the repo already threads through the sibling source-field components.
