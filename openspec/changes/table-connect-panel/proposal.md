...
- **The panel states the authentication contract**: how to pass a DIAL API key (`Api-Key` header, sourced from `ADAS_API_KEY` so it never lands in the script), and, on the Write tab, **this table's live `write` roles**, read from `GET /v1/tables/{name}/access` — with a **Manage access** shortcut. Those role names are the only ones it renders; the analytics backend's internal application roles are neither readable by this app nor attachable to a key, so the panel refers to administrator access descriptively instead of naming a constant it cannot verify.
