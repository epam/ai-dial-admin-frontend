...
- [x] 1.1 Add `TestSuiteAdditionalRequest` in `src/models/evaluation/test-suite.ts`: `name?: string`,
...
- [x] 1.2 Add `requestIndex?: number` and `totalRequests?: number` to `ResultDto` in
  `src/models/evaluation/run.ts`, alongside the existing `turnIndex`/`totalTurns`.
- [x] 1.3 Add `src/utils/evaluation/request-chain.ts`: `getRequestCount(testSuite): number`,
...
- [x] 2.1 In `src/utils/evaluation/test-suite-payload.ts` (`normalizeTestSuitePayload`), normalize every
...
- [x] 2.2 Extend `TestSuitesI18nKey.ConditionHint` in `src/locales/en.ts` to document the `request.*`
...
- [x] 3.1 Add `RequestChainSelector` (location decided per design.md's open question — likely
...
- [x] 3.2 Wire `RequestChainSelector` into `src/components/TestSuites/View/MethodTabContent.tsx`'s
...
- [x] 3.3 Add the info banner, shown iff `selectedIndex > 0`, naming the response columns of every
...
- [x] 3.4 Confirm `TryOutButton`/`TryOut.tsx` are left unwired to the proxy view (they must keep reading
  ...
  - **Verification:** `npx vitest run src/components/TestSuites/RequestTemplate/tests/RequestChainSelector.spec.tsx src/components/TestSuites/View/tests/MethodTabContent.spec.tsx`.
    ...
    selected request.
- [x] 3.5 Offer the preceding requests' output columns as JSONata variables in both JSONata surfaces:
...
- [x] 4.1 In `src/components/TestSuites/TestCases/TestCasesList.tsx` (or a new sibling section
...
- [x] 5.1 In `src/components/TestSuites/Metrics/AddMetric/Values/Inputs.tsx`, replace the direct read of
...
- [x] 6.1 Append `requestIndex` (headerName `Request`, 1-based `valueGetter`) and `totalRequests`
...
- [x] 6.2 Extend `getCompareIdKey`/`getCompareNameKey` in the same file from
...
- [x] 6.3 Add a "Requests in chain: N" row to `src/components/Runs/Summary/Header.tsx`, `N = 1 +`
...
- [x] 6.4 Extend `sortCompareRows` in `src/components/Runs/View/utils.ts` to order by `requestIndex`
...
- [x] 6.5 Make the Heat Map tab request-aware in
...
- [x] 6.6 Add hidden-by-default `Request` and `Turn` column pairs to the compare grid's `EXECUTION` group
...
- [x] 7.1 Regression pass: a suite with an empty chain is pixel- and behavior-identical to today on the
...
- [x] 7.2 `src/utils/evaluation/tests/request-chain.spec.ts` (from task 1.3) is complete and covers every
  function's boundary conditions listed there.

## 8. Quality checks

- [x] 8.1 `npm run lint`, `npm run format`, `npm run test` (all from `apps/ai-dial-admin/` unless run via
