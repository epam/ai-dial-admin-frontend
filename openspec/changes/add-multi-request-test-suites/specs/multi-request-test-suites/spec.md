...
### Requirement: A tab strip selects, adds, renames, and removes chain entries

`RequestChainSelector` SHALL render, on the Method tab of a DEPLOYMENT suite only, one tab (ui-kit
`DialTabs`, so the strip matches the entity tab strip above it in size and styling) per chain entry,
labelled by `getRequestName`. Selecting a tab SHALL switch which chain entry the Request Template and
Endpoint Schema editors display and edit, via the proxy view.
...
request" affordance SHALL sit beside the strip, act on the currently selected request (via
`removeRequestAt`), and SHALL NOT be offered while request `#0` is selected — the chain always has at
least one request.

Each request's name SHALL be editable, writing to `requestName` for request `#0` or to the matching
...
#### Scenario: Adding a request appends a new tab

- **WHEN** "add request" is used on a suite with two requests
- **THEN** the chain has three requests, and a new tab for request `#3` (empty template) is shown
...
#### Scenario: Removing an additional request drops its tab

- **WHEN** the second tab of a three-request chain is selected and "remove request" is used
- **THEN** the chain has two requests, and the removed entry's tab is gone

#### Scenario: Request #0 cannot be removed

- **WHEN** request `#0` is the selected tab
- **THEN** no remove affordance is offered

#### Scenario: Renaming a request updates its tab label
...
equivalent remount trigger), so that selecting a different tab unmounts and remounts both editors rather
...
tab strip, and SHALL show no such banner when request `#0` is selected. When any request before the
...
request is currently selected in `RequestChainSelector`.

#### Scenario: Try Out is unaffected by the selected request

- **WHEN** a non-zero request is selected in the tab strip and Try Out is invoked
