...
- [ ] 3.5 Update/extend existing Analytics tests to verify: row click opens sidebar in default mode, re-click same row closes sidebar (toggle), mode switch closes sidebar and opens drawer, mode switch from drawer closes drawer and opens sidebar, close preserves mode preference, grid container height reduced by drawer currentHeight, clearPinIfMissing called on grid refresh, unmount cleans up both sidebar and drawer, focus moves to drawer toolbar on switchToDrawer; file: `src/components/Runs/View/tests/Analytics.spec.tsx` (extend existing or create)
...
- [ ] 4.3 Implement resize drag logic on the resize handle (mousedown/mousemove/mouseup on document, min 200px, max viewport - 100px); disable `transition: height` during active drag (re-enable on mouseup) to avoid lag; add keyboard resize support (Arrow Up/Down ±20px, Shift+Arrow ±100px) when handle is focused; handle must be focusable (`tabIndex={0}`, `role="separator"`, `aria-orientation="horizontal"`)
...
- [ ] 4.5 Implement Escape key handler to close the drawer when focus is within the drawer; check `event.defaultPrevented` first — if a child handler (FullscreenViewer modal, dropdown, AG Grid cell editor, etc.) already called `preventDefault()`, skip the drawer close; as fallback also check FullscreenViewer context if `defaultPrevented` is not set
