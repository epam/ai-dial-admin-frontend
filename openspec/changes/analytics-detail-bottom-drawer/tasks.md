...
- [ ] 2.1 Create `src/components/Runs/Details/BottomDrawer/useDetailMode.ts` — manages `detailMode` ('sidebar' | 'drawer'), **sole owner** of `sidebar.showSidebar()`/`sidebar.closeSidebar()` calls within Analytics.tsx (Analytics.tsx must NOT call sidebar APIs directly), exposes `switchToDrawer()`, `switchToSidebar(resultId)`, `openDetail(resultId)`, `closeDetail()`; preserves mode preference across close/reopen
...
- [ ] 4.5 Implement Escape key handler to close the drawer when focus is within the drawer and no inner overlay (tooltip, dropdown) is currently open
...
- [ ] 6.3 Implement Order tab: draggable section rows using existing `DraggableList`/`DraggableItem` components with scoped `DndProvider` + `HTML5Backend`, numbered positions, eye toggle for section visibility; add keyboard reorder via Arrow Up/Down on focused drag handle (calls `moveSectionByKeyboard`)
