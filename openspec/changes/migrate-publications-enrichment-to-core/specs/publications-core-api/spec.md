...
The system SHALL fetch a single publication via `POST /v1/ops/publication/get` (path prefixed with `publications/`). It SHALL treat a publication whose status is APPROVED or REJECTED as not found. For each resource it SHALL resolve the effective URL by action and status (ADD/ADD_IF_ABSENT → reviewUrl when PENDING, targetUrl when APPROVED, sourceUrl when REJECTED; DELETE → targetUrl), enrich the resource with its body **fetched directly from DIAL Core** (`GET /v1/{type}/{path}` merged with `GET /v1/metadata/{type}/{path}`), and collect "not found" / "target already exists" problems into `resourceIssues` instead of failing the whole request. Enrichment SHALL NOT call the admin BE.
...
- **THEN** each resource is resolved to its review/target/source URL per its action and enriched with its body fetched from DIAL Core, and the result carries the same enriched `Publication` shape consumers received from the admin BE path
...
For a PENDING publication resource whose action is not DELETE, the system SHALL check **against DIAL Core** (`GET /v1/metadata/{type}/{path}`) whether the resource already exists at its target path and SHALL record a "target already exists" issue when it does, matching the BE's per-type validation.
...
- **WHEN** a publication update persists its resource bodies
- **THEN** each body is written via `PUT /v1/{type}/{path}` to DIAL Core, not through the admin BE

## ADDED Requirements

### Requirement: Core-native resource content and metadata mapping
The system SHALL build each enriched publication resource by merging the Core content response (`GET /v1/{type}/{path}`) with the Core metadata response (`GET /v1/metadata/{type}/{path}`), parsing the prefix-stripped, URL-decoded, `__`-versioned path, and producing the same FE resource model the admin BE produced for that type (e.g. prompt content/description plus path/name/version/folderId/author/updatedAt). No field that the BE mapper populated SHALL be dropped.
...
### Requirement: Feature-flagged cutover with BE fallback
**Reason:** Publications are now fully backed by DIAL Core with no remaining admin-BE call, so the `PUBLICATIONS_USE_CORE` flag and its BE fallback are removed.
**Migration:** Delete `PUBLICATIONS_USE_CORE` from configuration; the Core path is unconditional. No consumer-visible change (the `Publication` contract is unchanged).
