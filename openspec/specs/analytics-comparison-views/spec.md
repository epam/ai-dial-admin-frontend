# Analytics Comparison Views — Spec

## Purpose

Defines the Table and Pivot renderers used inside the Analytics bottom drawer to display one or two
(pinned + active) test case results side by side. Covers section grouping, column/row headers, diff
highlighting between the pinned and active case, adaptive field-value rendering (scalars, JSON,
truncation), metric-value formatting, the Table-view focus strip for spotlighted fields, Pivot-view
sticky headers, the Table/Pivot crossfade transition, and memoization of the comparison data
transformation.
