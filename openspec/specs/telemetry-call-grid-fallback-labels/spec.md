# telemetry-call-grid-fallback-labels Specification

## Purpose

The Telemetry call dashboards include several grids whose key columns can carry a missing value: project-name grids (Consumption "Calls by Projects", MCP "Calls by Project", Route "Calls by Project") may have a missing `name`, and parent-deployment grids (MCP "Calls from Parent Deployments", Route "Calls from Parent Deployments") may have a missing `parent_deployment`. This capability defines the presentational fallback labels these grids render in those cases — `No Project` and `Direct call` — together with context-specific info-circle tooltips, so the literal text `undefined` is never shown. The fallback lives entirely in the column rendering layer, is localized via i18n, and is reflected in the grid's Copy action.
