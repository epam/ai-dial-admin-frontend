# Analytics Field Selector — Spec

## Purpose

Defines the left-hand field selector sidebar rendered inside the Analytics bottom drawer, offering a
"Fields" tab (search, per-section checkboxes with select-all/deselect-all, collapsible sections) and
an "Order" tab (drag-to-reorder sections via `DraggableItem`, per-section eye toggle to hide/show).
Field visibility and section order/hidden state drive what the Table and Pivot comparison views
render, persist only for the lifetime of the drawer session, and reset to defaults when the drawer is
closed or the user switches to sidebar mode.
