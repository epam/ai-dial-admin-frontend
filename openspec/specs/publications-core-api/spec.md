# publications-core-api Specification

## Purpose
Server-side publication operations executed directly against DIAL Core (with the admin BE's hidden transformations preserved: status-driven URL resolution, file staging, comment sanitization, target-recalculation), replacing the BE-backed client outright, while the FE-facing publication contract and UI stay identical — created by archiving change `migrate-publications-to-core-api` (shipped in PR #3776) and completed by archiving change `migrate-publications-enrichment-to-core`. This was a hard, unconditional cutover — no feature flag, no BE fallback (an earlier design draft considered one; superseded during design). Publications have **zero remaining admin-BE dependency**: per-resource enrichment on `get` and the per-resource re-PUT on `update` for the four versioned types (application, conversation, prompt, toolset) now run through the shared `core-asset-client`; file resources were already Core-native since the first change.
...
- **WHEN** a publication operation runs for an authenticated user
