# container-events-grid-scroll Specification

## Purpose

Defines the scroll, state-preservation, and live-feed behavior of the container Events tab grid under SSE-driven updates, and the supporting opt-in flags on `AgGridWrapper` (`isLiveData`, `getRowId`) that make this behavior possible without affecting other call sites.
