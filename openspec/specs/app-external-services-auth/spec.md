# app-external-services-auth Specification

## Purpose
The External Services section of App Properties: how a declared service is listed, added, edited and
deleted, which auth types are offered, how a user signs in to and out of an OAuth-backed service, and
how a `DIAL_NATIVE` service differs — it renders admin approval state rather than sign-in state, and
consent is the only mutation its row offers.
## Requirements
