# consumption-project-aggregation Specification

## Purpose

Two consumption views on the Dashboard route — the Entities Consumption tree and the Projects Consumption grid — used to read from two independent analytics queries (`ENTITY_CONSUMPTION_TREE_QUERY` grouped by `(deployment, parent_deployment, execution_path)`, and `PROJECT_CONSUMPTION_QUERY` grouped by `project_id`). This capability collapses both into a single source: one query that adds `project_id` as a fourth grouping dimension, plus two frontend folds that produce the rows each grid expects.

The capability owns:

- The query shape (its `project_id` dimension is co-owned with [[entities-consumption-tree]]).
- A `ConsumptionDashboard` proxy component that holds the single fetch, refresh polling, and aggregation state for the Dashboard route.
- Two pure aggregator helpers — `aggregateByDeployment` (collapses the `project_id` axis for the tree) and `aggregateByProject` (sums project-root tokens only, including orphan-child synthetic roots, for the projects grid).
- The presentational contract that lets `EntitiesConsumptionTree` and `TelemetryGrid` accept pre-aggregated rows as props and skip their own fetch.
