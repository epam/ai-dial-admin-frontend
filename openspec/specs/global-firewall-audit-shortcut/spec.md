# Global Firewall Audit Shortcut

## Purpose

Provides a one-click navigation from the Global Firewall popup (the `GlobalWhitelist` modal opened from the Images list) to the Activity Audit page, pre-scoped to the Deployments view with the Resource type column filtered to Global Firewall activities. Establishes a `sessionStorage`-backed preselect handoff (`audit-list-preselect` key, `AuditListPreselect.GlobalFirewall` enum value) consumed once on the audit list's first mount. Also fixes the long-standing Resource type column filter bug for single-match queries by translating the user's localized-label freetext input into an `eq` filter against the corresponding raw enum value.
