# model-serving-capability-gating

## Purpose

Gate the Model-Serving (container) model creation flow by the backend inference capability of each container, so users only create the artifact that matches a container's capability (text-generation models, text-classification toolsets), while leaving the External-Endpoint source unaffected.
