...
## Purpose
The `Assets > Skills` surface — a menu entry, metadata-only folder-tree list, and a detail view
(Save/Discard header; staged file add/preview/download/remove with `SKILL.md` protected from
removal; a destination-folder field that moves the skill on Save) — plus delete and bulk delete.
Skill is a folder-of-files resource (rooted at a `SKILL.md` manifest), scoped to what Core's
metadata-only listing endpoint actually returns: no per-row content fetch, no bundle-authoring UI,
and no editing of the skill's own parsed metadata (name/description/version), which lives only in
`SKILL.md`'s frontmatter — created by archiving change `add-assets-skills`.
...
(name/description/version) is out of scope for this surface — since it lives only in `SKILL.md`'s
frontmatter, not parsed or shown here. The file listing and destination folder are editable per the
requirements below.
