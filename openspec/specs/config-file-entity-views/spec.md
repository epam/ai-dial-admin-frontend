...
persistence, the list component swap, the lazy data fetch, the `configFile=true` detail page
rendering, and the breadcrumb fix that ensures the "back" link is valid without the admin API.
Created by archiving change `add-config-file-entity-views`.
