...
Rules for composing `source.completionEndpointPath` on Models based on the source container's `$type`. NIM-sourced Models use the `v1` prefix; all other container types (and unresolved ids) use `openai/v1`. The rule applies to template creation and explicit source-container selection only, never to opening existing Models for edit.
