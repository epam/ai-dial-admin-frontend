# core-config-file-client Specification

## Purpose
A read-only server-side client for DIAL Core's config-file entity surface (`GET /v1/admin/config/file/{type}[/{name}]`), and the union it composes with Core's metadata route — created by archiving change `unlink-app-runner-assets-from-admin-be`. Core keeps each config entity type in two populations and validates a reference against their union, so offering an option from only one of them cannot express every configuration Core accepts. Covers the readable-type allow-list, the distinction between a refused read and an empty population, the normalisation both populations share, and the per-origin reference forms a selection must be stored as. Deliberately read-only: config-file entities are owned by whoever deploys the configuration.
