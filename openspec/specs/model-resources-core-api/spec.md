...
Model-resource list/get/create/update/delete against DIAL Core's `ConfigResourceController`-backed `models/platform` routes — flat, unversioned, etag sourced from metadata, with the write-payload and upstream-secret rules Core's contract requires.
