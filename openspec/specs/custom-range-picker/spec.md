# custom-range-picker Specification

## Purpose
Cap-aware inline calendar range picker with FSM-driven selection, configurable via the backend
telemetry dataset retention. Covers how a range is selected and committed, how the maximum span is
sourced and enforced, the `TimeFilter` layout that hosts the picker, and the shared `useTimeFilter`
hook every caller reads it through.
## Requirements
