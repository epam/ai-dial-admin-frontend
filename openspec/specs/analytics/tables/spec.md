...
- **AND** a description too long for the line offers Show more beside it, which reveals the rest in place
...
#### Scenario: A column's description is read without hovering

- **WHEN** the columns grid renders a column whose description exceeds its line
- **THEN** the description is held to that line with a Show more control beside it, rather than truncated
  into a tooltip
- **AND** activating it reveals the rest of the description
