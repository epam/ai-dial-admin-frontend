# Analytics Sessions Listing

## Purpose

The sessions log page: its route and guard, the filters and the second query that resolves the feedback filter, the provenance line, and the grid's columns, server-side ordering and per-column filtering.

## Requirements

### Requirement: Sessions page route, access guard, and server prefetch

The system SHALL expose an Analytics page at `/sessions`, present in the `ApplicationRoute`
enum (`types/routes.ts`) as `SessionsTrace`. The route directory SHALL be
`src/app/[lang]/sessions/`. The page SHALL be a server component declaring
`export const dynamic = 'force-dynamic'`, SHALL call `isAnalyticsForbidden()` before any data access and
render `Page403` when it returns `true`. Code identifiers SHALL use `sessions-trace` / `SessionsTrace`
outside the route directory, which the URL names `sessions`; every user-facing string SHALL read
"Sessions". The route MUST NOT collide with
...
against. Where it is absent, the access guard still passes and the session query fails with HTTP 400; the
...
- **AND** no session query is issued

#### Scenario: Route does not shadow the DIAL Core sessions route

- **WHEN** the browser is at `/sessions`
- **THEN** the Analytics "Sessions" menu item is the active item
- **AND** the existing `/conversations` menu item is not marked active

#### Scenario: A missing sessions entity reads as a failure
...
- **AND** it does not report that the period held no sessions
...
The sessions page SHALL provide a feedback filter with exactly four mutually exclusive states — all,
...
The two ids join without translation: the analytics service sets a session's id to the session's
...
incomplete and that the sessions shown are the most recently rated ones. The cap truncates the result
regardless of how it is ordered, and the ordering is the operator's to choose, so a truncated result MUST NOT
be presented as the complete set of sessions carrying that feedback. The disclosure SHALL be visible
...
When the candidate query returns no ids the page SHALL return no rows **without** issuing the session
...
per instance exactly as the session and turn rollups are, and the page cannot render without those
...
#### Scenario: No session carries the feedback
...
- **THEN** its predicate matches a session whose only rating was normalized to zero, alongside one
  carrying an unambiguously negative rating
- **AND** it selects the same sessions the previous non-positive predicate selected

#### Scenario: Rated covers both thumbs

- **WHEN** the rated state is selected
- **THEN** its predicate matches every session the two thumb states match
- **AND** it does not match a session whose only rate event carried no rating value
...
The header SHALL show summary pills for the session count, the rated count, the count carrying negative
...
The rated pill SHALL state the number of sessions carrying at least one rating in the period, as a bare
count. It MUST NOT be rendered as a ratio over the session count: the two are bounded by different clocks
— ratings by when they were submitted, sessions by when they were last active — so a session rated
...
the rated and negative pills unavailable without disturbing the session count or the cost, since they are
...
- **WHEN** the page renders for an instance whose sessions schema reports two enrichment namespaces
...
- **WHEN** the result holds more sessions than one page and only the first page is loaded
...
#### Scenario: The session count is exact regardless of how much is loaded

- **WHEN** the result holds more sessions than one page and only the first page is loaded
- **THEN** the session count shows the period's total
...
- **AND** no pill states that it covers only the sessions loaded so far

#### Scenario: The rated pill is a count, not a ratio

- **WHEN** the rated pill renders
- **THEN** it shows the number of sessions rated in the period and no denominator
- **AND** it is never rendered as a fraction of the session count
...
- **WHEN** the rating aggregate fails but the session count and cost resolved
- **THEN** the rated and negative pills report unavailability
- **AND** the session count and cost pills show their resolved figures
...
### Requirement: Session filters re-query the backend

The sessions page SHALL provide a free-text search box, a time-period control and a feedback filter, and
...
A failed sessions request SHALL be reported to the operator rather than rendered as an absence of
data. An emptied grid alone is indistinguishable from a period that genuinely held no sessions, so a
...
- **AND** the empty grid reports the failure instead of "No sessions"
...
The grid SHALL show a Rating column giving each session's positive and negative rating counts, attributed
in the provenance band to `response_ratings` rather than to `sessions`.

A session with no rating SHALL render the same unavailable placeholder as a session nobody rated. An
...
MUST NOT be done: that set is capped, so a displayed session could fall outside it and be reported as
...
a session the Positive filter selected cannot then display a zero positive count, and the same holds for
the negative side.
...
provably a thumbs-down, and how much of the session's feedback had its submitted form captured at all —
...
established. The session rows themselves SHALL still be returned.
...
cell is a two-direction figure, a third signal in it is a design question of its own, and the session's
...
- **WHEN** a page of sessions is returned
...
- **WHEN** a session has positive ratings and no negative ones
- **THEN** the cell shows the positive count coloured and a muted zero for the negative side

#### Scenario: A session rated both ways shows both counts

- **WHEN** a session carries one like and one dislike
...
- **WHEN** a session's only rating was submitted as a boolean false and normalized to zero
...
- **WHEN** part of a session's negative count comes from events whose submitted form was never captured
...
- **WHEN** a session's negative figure includes a rating submitted as a numeric zero whose form was
  captured
...
- **WHEN** every event behind a session's negative count had its submitted form captured
- **THEN** the negative side carries no caveat

#### Scenario: An unrated session is muted, not blank

- **WHEN** a session has no ratings in the period
...
- **THEN** its Rating cell shows the same unavailable placeholder an unrated session shows
...
- **THEN** the session rows are still returned
- **AND** their rating cells render nothing, asserting no absence of feedback

### Requirement: Session cells render composed values, not raw aggregates

The grid SHALL render composed cells rather than one raw stored value per column:

- The session column SHALL be a single **identity** cell stacking the session's title over its id,
  rather than two columns. The title labels the session and the id addresses it; they are one identity,
  and a column apiece printed the id twice on every session the enrichment has not reached, which is
...
- The activity column SHALL stack how long ago the session was last active over how long it ran. The span
...
- The project column SHALL show the project alone. It MUST NOT show a model chip: the session rollup does
  not carry `deployment`, so the page has no model to attribute to a session and MUST NOT infer one. A
  session with no project SHALL render an explicit placeholder rather than an empty cell, because an
...
- **WHEN** a row carries both a session id and an insight title
- **THEN** one cell states the title above the id
- **AND** no separate title column exists

#### Scenario: An untitled session shows the marker, not a repeated id
...
#### Scenario: A session with no project is marked, not blank
...
### Requirement: Session row values tolerate either backend wire shape

The `SessionRow` model in `src/models/analytics/sessions.ts` SHALL type its timestamp and
...
### Requirement: Session list query over the sessions entity

The system SHALL provide
`buildSessionListQuery({ range, search, chatIds, sort, columnFilters, visibleFields, offset })` in
`src/utils/analytics/sessions-queries.ts` returning a `StructuredQuery` over the entity `sessions`
...
- a **cheap field of the entity's own source** SHALL be named whether or not its column is visible. Measured
  on a rollup of 6 328 sessions, twenty such columns instead of two read 2.08 MiB instead of 492 KiB and
...
`turn_count` is the pipeline's count of the session's **distinct trace ids**, one trace per request, so
...
SHALL apply to `last_request_time`, so a selected period means *sessions whose last activity falls in the
...
Search SHALL NOT reach the session title either: the title is an enrichment column, absent for any
session the evaluator has not processed, so a term matched against it would silently narrow the result to
enriched sessions only.
...
#### Scenario: Query reads the sessions entity in row mode

- **WHEN** `buildSessionListQuery` is called with a time range
...
- **THEN** the whole-result session count and cost do not change

### Requirement: Server-side paging with an exact result total

The sessions page SHALL fetch its rows one page at a time from the backend. The result total SHALL be
the session count the result summary resolves, and the list query SHALL NOT request a total of its own.
...
which sessions are in the result, only which slice of it is delivered.
...
prefix of the new one either, even though the set of sessions is unchanged. A change to any column filter
SHALL restart paging as a filter change.

Ratings SHALL be resolved for each page as it arrives, restricted to the sessions that page contains.
...
- **WHEN** a page of sessions arrives
- **THEN** the rating counts are resolved for exactly the sessions on that page

### Requirement: Sessions grid with server-side ordering and per-column filtering

The sessions view SHALL render a grid of five visible columns — session, project, user, activity,
cost — plus the Rating column. Turns, tokens, deployments and topics are curated columns that default to
hidden; see "Session grid columns are a fixed curated set gated by the entity schema" for the whole set
and its origins.
...
| Column | Sort | Filter |
|---|---|---|
| session (`client_session_id`) | yes | text |
...
dropped. A reader SHALL NOT be led to believe such a filter searched every session: it matches only rows
...
the feedback narrowing and every column filter SHALL all hold for a returned session. The page's filter
...
session count and cost SHALL be re-resolved under the same predicates, so the summary cannot describe a
...
The session column SHALL keep **both** of its lines reachable when either is too long to display, since
...
The session column MUST NOT carry a copy control per row. The full id is already reachable there, and a
...
The user column SHALL show the session's `user_hash`, labelled the way the session detail page
...
Rows SHALL be openable, navigating to that session's detail view. The grid SHALL indicate that its rows
are openable rather than leaving the affordance undiscoverable, and SHALL honour the app's convention for
opening a row in a new tab. The session id SHALL be URL-encoded into the detail address, since real ids
...
- **THEN** the request carries all of them, and a returned session satisfies every one

#### Scenario: The summary follows the sort and filter state

- **WHEN** a column filter is applied
- **THEN** the whole-result session count and cost are re-resolved under the same predicates
...
- **AND** the session, project, user, turns, activity, tokens and cost columns are attributed to
...
#### Scenario: A long session id stays reachable

- **WHEN** a session id is too long to fit its column
- **THEN** the cell truncates it and the full value remains reachable

#### Scenario: Opening a row navigates to the session

- **WHEN** a grid row is opened
- **THEN** that session's detail view is navigated to, with its id URL-encoded in the address

#### Scenario: Opening a row in a new tab

- **WHEN** a grid row is opened with the app's new-tab modifier
- **THEN** the session's detail view opens in a new tab and the grid keeps its fetched pages
...
- **WHEN** the result holds zero sessions
- **THEN** the no-data content renders instead of an empty grid body

### Requirement: Session grid columns are the curated set plus every field the entity schema reports

The sessions grid SHALL offer, in addition to its curated columns, one column per field the fetched
...
are Session, Project, User, Turns, Activity, Tokens, Cost, Deployments, Topics and Rating.
...
- its tooltip from the field's `description`, **verbatim**. The descriptions are authoritative and several
  ...
  and so exceeds the session's elapsed time, and `chain_price_total` is NULL wherever no turn carries a
...
The default visible set SHALL be exactly the set visible today — Session, Project, User, Activity, Cost
and Rating — and every derived column SHALL ship hidden. Grouping constrains column order, so the default
visible **order** SHALL become Session, Activity, Project, User, Cost, Rating. The change of order is
...
session: the evaluator's DIAL deployment, its version, the input's group version, when the row was
...
the session. In particular the field whose reported `display_name` is "Model" — described by the service
as the DIAL deployment that produced the row — MUST NOT appear as a column headed "Model" without that group
above it: read bare, it is indistinguishable from the deployments the session actually used, which is
...
grid without it is a table of values belonging to sessions the reader cannot name. It SHALL declare the
rollup as its origin even though it reads the enrichment for its title — a session's identity is its id
and the title only labels it — and SHALL state in its own disclosure that the title comes from the insight
enrichment and may describe only part of the session. It SHALL state that size cap **once**, for the
...
- **THEN** the Session, Project, User, Activity and Cost columns are visible, together with Rating
- **AND** every derived column is hidden
- **AND** the visible order is Session, Activity, Project, User, Cost, Rating
...
#### Scenario: The evaluator's deployment never reads as the session's model
...
- **THEN** the Session column offers no way to hide it
...
### Requirement: Sessions grid names and filters the deployments a session used

The sessions grid SHALL present a curated **Deployments** column reading the rollup's `deployments`
array, so an operator can see which deployments served a session without opening it. The column SHALL
...
Where a per-session set of **billed models** is wanted, it SHALL come from a session-level field
...
understate a longer session — so the grid MUST NOT synthesize one.
...
recorded, a filter's matches SHALL be exactly the sessions whose visible pills satisfy it — the column
...
- **WHEN** the sessions grid loads with no stored column choice
...
- **WHEN** a session's list holds more values than the column width fits
...
- **WHEN** a session's deployments include an application resource path, a toolset resource path, an
...
- **WHEN** the operator applies a contains filter matching one of a session's rendered pills
- **THEN** that session is in the result
...
return fewer sessions than match, with nothing to say so.
...
dropped: no value matched, so no session does.
...
- **AND** a session's values are `["embedding-ada", "gpt-4o"]`
- **THEN** that session is in the result
...
- **THEN** sessions whose values include `gpt-4o` are returned
- **AND** a session whose only value is `gpt-4o-mini` is not returned

#### Scenario: A negated filter requires no element to match

- **WHEN** the operator applies a does-not-contain filter of `claude`
- **AND** a session's values are `["gpt-4o", "claude-sonnet"]`
- **THEN** that session is not in the result

#### Scenario: Text matching no value narrows the result to nothing

- **WHEN** the entered text resolves to no values
- **THEN** the result holds no sessions
...
enrichment-backed field means the enrichment has not reached that session, which is a statement about
