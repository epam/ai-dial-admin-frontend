# Analytics Session Trace Detail
...
## Requirements
### Requirement: A span's Chat states the session that span received

The **Chat** tab SHALL render the selected span's own recorded request message list as a session, in the
order the request carried it, followed by that span's assembled answer as the trailing turn. It answers what
the session looked like **at that hop** — the history a deep hop received carries sub-agent prompts,
tool results and intermediate turns that a session-level transcript never shows, and that history is
...
message in full on demand exactly as the Request tab does. A session view that silently truncates is
...
in it, has no session to state — and saying that is the answer, where fifty bubbles of tool traffic was
...
embedding probe are not sessions, and a tab that resolves to "this hop has no session" on every
...
rather than render an empty session; the raw body stays the Request tab's answer.

#### Scenario: A hop's history renders as a session

- **WHEN** a model-call span carrying prior turns is selected and Chat is opened
- **THEN** the request's messages render as a session in recorded order
...
- **THEN** the session states the user and assistant turns
...
#### Scenario: A span with no session offers no Chat
...
- **THEN** Chat states that the span received no session
- **AND** it does not render an empty session
...
sessions views already use: named only when the fetched schema reports it, through the same
...
response body was 1.4 MB and a 116-turn session's newest request body was 405 KB — so shipping them
...
bloom-filtered fast path for every session the view reads today. A session whose id came from a harness
...
no body column establishes the session's entry hops, their times, their deployments and their message
...
immediately. The bound MUST NOT be widened to the session's own span — sessions run for weeks, and
one observed session spanned 27 daily partitions, enough to exceed the budget again. The first query
...
to the reader from a session that recorded no bodies. The column has millisecond precision, so the
...
#### Scenario: Every hop-log query filters by session
...
- **AND** the bound is not widened to the session's own first and last request time
...
### Requirement: A hop's bodies are located by its trace and its span, never by the session header

The read that fetches a selected hop's request and response SHALL be located by the trace the span was read
under, the span's own id and its recorded time, and SHALL NOT be conditioned on the session or session
...
#### Scenario: A hop recorded with no session header states its body

- **WHEN** a span whose session header is empty is selected
...
shortly after the turn it rates, carrying that turn's session id, so it is reachable in the
session's trace list and opens as one row. Typing it generically would put the console's least
...
token-counting and prompt-truncation endpoints as utility rather than session, which the one-row-per-span
...
render. They sit inside session traces and orchestrate other spans: one measured turn of 18 spans has a
...
**An empty session id SHALL NOT be read as evidence that a span is outside the turn.** The field is
unpopulated on whole classes of in-turn spans — one measured 18-span turn carries an empty session id on
...
- **WHEN** a session records a rating span in its own trace
...
#### Scenario: An empty session id does not exclude a span

- **WHEN** every span of a turn records an empty session id
- **THEN** every one of them renders
- **AND** no span is excluded for that reason

#### Scenario: A background route call does not reach a session's trace

- **WHEN** a route call is the root of its own trace and belongs to no session
- **THEN** no session's trace renders a row for it
...
One 360px rail cannot hold a span's facts, its request, its response and its session at once — the facts
...
session" for a reader who has not yet picked a hop. Ordering alone does not find it: a Core-internal
root can fire long after the hop it belongs to, so the earliest span lands on the session only usually.
...
#### Scenario: A trace opens on the span whose history is the session
...
**The session tab SHALL state neither half**: it presents a history rather than a call.
...
#### Scenario: The session tab states no transport facts
...
This is stated here and not on the Chat tab: Chat leaves tool traffic out of the session entirely, so
...
**Only the members that carry the session itself SHALL be left out** — the message list, its per-dialect
...
a probe vector are not sessions. What Chat states once offered is governed by
**A span's Chat states the session that span received**.
...
#### Scenario: A hop with no session offers no Chat tab
...
reads as a control for the session rather than for the turn it opens.
...
SHALL be taken from the hop's deployment: in one measured session all 277 MCP hops shared a single parent
...
### Requirement: A failed hop states the error it recorded, in its session
...
- **THEN** the Chat tab states that error's message after the session
