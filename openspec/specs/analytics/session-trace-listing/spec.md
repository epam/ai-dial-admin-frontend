# Analytics Session Trace Listing

## Purpose

One session's page: route, guard and not-found handling, the header, the side panels and their provenance, and the trace listing that groups by trace and cards by root span. A single span's bodies are `analytics/session-trace-detail`.

## Requirements

### Requirement: Session detail route, access guard, and not-found handling

The system SHALL provide a per-session detail view at `/<lang>/sessions/<client_session_id>`, reached
by opening a row of the sessions log. The session id SHALL be carried in the path and MUST be
...
The route SHALL apply the same analytics access guard as the sessions log and SHALL render the shared
...
When no session exists for the requested id the route SHALL render the application's not-found view.
An unknown id MUST NOT render an empty detail page, because every value on it would then read as unavailable
and the page would be indistinguishable from a session whose data is genuinely missing.

Returning to the sessions log is the application navigation's responsibility. The detail view MUST NOT
own a back control, so there is one way back rather than two that can disagree.

#### Scenario: Opening a session renders its detail view

- **WHEN** a session row in the log is opened
- **THEN** the detail view for that session renders
- **AND** the address carries the session id, URL-encoded

#### Scenario: A session id containing path separators survives the round trip

- **WHEN** a session whose id contains `/` or percent-encoded characters is opened
- **THEN** the detail view resolves that exact session
...
- **WHEN** the requested session id matches no session
...
### Requirement: Single-session query over the sessions entity
...
its own, per "A session query names only fields the entity's schema reports". The columns the view has
...
render at all, and the reader has no way to tell that from a session the evaluator never reached. The
...
silent on a session it labelled.

No column SHALL be withheld for the **kind** of column it is. The enrichment's own bookkeeping — which
evaluator produced a row, and from what input — was previously excluded as not describing the session,
and that exclusion is removed: those columns are what separate a session the current evaluator has not
...
showing — a deep link or a bookmark MUST NOT fail because a session falls outside the current window.
...
#### Scenario: The query is narrowed to one session by id

- **WHEN** the single-session query is built for a session id
...
#### Scenario: An instance without the enrichment still resolves a session

- **WHEN** the single-session query is built and the schema reports no insight column
- **THEN** its select names none of them
- **AND** it names the session's own stored columns
...
#### Scenario: A session outside the log's period still resolves

- **WHEN** a session whose last activity precedes the log's selected period is opened
- **THEN** its detail view renders that session's values
...
### Requirement: A session query names only fields the entity's schema reports
...
The sessions views SHALL therefore treat the fetched entity schema as the authority on what may be
...
The single-session query alone SHALL wait on it: the reads that name no optional field — the
session's feedback and its turns — SHALL stay parallel with the schema read rather than queue behind
it.
...
- **WHEN** the sessions list query is built and the schema does not report the session title
...
- **WHEN** the schema reports the session title
...
### Requirement: Unavailable session values render an explicit placeholder
...
- **empty** — a queried source carries the field and its value is absent for this session;
...
one: a deployment that lacks a column and a session that lacks a value are different findings, and only
...
session that occurred — an elapsed duration being the case in hand, since a session that ran took
...
value is presented, so the grid and the detail view state the same thing about the same session.

A value supplied by a session-insight enrichment is a fourth case and SHALL NOT use any of the three
presentations above. The enrichment runs per session and can be absent — the evaluator has not processed
the session yet — or partial, flagged `truncated`, when the session exceeded its budget. An absent
...
under a quarter of sessions carry an insight row — so this is the common case, not an edge one.

Where such a value **labels** the session, an absent or blank one SHALL degrade to the unavailable marker
and MUST NOT degrade to the session id. Both surfaces that render a title render the id alongside it, so
substituting one for the other states the id twice and reads as though the session were named after its
own hash.
...
- **THEN** it presents no field for a session's region, because DIAL records none
...
- **WHEN** a session has zero ratings
...
- **WHEN** a session's recorded duration is `0`
...
- **WHEN** a session has no insight row, or its title is blank
- **THEN** the title renders as the unavailable marker
- **AND** the session id is not rendered in its place
...
- **WHEN** the Topics column renders a session the evaluator has not processed
...
- **WHEN** the metadata panel renders a session whose row carries no `traces` key, because the instance
...
### Requirement: Session detail header names the session and states its turn count

The header SHALL lead with the session's **title** as the view's heading, and SHALL state the
session **id** in the meta row alongside its project, turn count, activity span and time since last
activity. A heading names the thing on the page; the id addresses it. Leading with the id made every
session's heading a hash, and the reader who needs the id needs to copy it rather than read it.
...
The title SHALL be read from the session-insight enrichment. Where the enrichment carries no row for the
session, or its title is blank, the heading SHALL render the unavailable marker and MUST NOT render the
...
is a dash names nothing, so it SHALL carry an accessible name stating that the session is untitled. The
...
NOT qualify it with a truncation caveat. The flag holds for most titled sessions, so a caveat rendered on
nearly every one of them, and what it reported was a property of the evaluator's input budget rather than
anything about the session the reader is looking at.

The header MUST NOT state the session's deployments. The metadata panel states them, and one fact
...
The header MUST NOT state a **model** field. The rollup carries no session-level model column;
...
understate a session longer than that bound, the same error the turn-count rule already forbids.
Presenting a real model set requires a session-level field the rollup does not yet carry.
...
session longer than the bound the derived figure is the bound itself, stated as though it were the
session's length.

Numeric, currency and time values in the header SHALL carry the same formatting those value types carry in
the sessions log, so the same session reads identically in both places.

#### Scenario: The heading is the session's title

- **WHEN** a session's insight row carries a title
- **THEN** that title is the view's heading
- **AND** the session id is stated in the meta row

#### Scenario: An untitled session still has a named heading

- **WHEN** a session has no insight row, or its title is blank
- **THEN** the heading renders the unavailable marker
- **AND** the heading carries an accessible name stating the session is untitled
- **AND** the session id is not rendered as the heading

#### Scenario: A truncated title says so

- **WHEN** a session's insight row is flagged `truncated`
- **THEN** the title itself is stated as the heading, exactly as an untruncated title is
- **AND** the view states nothing about the title having been computed from part of the session

#### Scenario: The header states no deployments and no model

- **WHEN** a session's rollup records deployments including a router, an application and a model
...
#### Scenario: A long session id stays reachable and copyable

- **WHEN** the session id is too long to fit the meta row
...
#### Scenario: The header states the session's facts
...
- **WHEN** the detail view renders a session whose `turn_count` is 911
...
- **WHEN** a session's `turn_count` is 911 and the view loads only the first 200 turns
- **THEN** the header states 911
- **AND** it does not state 200

#### Scenario: Header values match the log

- **WHEN** the same session is read in the log and in the detail view
- **THEN** its token, cost and activity values are formatted identically in both

### Requirement: The session trace listing groups by trace and cards by root span

The listing SHALL present a session's recorded activity as **traces**, and within a trace one **card per
...
A trace SHALL belong to the session when **any** of its rows carries the session's chat id. Core
writes the session header per request, so the header routinely lands on a child row while the root
...
drops whole sessions from the listing, which is the defect this requirement replaces.
...
The root span carrying the session's own call SHALL be identified as the root carrying the chat id where
one does, and otherwise as the trace's sole root. That is a **labelling** rule, not a selection rule: every
root renders as a card, and the rule only decides which card is the session's own call. No trace records
...
#### Scenario: A trace is listed when any of its rows carries the session header

- **WHEN** a trace's root span carries no chat id and one of its child rows carries the session's
...
session's project, and a padded day range. It SHALL return exactly three things: the trace ids of the
...
Core-internal marker is a comparison against the session's project, so the value has to be read; filtering
...
**The session's project SHALL filter the paging query and MUST NOT filter the other two.** It is
...
Core-internal calls are recorded under Core's own project while the client's rows carry the session's, so
filtering by the session's project deletes exactly the cards and the rows this design added. That
...
#### Scenario: The figures query carries no session filter
...
- **AND** no increment is applied to it for a root missing from the session's row set
...
The bounds SHALL be derived from the **page's** traces rather than from the session's own span. A
session-wide window would make the figures query read one partition per day of the session's life on
...
#### Scenario: The window is derived from the page, not the session

- **WHEN** a page of traces spanning ten minutes is fetched from a session that ran for a year
- **THEN** the window covers those ten minutes plus the padding
- **AND** it does not span the session's activity
...
how much of a session can be reached.
...
A card whose root is recorded under a **different project than the session's** SHALL be marked as a
**system request** — a call the platform made rather than the client. Core makes its own service calls —
title generation and similar — under its own project, while the client's rows carry the session's, so
...
- **WHEN** a card's root is recorded under a project other than the session's
- **THEN** that card is marked as a system request

#### Scenario: A client call sharing the session's project is not marked

- **WHEN** a trace's sole root carries no chat id but carries the session's project
...
1. **One session per trace.** No trace carries two distinct non-empty chat ids. This is what licenses
...
5. **The labelling rule agrees with the Core-internal marker.** A trace carrying both a chat-id-bearing root
   and another root under the session's own project would split them; no such trace is recorded, so the
...
#### Scenario: A trace carrying two sessions trips a guard
...
#### Scenario: A second unlabelled root under the session's project trips a guard

- **WHEN** a trace carries a chat-id-bearing root and another root under the session's own project
...
### Requirement: Session detail side panels and their provenance

The detail view SHALL present its supporting fields as labelled panels: the session's insights, token
...
- A **catalog identifier**, rendered in monospace, claims **the entity the page queried**. It SHALL name
  `sessions` or the entity the session's ratings are read from, and SHALL NEVER name an
...
The **insights panel** SHALL present the session's insight enrichment. Which of its fields the panel
...
SHALL name `sessions` as its source, per the rule above. It MUST NOT restate the session's title,
which is the view's heading.

The insights panel SHALL render **only where the session carries an insight row**. Where it does not,
the view SHALL state in the panel's place, in text, that the session has not been evaluated — and MUST
NOT render the panel with its fields marked unavailable. The enrichment runs per session and reaches a
...
*this instance carries no insight enrichment at all*: the first is a session the evaluator has not
...
The recorded durations are two such fields. `duration_ms` sums a session's hop durations, and an outer
hop's duration already contains the hops it called, so a session whose turns fan out into chains reads
...
This restates a caveat previously carried only by the sessions grid's Duration column, which no longer
...
session id, a user hash or a trace id is a value of the record rather than a name in the catalog.
...
The metadata panel SHALL state the session id, the anonymized user identifier, the project, the first
activity time, the successful-request count, the session's **trace ids** and the deployments that served
the session, all from the rollup. A field the rollup carries SHALL NOT be rendered as unavailable: the
...
- **WHEN** the detail view renders a session carrying an insight row
...
- **WHEN** a session carries an insight row
- **THEN** the insights panel states what the enrichment recorded for it
- **AND** it does not restate the session's title

#### Scenario: An unevaluated session gets a statement, not a panel of dashes

- **WHEN** the detail view renders a session the insight enrichment carries no row for
- **THEN** no insights panel renders
- **AND** the view states in text that the session has not been evaluated
...
- **THEN** the view's statement distinguishes an absent enrichment from an unevaluated session
...
- **WHEN** the usage panel renders a session's duration and average duration
...
- **WHEN** the metadata panel renders a field the session-insight enrichment supplies
...
- **WHEN** the detail view renders
- **THEN** the metadata panel states the session id, user identifier, project, first activity,
  successful-request count, trace ids and the session's deployments
...
- **WHEN** a session's rollup records several trace ids
...
truncation caveat, per "Session detail header names the session and states its turn count". A
fact stated as a field in a panel is not a caveat attached to a heading.

Whether the session carries an insight row at all SHALL be decided over the enrichment's **namespace
...
depend on that field continuing to exist, and would report a session whose one field happens to be
...
- **WHEN** a session's insight row carries the enrichment's own bookkeeping fields alongside its
  descriptive ones
...
- **WHEN** a session's insight row carries no value for a field the schema still reports
...
- **AND** the session is not reported as unevaluated
...
- **THEN** the view reports the session as not yet evaluated
...
### Requirement: Session detail feedback reads the rating source

The detail view SHALL read this session's ratings from the rating source and SHALL state, **in the
feedback panel**, how many were positive and how many negative. A session with no ratings SHALL state
zero in both directions rather than rendering them as unavailable.

Those figures SHALL come from an **aggregate scoped to the session**, not from counting the rows the
panel loaded. The listed ratings are bounded, so counting them reports the bound rather than the session:
a session with more rated responses than the view requested would state the count of the ones on screen
while presenting it as the session's total. The figures SHALL therefore be exact regardless of how many
...
The feedback panel SHALL list the session's rated **responses**, most recently rated first. The rating
...
by time. The panel's own figures come from an aggregate scoped to the session rather than from what the
...
- **WHEN** a session has positive and negative ratings
- **THEN** the feedback panel states the count in each direction

#### Scenario: The counts are exact, not the loaded subset

- **WHEN** a session has more rated responses than the panel lists
- **THEN** the stated direction counts cover every rated response of the session
- **AND** they are not derived from the listed entries

#### Scenario: An unrated session reports zero

- **WHEN** a session has no ratings
...
#### Scenario: A session-wide figure is not announced as period-scoped

- **WHEN** the detail view's rating figures render
- **THEN** their accessible names state the session's ratings without claiming a selected period
...
- **WHEN** a session has more rated responses than the view requested
...
### Requirement: The session detail view presents its traces without a view switch

The session detail view SHALL present the session's **traces** and SHALL NOT offer a view switch.
The transcript is no longer a view of its own: a session's readable exchange is the request history of
...
**The session trace listing groups by trace and cards by root span** defines, each card opening its
trace's hop chain. The session's header and the supporting panels beside the listing SHALL be
unaffected, and no body read SHALL be issued when the page opens.

A session whose trace listing is empty SHALL render the listing's own empty state, and a failed listing
...
No page of the console SHALL offer a session-level transcript, and none SHALL state that one is
unavailable: an option that no longer exists is not an option to explain.

#### Scenario: The detail view opens on its traces

- **WHEN** a session's detail view loads
...
#### Scenario: A session with no traces states so

- **WHEN** a session's trace listing is empty
...
While a trace is open the session's header SHALL be replaced rather than kept above it. The trace states
...
- **THEN** its kind, status, recorded time, tokens, cost, endpoint, upstream, caller and HTTP status render
  beside the tree
- **AND** its request, response and session render in the section below the tree
...
- **WHEN** a card whose root carries no session header is opened
