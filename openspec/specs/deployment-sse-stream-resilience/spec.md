# deployment-sse-stream-resilience Specification

## Purpose

Defines error-handling and reconnect semantics for long-lived deployment SSE streams (container events, container pod logs). Specifies how the client distinguishes transient transport errors (browser-managed auto-reconnect) from terminal failures (server-sent named errors, exhausted reconnect attempts), and how the displayed buffer behaves across reconnects so that the deployment-manager backend's periodic 10-minute connection rotation does not surface spurious error toasts or drop accumulated events.
